...
__STATIC_FORCEINLINE void SCB_InvalidateICache_by_Addr(void *addr, int32_t isize)
{
	...
	if (isize > 0) {
		...
		do {
			SCB->ICIMVAU = op_addr; /* register accepts only 32byte aligned values, only
						   bits 31..5 are valid */
...
__STATIC_FORCEINLINE void SCB_InvalidateDCache_by_Addr(void *addr, int32_t dsize)
{
	...
	if (dsize > 0) {
		...
		do {
			SCB->DCIMVAC = op_addr; /* register accepts only 32byte aligned values, only
						   bits 31..5 are valid */
...
__STATIC_FORCEINLINE void SCB_CleanDCache_by_Addr(uint32_t *addr, int32_t dsize)
{
	...
	if (dsize > 0) {
		...
		do {
			SCB->DCCMVAC = op_addr; /* register accepts only 32byte aligned values, only
						   bits 31..5 are valid */
			op_addr += __SCB_DCACHE_LINE_SIZE;
			op_size -= __SCB_DCACHE_LINE_SIZE;
		} while (op_size > 0);
