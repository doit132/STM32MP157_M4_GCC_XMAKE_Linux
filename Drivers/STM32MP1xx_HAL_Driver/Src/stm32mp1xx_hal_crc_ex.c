 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_CRCEx_Polynomial_Set(CRC_HandleTypeDef *hcrc, uint32_t Pol,
					   uint32_t PolyLength)
{
	HAL_StatusTypeDef status = HAL_OK;
	uint32_t msb =
		31U; /* polynomial degree is 32 at most, so msb is initialized to max value */
 ...
 *          @arg @ref CRC_OUTPUTDATA_INVERSION_ENABLE  bit-level inversion (e.g. for a 8-bit CRC:
 * 0xB5 becomes 0xAD)
 * @retval HAL status
 */
