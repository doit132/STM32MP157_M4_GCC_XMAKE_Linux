					 ...
					 RTC_DateTypeDef *sTimeStampDate, uint32_t Format)
{
	...
	sTimeStamp->Minutes =
		(uint8_t)((tmptime & (RTC_TSTR_MNT | RTC_TSTR_MNU)) >> RTC_TSTR_MNU_Pos);
	...
	sTimeStampDate->Month =
		(uint8_t)((tmpdate & (RTC_TSDR_MT | RTC_TSDR_MU)) >> RTC_TSDR_MU_Pos);
...
void HAL_RTCEx_TimeStampIRQHandler(RTC_HandleTypeDef *hrtc)
{
	if (READ_BIT(RTC->SMISR, RTC_SMISR_TSMF) != 0U) {
		...
		#endif
		/* Clearing flags after the Callback because the content of RTC_TSTR and RTC_TSDR
		 * are cleared when TSF bit is reset.*/
		WRITE_REG(RTC->SCR, RTC_SCR_CITSF | RTC_SCR_CTSF);
	}
 ...
 *                       - This feature is meaningful in case of Low power mode to avoid any RTC
 * software execution after Wake Up. That is why when WakeUpAutoClr is set, EXTI is configured as
 * EVENT instead of Interrupt to avoid useless IRQ handler execution.
 * @retval HAL status
 */
...
HAL_StatusTypeDef HAL_RTCEx_MonotonicCounterIncrement(RTC_HandleTypeDef *hrtc, uint32_t Instance)
{
	UNUSED(hrtc);
	UNUSED(Instance);
	/* This register is read-only only and is incremented by one when a write access is done to
	   this register. This register cannot roll-over and is frozen when reaching the maximum
	   value. */
					     ...
					     RTC_ActiveTampersTypeDef *sAllTamper)
{
	...
	for (i = 0; i < RTC_TAMP_NB; i++) {
		...
		/* Mask flag only supported by TAMPER 1, 2 and 3 */
		assert_param(!((sAllTamper->TampInput[i].MaskFlag != RTC_TAMPERMASK_FLAG_DISABLE) &&
			       (i > RTC_TAMPER_3)));
	}
	assert_param(
		IS_RTC_TAMPER_TIMESTAMPONTAMPER_DETECTION(sAllTamper->TimeStampOnTamperDetection));
...
HAL_StatusTypeDef HAL_RTCEx_DeactivateActiveTampers(RTC_HandleTypeDef *hrtc)
{
	...
	CLEAR_BIT(TAMP->CR2,
		  (ATamp_mask |
		   ((ATamp_mask & (TAMP_ATCR1_TAMP1AM | TAMP_ATCR1_TAMP2AM | TAMP_ATCR1_TAMP3AM))
		    << TAMP_CR2_TAMP1MSK_Pos)));

	/* Clear tamper interrupt and event flags (WO register) of all actives tampers but not
	 * passives tampers */
					      ...
					      RTC_InternalTamperTypeDef *sIntTamper)
{
	...
	assert_param(
		IS_RTC_TAMPER_TIMESTAMPONTAMPER_DETECTION(sIntTamper->TimeStampOnTamperDetection));
					  ...
					  RTC_SecureStateTypeDef *secureState)
{
	...
	/* Warning, rtcNonSecureFeatures is only relevant if secureState->rtcSecureFull ==
	 * RTC_SECURE_FULL_NO */
	...
	secureState->backupRegisterStartZone2 =
		READ_BIT(tamp_smcr, TAMP_SMCR_BKPRWDPROT) >> TAMP_SMCR_BKPRWDPROT_Pos;
	secureState->backupRegisterStartZone3 =
		READ_BIT(tamp_smcr, TAMP_SMCR_BKPWDPROT) >> TAMP_SMCR_BKPWDPROT_Pos;
					  ...
					  RTC_SecureStateTypeDef *secureState)
{
	...
	/* RTC, rtcNonSecureFeatures is only relevant if secureState->rtcSecureFull ==
	 * RTC_SECURE_FULL_NO */
	...
	WRITE_REG(TAMP->SMCR, secureState->tampSecureFull |
				      (TAMP_SMCR_BKPRWDPROT & (secureState->backupRegisterStartZone2
							       << TAMP_SMCR_BKPRWDPROT_Pos)) |
				      (TAMP_SMCR_BKPWDPROT & (secureState->backupRegisterStartZone3
							      << TAMP_SMCR_BKPWDPROT_Pos)));
