...
#define __STM32MP1xx_HAL_VERSION                                                                   \
	((__STM32MP1xx_HAL_VERSION_MAIN << 24) | (__STM32MP1xx_HAL_VERSION_SUB1 << 16) |           \
	 (__STM32MP1xx_HAL_VERSION_SUB2 << 8) | (__STM32MP1xx_HAL_VERSION_RC))
 ...
 *   @arg SYSCFG_REGISTER_CODE: Select Code from the SYSCFG compensation cell code register
 * (SYSCFG_CCCR)
 * @retval None
 */
...
void HAL_SYSCFG_CompensationCodeConfig(uint32_t SYSCFG_PMOSCode, uint32_t SYSCFG_NMOSCode)
{
	...
	MODIFY_REG(SYSCFG->CMPCR, SYSCFG_CMPCR_RANSRC | SYSCFG_CMPCR_RAPSRC,
		   (((uint32_t)(SYSCFG_PMOSCode) << 4) | (uint32_t)(SYSCFG_NMOSCode)));
}
