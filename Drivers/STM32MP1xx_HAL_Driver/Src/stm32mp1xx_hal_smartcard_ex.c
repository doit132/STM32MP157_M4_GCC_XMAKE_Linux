...
@verbatim
  ...
  This subsection provides a set of functions allowing to initialize the SMARTCARD.
     ...
     (+) HAL_SMARTCARDEx_TimeOut_Config() API allows to configure the receiver timeout value on the
fly
...
{
	MODIFY_REG(hsmartcard->Instance->RTOR, USART_RTOR_BLEN,
		   ((uint32_t)BlockLength << USART_RTOR_BLEN_Pos));
}
...
static void SMARTCARDEx_SetNbDataToProcess(SMARTCARD_HandleTypeDef *hsmartcard)
{
	...
	if (hsmartcard->FifoMode == SMARTCARD_FIFOMODE_DISABLE) {
		...
		rx_fifo_threshold =
			(uint8_t)(READ_BIT(hsmartcard->Instance->CR3, USART_CR3_RXFTCFG) >>
				  USART_CR3_RXFTCFG_Pos);
		tx_fifo_threshold =
			(uint8_t)(READ_BIT(hsmartcard->Instance->CR3, USART_CR3_TXFTCFG) >>
				  USART_CR3_TXFTCFG_Pos);
		hsmartcard->NbTxDataToProcess =
			((uint16_t)tx_fifo_depth * numerator[tx_fifo_threshold]) /
			(uint16_t)denominator[tx_fifo_threshold];
		hsmartcard->NbRxDataToProcess =
