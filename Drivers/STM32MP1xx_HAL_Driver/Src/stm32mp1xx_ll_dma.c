 ...
 */
		...
		#define IS_LL_DMA_MEMORY_BURST(BURST)                                              \
			(((BURST) == LL_DMA_MBURST_SINGLE) || ((BURST) == LL_DMA_MBURST_INC4) ||   \
			 ((BURST) == LL_DMA_MBURST_INC8) || ((BURST) == LL_DMA_MBURST_INC16))

		#define IS_LL_DMA_PERIPHERAL_BURST(BURST)                                          \
			(((BURST) == LL_DMA_PBURST_SINGLE) || ((BURST) == LL_DMA_PBURST_INC4) ||   \
			 ((BURST) == LL_DMA_PBURST_INC8) || ((BURST) == LL_DMA_PBURST_INC16))
...
uint32_t LL_DMA_DeInit(DMA_TypeDef *DMAx, uint32_t Stream)
{
	...
	if (Stream == LL_DMA_STREAM_ALL) {
		if (DMAx == DMA1) {
			...
			LL_AHB2_GRP1_ForceReset(LL_AHB2_GRP1_PERIPH_DMA1);

			/* Release reset of DMA clock */
			LL_AHB2_GRP1_ReleaseReset(LL_AHB2_GRP1_PERIPH_DMA1);
		} else if (DMAx == DMA2) {
			/* Force reset of DMA clock */
			LL_AHB2_GRP1_ForceReset(LL_AHB2_GRP1_PERIPH_DMA2);

			/* Release reset of DMA clock */
			LL_AHB2_GRP1_ReleaseReset(LL_AHB2_GRP1_PERIPH_DMA2);
		} else {
			status = ERROR;
		}
	} else {
...
uint32_t LL_DMA_Init(DMA_TypeDef *DMAx, uint32_t Stream, LL_DMA_InitTypeDef *DMA_InitStruct)
{
	...
	if (IS_LL_DMA_ALL_STREAM_INSTANCE(DMAx, Stream)) {
		...
		if (DMA_InitStruct->FIFOMode != LL_DMA_FIFOMODE_DISABLE) {
			 ...
			 * ------------------------ Configure DMAx_Streamy:  fifo mode and fifo
			...
			/*---------------------------- DMAx SxCR Configuration
			 * -------------------------- Configure DMAx_Streamy:  memory burst transfer
			 * with parameters :
			...
			/*---------------------------- DMAx SxCR Configuration
			 * -------------------------- Configure DMAx_Streamy:  peripheral burst
