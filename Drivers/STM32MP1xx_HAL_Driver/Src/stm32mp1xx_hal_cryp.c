/**
  ...
  ==============================================================================
    [..]
	 ...
	 new parametres, finally user can  start encryption/decryption.
       ...
       (#)To process a single message with consecutive calls to HAL_CRYP_Encrypt() or
 HAL_CRYP_Decrypt() without having to configure again the Key or the Initialization Vector between
 each API call, the field KeyIVConfigSkip of the initialization structure must be set to
 CRYP_KEYIVCONFIG_ONCE. Same is true for consecutive calls of HAL_CRYP_Encrypt_IT(),
 HAL_CRYP_Decrypt_IT(), HAL_CRYP_Encrypt_DMA() or HAL_CRYP_Decrypt_DMA().
    ...
    [..]  This section describes the AES Galois/counter mode (GCM) supported by both CRYP1 IP:
	 ...
	 (##) The last block is composed of the length of A (on 64 bits) and the length of
 ciphertext (on 64 bits)
    ...
    [..]  This section describe The AES Counter with Cipher Block Chaining-Message
	 ...
	 (##) B0 block  : According to NIST Special Publication 800-38C,
	    ...
	    most-significant-byte first order(see below table 3)
	      ...
	      (+++)  q The octet length of the binary representation of the octet length of the
 payload
 ...
 Table 4. CTRx block
		Bit Number    Register           Contents
		----------   ---------------       -----------
		127 ...96    CRYP_IV1R[31:0]     B0[127:96], where Q length bits are set to 0,
 except for bit 0 that is set to 1 95  ...64    CRYP_IV1L[31:0]     B0[95:64] 63 ... 32
 CRYP_IV0R[31:0]     B0[63:32] 31 ... 0     CRYP_IV0L[31:0]     B0[31:0], where flag bits set to 0
 ...
 */
		...
		#define CRYP_TIMEOUT_KEYPREPARATION                                                \
			82U /*The latency of key preparation operation is 82 clock cycles.*/
		#define CRYP_TIMEOUT_GCMCCMINITPHASE                                               \
			299U /*  The latency of  GCM/CCM init phase to prepare hash subkey is 299  \
				clock cycles.*/
		#define CRYP_TIMEOUT_GCMCCMHEADERPHASE                                             \
			290U /*  The latency of  GCM/CCM header phase is 290 clock cycles.*/

		#define CRYP_PHASE_READY                                                           \
			0x00000001U /*!< CRYP peripheral is ready for initialization. */
		#define CRYP_PHASE_PROCESS                                                         \
			0x00000002U /*!< CRYP peripheral is in processing phase */
	 ...
	 */
		...
		#define CRYP_SET_PHASE(__HANDLE__, __PHASE__)                                      \
			do {                                                                       \
				(__HANDLE__)->Instance->CR &= (uint32_t)(~CRYP_CR_GCM_CCMPH);      \
				(__HANDLE__)->Instance->CR |= (uint32_t)(__PHASE__);               \
			} while (0)

		#define HAL_CRYP_FIFO_FLUSH(__HANDLE__)                                            \
			((__HANDLE__)->Instance->CR |= CRYP_CR_FFLUSH)
...
static void CRYP_SetDMAConfig(CRYP_HandleTypeDef *hcryp, uint32_t inputaddr, uint16_t Size,
			      uint32_t outputaddr);
static void CRYP_DMAInCplt(DMA_HandleTypeDef *hdma);
static void CRYP_DMAOutCplt(DMA_HandleTypeDef *hdma);
static void CRYP_DMAError(DMA_HandleTypeDef *hdma);
		#ifdef HAL_MDMA_MODULE_ENABLED
static void CRYP_SetMDMAConfig(CRYP_HandleTypeDef *hcryp, uint32_t inputaddr, uint16_t Size,
			       uint32_t outputaddr);
...
@verbatim
  ...
  ========================================================================================
    [..]  This section provides functions allowing to:
      ...
      (+) configure CRYP (HAL_CRYP_SetConfig) with the specified parameters in the
CRYP_ConfigTypeDef Parameters which are configured in This section are :
	  (++) Key size
...
HAL_StatusTypeDef HAL_CRYP_Init(CRYP_HandleTypeDef *hcryp)
{
	...
	/* Set the key size(This bit field is ‘don’t care’ in the DES or TDES modes) data type and
	 * Algorithm */
...
HAL_StatusTypeDef HAL_CRYP_SetConfig(CRYP_HandleTypeDef *hcryp, CRYP_ConfigTypeDef *pConf)
{
	...
	if (hcryp->State == HAL_CRYP_STATE_READY) {
		...
		/* Set the key size(This bit field is ‘don’t care’ in the DES or TDES modes) data
		 * type, AlgoMode and operating mode*/
		MODIFY_REG(hcryp->Instance->CR,
			   CRYP_CR_DATATYPE | CRYP_CR_KEYSIZE | CRYP_CR_ALGOMODE,
			   hcryp->Init.DataType | hcryp->Init.KeySize | hcryp->Init.Algorithm);
	...
	if (hcryp->State == HAL_CRYP_STATE_READY) {
		...
		case HAL_CRYP_INPUT_COMPLETE_CB_ID:
			hcryp->InCpltCallback =
				HAL_CRYP_InCpltCallback; /* Legacy weak  InCpltCallback  */
			break;

		case HAL_CRYP_OUTPUT_COMPLETE_CB_ID:
			hcryp->OutCpltCallback =
				HAL_CRYP_OutCpltCallback; /* Legacy weak OutCpltCallback       */
			break;
				   ...
				   uint32_t *Output, uint32_t Timeout)
{
	...
	if (hcryp->State == HAL_CRYP_STATE_READY) {
		...
		/*  Reset CrypInCount, CrypOutCount and Initialize pCrypInBuffPtr, pCrypOutBuffPtr
		 * and Size parameters*/
		...
		case CRYP_TDES_CBC:
			...
			if ((hcryp->Init.Algorithm == CRYP_TDES_ECB) ||
			...
			if ((hcryp->Init.Algorithm == CRYP_DES_CBC) ||
			    (hcryp->Init.Algorithm == CRYP_TDES_CBC)) {
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_CRYP_Encrypt_IT(CRYP_HandleTypeDef *hcryp, uint32_t *Input, uint16_t Size,
				      uint32_t *Output)
{
	uint32_t algo;
	HAL_StatusTypeDef status = HAL_OK;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_CRYP_Decrypt_IT(CRYP_HandleTypeDef *hcryp, uint32_t *Input, uint16_t Size,
				      uint32_t *Output)
{
	uint32_t algo;
	HAL_StatusTypeDef status = HAL_OK;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_CRYP_Encrypt_DMA(CRYP_HandleTypeDef *hcryp, uint32_t *Input, uint16_t Size,
				       uint32_t *Output)
{
	uint32_t algo;
	HAL_StatusTypeDef status = HAL_OK;
	...
	if (hcryp->State == HAL_CRYP_STATE_READY) {
		...
		case CRYP_TDES_CBC:
			...
			if (hcryp->hdmain != NULL) {
				...
				CRYP_SetMDMAConfig(hcryp, (uint32_t)(hcryp->pCrypInBuffPtr),
						   (hcryp->Size / 4U),
						   (uint32_t)(hcryp->pCrypOutBuffPtr));
			}
		#endif
		...
		case CRYP_AES_CTR:
			...
			if (hcryp->Init.KeyIVConfigSkip == CRYP_KEYIVCONFIG_ONCE) {
				if (hcryp->KeyIVConfig == 1U) {
					...
					/* If the Key and IV configuration has to be done only once
					   and if it has not been done already, do it and set
					   KeyIVConfig to keep track it won't have to be done again
					   next time */
			...
			if (DoKeyIVConfig == 1U) {
				...
				if (hcryp->Init.Algorithm != CRYP_AES_ECB) {
					hcryp->Instance->IV0LR =
						*(uint32_t *)(hcryp->Init.pInitVect);
					hcryp->Instance->IV0RR =
						*(uint32_t *)(hcryp->Init.pInitVect + 1U);
					hcryp->Instance->IV1LR =
						*(uint32_t *)(hcryp->Init.pInitVect + 2U);
					hcryp->Instance->IV1RR =
						*(uint32_t *)(hcryp->Init.pInitVect + 3U);
				}
			} /* if (DoKeyIVConfig == 1U) */
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_CRYP_Decrypt_DMA(CRYP_HandleTypeDef *hcryp, uint32_t *Input, uint16_t Size,
				       uint32_t *Output)
{
	uint32_t algo;
	HAL_StatusTypeDef status = HAL_OK;
...
static HAL_StatusTypeDef CRYP_TDES_Process(CRYP_HandleTypeDef *hcryp, uint32_t Timeout)
{
	...
	while ((hcryp->CrypInCount < (hcryp->Size / 4U)) && (outcount < (hcryp->Size / 4U))) {
		...
		if (((hcryp->Instance->SR & CRYP_FLAG_IFNF) != 0x0U) &&
		    (incount < (hcryp->Size / 4U))) {
			/* Write the input block in the IN FIFO */
			hcryp->Instance->DIN =
				*(uint32_t *)(hcryp->pCrypInBuffPtr + hcryp->CrypInCount);
			hcryp->CrypInCount++;
			hcryp->Instance->DIN =
				*(uint32_t *)(hcryp->pCrypInBuffPtr + hcryp->CrypInCount);
			hcryp->CrypInCount++;
	...
	if (hcryp->State == HAL_CRYP_STATE_BUSY) {
		...
		if (__HAL_CRYP_GET_IT(hcryp, CRYP_IT_OUTI) != 0x0U) {
			if (__HAL_CRYP_GET_FLAG(hcryp, CRYP_FLAG_OUTRIS) != 0x0U) {
				/* Read the output block from the Output FIFO and put them in
				 * temporary Buffer then get CrypOutBuff from temporary buffer  */
				temp = hcryp->Instance->DOUT;
				*(uint32_t *)(hcryp->pCrypOutBuffPtr + (hcryp->CrypOutCount)) =
					temp;
				hcryp->CrypOutCount++;
				temp = hcryp->Instance->DOUT;
				*(uint32_t *)(hcryp->pCrypOutBuffPtr + (hcryp->CrypOutCount)) =
...
static void CRYP_DMAOutCplt(DMA_HandleTypeDef *hdma)
{
	...
	if (((hcryp->Size) % 16U) != 0U) {
		...
		if (((temp_cr_algodir == CRYP_OPERATINGMODE_ENCRYPT) &&
		     (hcryp->Init.Algorithm == CRYP_AES_GCM)) ||
		    ((temp_cr_algodir == CRYP_OPERATINGMODE_DECRYPT) &&
			      ...
			      uint32_t outputaddr)
{
	...
	if (HAL_DMA_Start_IT(hcryp->hdmain, inputaddr, (uint32_t)&hcryp->Instance->DIN, Size) !=
	    HAL_OK) {
		/* DMA error code field */
		hcryp->ErrorCode |= HAL_CRYP_ERROR_DMA;
	...
	if (HAL_DMA_Start_IT(hcryp->hdmaout, (uint32_t)&hcryp->Instance->DOUT, outputaddr, Size) !=
	    HAL_OK) {
		/* DMA error code field */
		hcryp->ErrorCode |= HAL_CRYP_ERROR_DMA;
			       ...
			       uint32_t outputaddr)
{
	...
	/* Enable the input DMA Stream */
	if (HAL_MDMA_Start_IT(hcryp->hmdmain, inputaddr, (uint32_t)&hcryp->Instance->DIN,
			      (Size * 4U), 1U) != HAL_OK) {
		/* DMA error code field */
		hcryp->ErrorCode |= HAL_CRYP_ERROR_DMA;
	...
	/* Enable the output DMA Stream */
	if (HAL_MDMA_Start_IT(hcryp->hmdmaout, (uint32_t)&hcryp->Instance->DOUT, outputaddr,
			      Size * 4U, 1U) != HAL_OK) {
		/* DMA error code field */
		hcryp->ErrorCode |= HAL_CRYP_ERROR_DMA;
...
/**
 * @brief  Encryption/Decryption process in AES GCM mode and prepare the authentication TAG in
 * interrupt mode
 * @param  hcryp: pointer to a CRYP_HandleTypeDef structure that contains
 *         the configuration information for CRYP module
 * @retval HAL status
...
/**
 * @brief  Encryption/Decryption process in AES GCM mode and prepare the authentication TAG using
 * DMA
 * @param  hcryp: pointer to a CRYP_HandleTypeDef structure that contains
 *         the configuration information for CRYP module
 * @retval HAL status
...
static HAL_StatusTypeDef CRYP_AESGCM_Process_DMA(CRYP_HandleTypeDef *hcryp)
{
	...
	if (hcryp->Size == 0U) {
		...
		/* for STM32H7 below rev.B : Size should be %4  otherwise Tag will  be incorrectly
		generated for GCM Encryption: Workaround is implemented in polling mode, so if last
		block of payload <128bit don't use DMA mode otherwise TAG is incorrectly generated
	      */
		...
		if (hcryp->hdmain != NULL)
			CRYP_SetDMAConfig(hcryp, (uint32_t)(hcryp->pCrypInBuffPtr),
					  (uint16_t)wordsize, (uint32_t)(hcryp->pCrypOutBuffPtr));
		#ifdef HAL_MDMA_MODULE_ENABLED
		else if (hcryp->hmdmain != NULL)
			CRYP_SetMDMAConfig(hcryp, (uint32_t)(hcryp->pCrypInBuffPtr),
					   (uint16_t)wordsize, (uint32_t)(hcryp->pCrypOutBuffPtr));
...
static HAL_StatusTypeDef CRYP_AESCCM_Process_DMA(CRYP_HandleTypeDef *hcryp)
{
	...
	if (hcryp->Size == 0U) {
		...
		/* for STM32H7 below rev.B ::  Size should be %4  otherwise Tag will  be incorrectly
		 * generated for CCM Decryption, Workaround is implemented in polling mode*/
		/*DMA transfer must not include the last block in case of Size is not %16 */
		wordsize = wordsize - (wordsize % 4U);
...
static void CRYP_GCMCCM_SetPayloadPhase_IT(CRYP_HandleTypeDef *hcryp)
{
	...
	if (hcryp->Size == 0U) {
	...
	else if ((((hcryp->Size / 4U) - (hcryp->CrypInCount)) >= 4U) && (negative == 0U)) {
		if ((hcryp->Instance->IMSCR & CRYP_IMSCR_INIM) != 0x0U) {
			...
			if (hcryp->CrypOutCount < (hcryp->Size / 4U)) {
				if ((hcryp->Instance->SR & CRYP_FLAG_OFNE) != 0x0U) {
					/* Read the output block from the Output FIFO and put them
					 * in temporary buffer then get CrypOutBuff from temporary
					 * buffer  */
					for (i = 0U; i < 4U; i++) {
						temp[i] = hcryp->Instance->DOUT;
					}
					i = 0U;
					while (((hcryp->CrypOutCount < ((hcryp->Size) / 4U))) &&
					       (i < 4U)) {
						*(uint32_t *)(hcryp->pCrypOutBuffPtr +
							      hcryp->CrypOutCount) = temp[i];
						hcryp->CrypOutCount++;
						i++;
					}
					if (((hcryp->Size / 4U) == hcryp->CrypOutCount) &&
					    ((hcryp->Size % 16U) == 0U)) {
						/* Disable interrupts */
						__HAL_CRYP_DISABLE_IT(hcryp, CRYP_IT_OUTI);
		...
		if ((hcryp->Instance->IMSCR & CRYP_IMSCR_INIM) != 0x0U) {
			/* Compute the number of padding bytes in last block of payload */
			npblb = ((((uint32_t)hcryp->Size / 16U) + 1U) * 16U) -
				(uint32_t)(hcryp->Size);

			/* Set Npblb in case of AES GCM payload encryption and CCM decryption to get
			 * right tag */
			temp_cr_algodir = hcryp->Instance->CR & CRYP_CR_ALGODIR;
...
static HAL_StatusTypeDef CRYP_GCMCCM_SetHeaderPhase(CRYP_HandleTypeDef *hcryp, uint32_t Timeout)
{
	...
	if ((hcryp->Init.HeaderSize != 0U)) {
		...
		if ((hcryp->Init.HeaderSize % 4U) == 0U) {
			...
			for (loopcounter = 0U; (loopcounter < hcryp->Init.HeaderSize);
			     loopcounter += 4U) {
				hcryp->Instance->DIN =
					*(uint32_t *)(hcryp->Init.Header + hcryp->CrypHeaderCount);
			...
			for (loopcounter = 0U; (loopcounter < (hcryp->Init.HeaderSize % 4U));
			     loopcounter++) {
				hcryp->Instance->DIN =
					*(uint32_t *)(hcryp->Init.Header + hcryp->CrypHeaderCount);
				hcryp->CrypHeaderCount++;
