...
HAL_StatusTypeDef HAL_EXTI_SetConfigLine(EXTI_HandleTypeDef *hexti, EXTI_ConfigTypeDef *pExtiConfig)
{
	...
	/*Set Interrupt And Event Mask for Core 1 if configuration for Core 1 given into parameter
	 * mode */
	...
	/*Set Interrupt And Event Mask for Core 2 if configuration for Core 2 given into parameter
	 * mode  */
...
HAL_StatusTypeDef HAL_EXTI_GetConfigLine(EXTI_HandleTypeDef *hexti, EXTI_ConfigTypeDef *pExtiConfig)
{
	...
	if ((pExtiConfig->Line & EXTI_CONFIG) != 0x0u) {
		...
		if ((pExtiConfig->Line & EXTI_GPIO) == EXTI_GPIO) {
			...
			regval = EXTI->EXTICR[linepos >> 2u];
			pExtiConfig->GPIOSel =
				((regval << (EXTI_EXTICR1_EXTI1_Pos * (3uL - (linepos & 0x03u)))) >>
				 24);
		}
	}
