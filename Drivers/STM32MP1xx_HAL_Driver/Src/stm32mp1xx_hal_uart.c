...
HAL_StatusTypeDef HAL_UART_Transmit_DMA(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t Size)
{
	...
	if (huart->gState == HAL_UART_STATE_READY) {
		...
		if (huart->hmdmatx != NULL) {
			...
			if (HAL_MDMA_Start_IT(huart->hmdmatx, (uint32_t)huart->pTxBuffPtr,
					      (uint32_t)&huart->Instance->TDR, Size, 1) != HAL_OK) {
...
HAL_StatusTypeDef HAL_UART_Receive_DMA(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t Size)
{
	...
	if (huart->RxState == HAL_UART_STATE_READY) {
		...
		if (huart->hmdmarx != NULL) {
			...
			if (HAL_MDMA_Start_IT(huart->hmdmarx, (uint32_t)&huart->Instance->RDR,
					      (uint32_t)huart->pRxBuffPtr, Size, 1) != HAL_OK) {
				/* Set error code to DMA */
				huart->ErrorCode = HAL_UART_ERROR_DMA;
...
HAL_StatusTypeDef HAL_UART_Abort_IT(UART_HandleTypeDef *huart)
{
	...
	/* Disable interrupts */
	CLEAR_BIT(huart->Instance->CR1, (USART_CR1_PEIE | USART_CR1_TCIE |
	...
	if (HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAT)) {
		...
		if (huart->hmdmatx != NULL) {
			...
			will lead to call HAL_UART_AbortCpltCallback() at end of MDMA abort
			procedure */
...
HAL_StatusTypeDef HAL_UART_AbortTransmit_IT(UART_HandleTypeDef *huart)
{
	...
	if (HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAT)) {
		...
		/* Abort the UART MDMA Tx channel : use non blocking MDMA Abort API (callback) */
			...
			/* Abort MDMA TX */
			if (HAL_MDMA_Abort_IT(huart->hmdmatx) != HAL_OK) {
				/* Call Directly huart->hmdmatx->XferAbortCallback function in case
				 * of error */
...
HAL_StatusTypeDef HAL_UART_AbortReceive_IT(UART_HandleTypeDef *huart)
{
	...
	if (HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAR)) {
		...
		/* Abort the UART MDMA Rx channel : use non blocking MDMA Abort API (callback) */
			...
			/* Abort MDMA RX */
			if (HAL_MDMA_Abort_IT(huart->hmdmarx) != HAL_OK) {
				/* Call Directly huart->hmdmarx->XferAbortCallback function in case
				 * of error */
...
void HAL_UART_IRQHandler(UART_HandleTypeDef *huart)
{
			...
			if ((HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAR)) ||
				...
				if (HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAR)) {
					...
					if (huart->hdmarx != NULL) {
						...
						if (HAL_DMA_Abort_IT(huart->hdmarx) != HAL_OK) {
							/* Call Directly
							 * huart->hdmarx->XferAbortCallback function
							 * in case of error */
							huart->hdmarx->XferAbortCallback(
								huart->hdmarx);
						}
					...
					/* Abort the UART MDMA Rx channel */
						...
						/* Set the UART MDMA Abort callback :
						    will lead to call HAL_UART_ErrorCallback() at
						   end of MDMA abort procedure */
						huart->hmdmarx->XferAbortCallback =
							UART_MDMAAbortOnError;
	...
	if (((isrflags & USART_ISR_WUF) != 0U) && ((cr3its & USART_CR3_WUFIE) != 0U)) {
		...
		/* UART Rx state is not reset as a reception process might be ongoing.
		   If UART handle state fields need to be reset to READY, this could be done in
		   Wakeup callback */
...
HAL_StatusTypeDef UART_SetConfig(UART_HandleTypeDef *huart)
{
	...
	if (huart->Init.OverSampling == UART_OVERSAMPLING_8) {
		...
		case UART_CLOCKSOURCE_PLL3Q:
			...
			usartdiv = (uint16_t)(UART_DIV_SAMPLING8(pll3_clocks.PLL3_Q_Frequency,
								 huart->Init.BaudRate,
								 huart->Init.ClockPrescaler));
			break;
		case UART_CLOCKSOURCE_PLL4Q:
			HAL_RCC_GetPLL4ClockFreq(&pll4_clocks);
			usartdiv = (uint16_t)(UART_DIV_SAMPLING8(pll4_clocks.PLL4_Q_Frequency,
								 huart->Init.BaudRate,
								 huart->Init.ClockPrescaler));
			break;
		...
		case UART_CLOCKSOURCE_CSI:
			usartdiv = (uint16_t)(UART_DIV_SAMPLING8(CSI_VALUE, huart->Init.BaudRate,
								 huart->Init.ClockPrescaler));
			break;
		case UART_CLOCKSOURCE_HSE:
			usartdiv = (uint16_t)(UART_DIV_SAMPLING8(HSE_VALUE, huart->Init.BaudRate,
								 huart->Init.ClockPrescaler));
			break;
		default:
			ret = HAL_ERROR;
		...
		case UART_CLOCKSOURCE_PLL3Q:
			HAL_RCC_GetPLL3ClockFreq(&pll3_clocks);
			usartdiv = (uint16_t)(UART_DIV_SAMPLING16(pll3_clocks.PLL3_Q_Frequency,
								  huart->Init.BaudRate,
								  huart->Init.ClockPrescaler));
			break;
		case UART_CLOCKSOURCE_PLL4Q:
			HAL_RCC_GetPLL4ClockFreq(&pll4_clocks);
			usartdiv = (uint16_t)(UART_DIV_SAMPLING16(pll4_clocks.PLL4_Q_Frequency,
								  huart->Init.BaudRate,
								  huart->Init.ClockPrescaler));
			break;
		case UART_CLOCKSOURCE_HSI:
			...
			break;
		case UART_CLOCKSOURCE_CSI:
			usartdiv = (uint16_t)(UART_DIV_SAMPLING16(CSI_VALUE, huart->Init.BaudRate,
								  huart->Init.ClockPrescaler));
			break;
		case UART_CLOCKSOURCE_HSE:
			usartdiv = (uint16_t)(UART_DIV_SAMPLING16(HSE_VALUE, huart->Init.BaudRate,
								  huart->Init.ClockPrescaler));
			break;
		default:
			ret = HAL_ERROR;
 ...
 *         HAL_UART_AbortTransmit_IT API (Abort only Tx transfer)
 *         (This callback is executed at end of MDMA Tx Abort procedure following user abort
 * request, and leads to user Tx Abort Complete callback execution).
 * @param  hmdma MDMA handle.
 * @retval None
 */
static void UART_MDMATxOnlyAbortCallback(MDMA_HandleTypeDef *hmdma)
 ...
 *         HAL_UART_AbortReceive_IT API (Abort only Rx transfer)
 *         (This callback is executed at end of MDMA Rx Abort procedure following user abort
 * request, and leads to user Rx Abort Complete callback execution).
 * @param  hmdma MDMA handle.
 * @retval None
 */
static void UART_MDMARxOnlyAbortCallback(MDMA_HandleTypeDef *hmdma)
