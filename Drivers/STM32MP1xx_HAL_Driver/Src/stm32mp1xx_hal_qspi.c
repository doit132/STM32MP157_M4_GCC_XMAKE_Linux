/**
 ...
 match is reached.
    ...
    *** MDMA functional mode ***
    ====================================
    [..]
      (#) Configure the SourceInc and DestinationInc of MDMA parameters in the HAL_QSPI_MspInit()
 function :
	 (++) MDMA settings for write operation :
	  (+) The DestinationInc should be MDMA_DEST_INC_DISABLE
	  (+) The SourceInc must be a value of MDMA_Source_increment_mode (Except the
 MDMA_SRC_INC_DOUBLEWORD).
	  (+) The SourceDataSize must be a value of MDMA Source data size (Except the
 MDMA_SRC_DATASIZE_DOUBLEWORD) aligned with MDMA_Source_increment_mode .
	  (+) The DestDataSize must be a value of MDMA Destination data size (Except the
 MDMA_DEST_DATASIZE_DOUBLEWORD)
	 (++) MDMA settings for read operation :
	  (+) The SourceInc should be MDMA_SRC_INC_DISABLE
	  (+) The DestinationInc must be a value of MDMA_Destination_increment_mode (Except the
 MDMA_DEST_INC_DOUBLEWORD).
	  (+) The SourceDataSize must be a value of MDMA Source data size (Except the
 MDMA_SRC_DATASIZE_DOUBLEWORD) .
	  (+) The DestDataSize must be a value of MDMA Destination data size (Except the
 MDMA_DEST_DATASIZE_DOUBLEWORD) aligned with MDMA_Destination_increment_mode.
	 (++)The buffer Transfer Length (BufferTransferLength) = number of bytes in the FIFO
 (FifoThreshold) of the Quadspi.
      (#)In case of wrong MDMA setting
	(++) For write operation :
	 (+) If the DestinationInc is different to MDMA_DEST_INC_DISABLE , it will be disabled by
 the HAL_QSPI_Transmit_DMA().
	(++) For read operation :
	 (+) If the SourceInc is not set to MDMA_SRC_INC_DISABLE , it will be disabled by the
...
#include "stm32mp1xx_hal.h"
	 ...
	 */
		...
		#define QSPI_FUNCTIONAL_MODE_INDIRECT_READ                                         \
			((uint32_t)QUADSPI_CCR_FMODE_0) /*!<Indirect read mode*/
		#define QSPI_FUNCTIONAL_MODE_AUTO_POLLING                                          \
			((uint32_t)QUADSPI_CCR_FMODE_1) /*!<Automatic polling mode*/
		#define QSPI_FUNCTIONAL_MODE_MEMORY_MAPPED                                         \
			((uint32_t)QUADSPI_CCR_FMODE) /*!<Memory-mapped mode*/
		/**
		 * @}
		 */
...
HAL_StatusTypeDef HAL_QSPI_Init(QSPI_HandleTypeDef *hqspi)
{
	...
	if (status == HAL_OK) {
		...
		MODIFY_REG(hqspi->Instance->CR,
			   (QUADSPI_CR_PRESCALER | QUADSPI_CR_SSHIFT | QUADSPI_CR_FSEL |
			    QUADSPI_CR_DFM),
			   ((hqspi->Init.ClockPrescaler << QUADSPI_CR_PRESCALER_Pos) |
			    hqspi->Init.SampleShifting | hqspi->Init.FlashID |
	...
	/* QSPI Transfer Complete interrupt occurred -------------------------------*/
		...
		if (hqspi->State == HAL_QSPI_STATE_BUSY_INDIRECT_TX) {
			if ((hqspi->Instance->CR & QUADSPI_CR_DMAEN) != 0U) {
				/* Disable the DMA transfer by clearing the DMAEN bit in the QSPI CR
				 * register */
			...
			if ((hqspi->Instance->CR & QUADSPI_CR_DMAEN) != 0U) {
				...
				while (READ_BIT(hqspi->Instance->SR, QUADSPI_SR_FLEVEL) != 0U) {
					if (hqspi->RxXferCount > 0U) {
						/* Read the last data received in the FIFO until it
						 * is empty */
						*hqspi->pRxBuffPtr = *((__IO uint8_t *)data_reg);
						hqspi->pRxBuffPtr++;
						hqspi->RxXferCount--;
	...
	if (hqspi->State == HAL_QSPI_STATE_READY) {
		...
		if (status == HAL_OK) {
			...
			if (cmd->DataMode == QSPI_DATA_NONE) {
				...
				configuration is done so wait until TC flag is set to go back in
				idle state */
				status = QSPI_WaitFlagStateUntilTimeout(hqspi, QSPI_FLAG_TC, SET,
									tickstart, Timeout);
...
HAL_StatusTypeDef HAL_QSPI_Command_IT(QSPI_HandleTypeDef *hqspi, QSPI_CommandTypeDef *cmd)
{
	...
	if (hqspi->State == HAL_QSPI_STATE_READY) {
		...
		if (status == HAL_OK) {
			...
			if (cmd->DataMode == QSPI_DATA_NONE) {
				/* When there is no data phase, the transfer start as soon as the
				configuration is done so activate TC and TE interrupts */
				/* Process unlocked */
				__HAL_UNLOCK(hqspi);
	...
	if (hqspi->State == HAL_QSPI_STATE_READY) {
		...
		if (pData != NULL) {
			...
			while (hqspi->TxXferCount > 0U) {
				...
				status = QSPI_WaitFlagStateUntilTimeout(hqspi, QSPI_FLAG_FT, SET,
									tickstart, Timeout);
		...
		if (pData != NULL) {
			...
			while (hqspi->RxXferCount > 0U) {
				...
				status = QSPI_WaitFlagStateUntilTimeout(
					hqspi, (QSPI_FLAG_FT | QSPI_FLAG_TC), SET, tickstart,
					Timeout);
...
HAL_StatusTypeDef HAL_QSPI_Transmit_IT(QSPI_HandleTypeDef *hqspi, uint8_t *pData)
{
	...
	if (hqspi->State == HAL_QSPI_STATE_READY) {
		...
		if (pData != NULL) {
			...
			/* Enable the QSPI transfer error, FIFO threshold and transfer complete
			 * Interrupts */
...
HAL_StatusTypeDef HAL_QSPI_Transmit_DMA(QSPI_HandleTypeDef *hqspi, uint8_t *pData)
{
	...
	if (hqspi->State == HAL_QSPI_STATE_READY) {
		...
		if (pData != NULL) {
			...
			/* In Transmit mode , the MDMA destination is the QSPI DR register : Force
			 * the MDMA Destination Increment to disable */
			MODIFY_REG(hqspi->hmdma->Instance->CTCR,
				   (MDMA_CTCR_DINC | MDMA_CTCR_DINCOS), MDMA_DEST_INC_DISABLE);

			/* Update MDMA configuration with the correct SourceInc field for Write
			 * operation */
			if (hqspi->hmdma->Init.SourceDataSize == MDMA_SRC_DATASIZE_BYTE) {
				MODIFY_REG(hqspi->hmdma->Instance->CTCR,
					   (MDMA_CTCR_SINC | MDMA_CTCR_SINCOS), MDMA_SRC_INC_BYTE);
			} else if (hqspi->hmdma->Init.SourceDataSize ==
				   MDMA_SRC_DATASIZE_HALFWORD) {
				MODIFY_REG(hqspi->hmdma->Instance->CTCR,
					   (MDMA_CTCR_SINC | MDMA_CTCR_SINCOS),
					   MDMA_SRC_INC_HALFWORD);
			} else if (hqspi->hmdma->Init.SourceDataSize == MDMA_SRC_DATASIZE_WORD) {
				MODIFY_REG(hqspi->hmdma->Instance->CTCR,
					   (MDMA_CTCR_SINC | MDMA_CTCR_SINCOS), MDMA_SRC_INC_WORD);
			} else {
			...
			if (HAL_MDMA_Start_IT(hqspi->hmdma, (uint32_t)pData,
				...
				/* Enable the MDMA transfer by setting the DMAEN bit not needed for
				 * MDMA*/
			} else {
				status = HAL_ERROR;
				hqspi->ErrorCode |= HAL_QSPI_ERROR_DMA;
...
HAL_StatusTypeDef HAL_QSPI_Receive_DMA(QSPI_HandleTypeDef *hqspi, uint8_t *pData)
{
	...
	if (hqspi->State == HAL_QSPI_STATE_READY) {
		...
		if (pData != NULL) {
			...
			/* In Receive mode , the MDMA source is the QSPI DR register : Force the
			 * MDMA Source Increment to disable */
			MODIFY_REG(hqspi->hmdma->Instance->CTCR,
				   (MDMA_CTCR_SINC | MDMA_CTCR_SINCOS), MDMA_SRC_INC_DISABLE);

			/* Update MDMA configuration with the correct DestinationInc field for read
			 * operation */
			if (hqspi->hmdma->Init.DestDataSize == MDMA_DEST_DATASIZE_BYTE) {
				MODIFY_REG(hqspi->hmdma->Instance->CTCR,
					   (MDMA_CTCR_DINC | MDMA_CTCR_DINCOS), MDMA_DEST_INC_BYTE);
			} else if (hqspi->hmdma->Init.DestDataSize == MDMA_DEST_DATASIZE_HALFWORD) {
				MODIFY_REG(hqspi->hmdma->Instance->CTCR,
					   (MDMA_CTCR_DINC | MDMA_CTCR_DINCOS),
					   MDMA_DEST_INC_HALFWORD);
			} else if (hqspi->hmdma->Init.DestDataSize == MDMA_DEST_DATASIZE_WORD) {
				MODIFY_REG(hqspi->hmdma->Instance->CTCR,
					   (MDMA_CTCR_DINC | MDMA_CTCR_DINCOS), MDMA_DEST_INC_WORD);
			} else {
			...
			/* Enable the MDMA */
			if (HAL_MDMA_Start_IT(hqspi->hmdma, (uint32_t)&hqspi->Instance->DR,
					      (uint32_t)pData, hqspi->RxXferSize, 1) == HAL_OK) {
				/* Process unlocked */
				__HAL_UNLOCK(hqspi);
				...
				/* Enable the MDMA transfer by setting the DMAEN bit in the QSPI CR
				 * register */
				       ...
				       QSPI_AutoPollingTypeDef *cfg, uint32_t Timeout)
{
	...
	if (hqspi->State == HAL_QSPI_STATE_READY) {
		...
		if (status == HAL_OK) {
			...
			status = QSPI_WaitFlagStateUntilTimeout(hqspi, QSPI_FLAG_SM, SET, tickstart,
								Timeout);
...
HAL_StatusTypeDef HAL_QSPI_Abort(QSPI_HandleTypeDef *hqspi)
{
	...
	if (((uint32_t)hqspi->State & 0x2U) != 0U) {
		...
		if (__HAL_QSPI_GET_FLAG(hqspi, QSPI_FLAG_BUSY) != RESET) {
			...
			/* Wait until TC flag is set to go back in idle state */
			status = QSPI_WaitFlagStateUntilTimeout(hqspi, QSPI_FLAG_TC, SET, tickstart,
								hqspi->Timeout);
			...
			if (status == HAL_OK) {
				...
				status = QSPI_WaitFlagStateUntilTimeout(
					hqspi, QSPI_FLAG_BUSY, RESET, tickstart, hqspi->Timeout);
			}

			if (status == HAL_OK) {
				/* Reset functional mode configuration to indirect write mode by
				 * default */
				CLEAR_BIT(hqspi->Instance->CCR, QUADSPI_CCR_FMODE);
...
HAL_StatusTypeDef HAL_QSPI_Abort_IT(QSPI_HandleTypeDef *hqspi)
{
	...
	if (((uint32_t)hqspi->State & 0x2U) != 0U) {
		...
		__HAL_QSPI_DISABLE_IT(
			hqspi, (QSPI_IT_TO | QSPI_IT_SM | QSPI_IT_FT | QSPI_IT_TC | QSPI_IT_TE));
	...
	if (cmd->InstructionMode != QSPI_INSTRUCTION_NONE) {
		...
		if (cmd->AlternateByteMode != QSPI_ALTERNATE_BYTES_NONE) {
			...
			if (cmd->AddressMode != QSPI_ADDRESS_NONE) {
				...
				if (cmd->DataMode != QSPI_DATA_NONE) {
					/* Configure QSPI: CCR register with all communications
					 * parameters */
					WRITE_REG(hqspi->Instance->CCR,
