 ...
 */
	...
	#define ADC_CFGR_FIELDS_1                                                                  \
		((uint32_t)(ADC_CFGR_RES | ADC_CFGR_CONT | ADC_CFGR_OVRMOD | ADC_CFGR_DISCEN |     \
			    ADC_CFGR_DISCNUM | ADC_CFGR_EXTEN |                                    \
			    ADC_CFGR_EXTSEL)) /*!< ADC_CFGR fields of parameters that can be       \
						 updated when no regular conversion is on-going */

	#define ADC_CFGR2_FIELDS                                                                   \
		((uint32_t)(ADC_CFGR2_ROVSE | ADC_CFGR2_OSR | ADC_CFGR2_OVSS | ADC_CFGR2_TROVS |   \
			    ADC_CFGR2_ROVSM)) /*!< ADC_CFGR2 fields of parameters that can be      \
						 updated when no conversion (neither regular nor   \
	...
	#define ADC_CONVERSION_TIME_MAX_CPU_CYCLES                                                 \
		((uint32_t)210688) /*!< ADC conversion completion time-out value */
	...
	if (((hadc->State & HAL_ADC_STATE_ERROR_INTERNAL) == 0UL) &&
		...
		if (LL_ADC_IsEnabled(hadc->Instance) == 0UL) {
			if (__LL_ADC_IS_ENABLED_ALL_COMMON_INSTANCE(
				...
				/*                                                                      */
				/*   - ADC clock mode and ACC prescaler (CKMODE and PRESC bits)are
				 * set  */
				/*     according to adc->Init.ClockPrescaler. It selects the clock
				 */
				/*    source and sets the clock division factor. */
				/*                                                                      */
				/* Some parameters of this register are not reset, since they are
				 * set   */
		...
		if ((tmp_adc_is_conversion_on_going_regular == 0UL) &&
			...
			if (hadc->Init.OversamplingMode == ENABLE) {
				...
				if ((hadc->Init.ExternalTrigConv == ADC_SOFTWARE_START) ||
					...
					/* Multi trigger is not applicable to software-triggered
					 * conversions */
					assert_param((hadc->Init.Oversampling.TriggeredMode ==
				...
				MODIFY_REG(hadc->Instance->CFGR2, ADC_CFGR2_FIELDS,
					   ADC_CFGR2_ROVSE |
						   ((hadc->Init.Oversampling.Ratio - 1UL)
						    << ADC_CFGR2_OSR_Pos) |
						   hadc->Init.Oversampling.RightBitShift |
						   hadc->Init.Oversampling.TriggeredMode |
						   hadc->Init.Oversampling.OversamplingStopReset);
			...
			/* Set the LeftShift parameter: it is applied to the final result with or
...
HAL_StatusTypeDef HAL_ADC_DeInit(ADC_HandleTypeDef *hadc)
{
	...
	/* ========== Reset ADC registers ========== */
	/* Reset register IER */
	__HAL_ADC_DISABLE_IT(hadc, (ADC_IT_AWD3 | ADC_IT_AWD2 | ADC_IT_AWD1 | ADC_IT_JQOVF |
				    ADC_IT_OVR | ADC_IT_JEOS | ADC_IT_JEOC | ADC_IT_EOS |
				    ADC_IT_EOC | ADC_IT_EOSMP | ADC_IT_RDY));

	/* Reset register ISR */
	__HAL_ADC_CLEAR_FLAG(hadc, (ADC_FLAG_AWD3 | ADC_FLAG_AWD2 | ADC_FLAG_AWD1 | ADC_FLAG_JQOVF |
	...
	CLEAR_BIT(hadc->Instance->CFGR,
		  ADC_CFGR_AWD1CH | ADC_CFGR_JAUTO | ADC_CFGR_JAWD1EN | ADC_CFGR_AWD1EN |
			  ...
			  ADC_CFGR_EXTEN | ADC_CFGR_EXTSEL | ADC_CFGR_RES | ADC_CFGR_DMNGT);
	SET_BIT(hadc->Instance->CFGR, ADC_CFGR_JQDIS);

	/* Reset register CFGR2 */
	CLEAR_BIT(hadc->Instance->CFGR2, ADC_CFGR2_ROVSM | ADC_CFGR2_TROVS | ADC_CFGR2_OVSS |
						 ADC_CFGR2_OVSR | ADC_CFGR2_JOVSE |
						 ADC_CFGR2_ROVSE);
	...
	CLEAR_BIT(hadc->Instance->SMPR2, ADC_SMPR2_SMP18 | ADC_SMPR2_SMP17 | ADC_SMPR2_SMP16 |
						 ...
						 ADC_SMPR2_SMP13 | ADC_SMPR2_SMP12 |
		...
		if (tmp_hal_status == HAL_OK) {
			...
			if ((__LL_ADC_MULTI_INSTANCE_MASTER(hadc->Instance) == hadc->Instance) ||
				...
				/* if Master ADC JAUTO bit is set, update Slave State in setting
				   HAL_ADC_STATE_INJ_BUSY bit and in resetting HAL_ADC_STATE_INJ_EOC
				   bit */
...
HAL_StatusTypeDef HAL_ADC_PollForConversion(ADC_HandleTypeDef *hadc, uint32_t Timeout)
{
	...
	else /* ADC_EOC_SINGLE_CONV */
	{
		...
		if ((tmp_multimode_config == LL_ADC_MULTI_INDEPENDENT) ||
			...
			if (LL_ADC_GetMultiDMATransfer(__LL_ADC_COMMON_INSTANCE(hadc->Instance)) !=
	...
	default: /* Case ADC_OVR_EVENT */
		...
		if (hadc->Init.Overrun == ADC_OVR_DATA_PRESERVED) {
			...
			/* Clear ADC Overrun flag only if Overrun is set to ADC_OVR_DATA_OVERWRITTEN
			   otherwise, data register is potentially overwritten by new converted data
			   as soon as OVR is cleared. */
			...
			if ((__LL_ADC_MULTI_INSTANCE_MASTER(hadc->Instance) == hadc->Instance) ||
				...
				if (READ_BIT(hadc->Instance->CFGR, ADC_CFGR_JAUTO) != 0UL) {
					...
					/* Enable as well injected interruptions in case
					 HAL_ADCEx_InjectedStart_IT() has not been called
					 beforehand. This allows to start regular and injected
					 conversions when JAUTO is set with a single call to
					 HAL_ADC_Start_IT() */
					switch (hadc->Init.EOCSelection) {
					case ADC_EOC_SEQ_CONV:
				...
				tmpADC_Master = __LL_ADC_MULTI_INSTANCE_MASTER(hadc->Instance);
				if (READ_BIT(tmpADC_Master->CFGR, ADC_CFGR_JAUTO) != 0UL) {
					/* First, update Slave State in setting
					   HAL_ADC_STATE_INJ_BUSY bit and in resetting
					   HAL_ADC_STATE_INJ_EOC bit */
					ADC_STATE_CLR_SET(hadc->State, HAL_ADC_STATE_INJ_EOC,
							  HAL_ADC_STATE_INJ_BUSY);
...
HAL_StatusTypeDef HAL_ADC_Start_DMA(ADC_HandleTypeDef *hadc, uint32_t *pData, uint32_t Length)
{
	...
	if (LL_ADC_REG_IsConversionOngoing(hadc->Instance) == 0UL) {
	...
	#endif
		{
			...
			if (tmp_hal_status == HAL_OK) {
				/* Set ADC state */
				/* - Clear state bitfield related to regular group conversion
				 * results   */
				/* - Set state bitfield related to regular operation */
				ADC_STATE_CLR_SET(hadc->State,
						  HAL_ADC_STATE_READY | HAL_ADC_STATE_REG_EOC |
	...
	#if defined(ADC_MULTIMODE_SUPPORT)
				/* Reset HAL_ADC_STATE_MULTIMODE_SLAVE bit
				  - if ADC instance is master or if multimode feature is not
				  available
				  - if multimode setting is disabled (ADC instance slave in
				  independent mode) */
				...
				/* Check if a conversion is on going on ADC group injected */
				if ((hadc->State & HAL_ADC_STATE_INJ_BUSY) != 0UL) {
					/* Reset ADC error code fields related to regular
					 * conversions only */
			...
			if (READ_BIT(tmp_cfgr, ADC_CFGR_CONT) != ADC_CFGR_CONT) {
				...
				if (__HAL_ADC_GET_FLAG(hadc, ADC_FLAG_EOS)) {
					...
					if (LL_ADC_REG_IsConversionOngoing(hadc->Instance) == 0UL) {
						 ...
						 */
						/* Note: Overrun interrupt was enabled with EOC
						 * interrupt in      */
						/* HAL_Start_IT(), but is not disabled here because
						 * can be used   */
						...
						/* Set ADC error code to ADC peripheral internal
						 * error */
						SET_BIT(hadc->ErrorCode, HAL_ADC_ERROR_INTERNAL);
					}
				}
		...
		if (tmp_adc_inj_is_trigger_source_sw_start != 0UL) {
			if ((READ_BIT(tmp_cfgr, ADC_CFGR_JAUTO) == 0UL) ||
				...
				if (__HAL_ADC_GET_FLAG(hadc, ADC_FLAG_JEOS)) {
					...
					/* when the last context has been fully processed, JSQR is
					 * reset      */
					/* by the hardware. Even if no injected conversion is
					 * planned to come */
					/* (queue empty, triggers are ignored), it can start again
					 */
					/* immediately after setting a new context (JADSTART is
					 * still set).   */
					/* Therefore, state of HAL ADC injected group is kept to
					 * busy.        */
					if (READ_BIT(tmp_cfgr, ADC_CFGR_JQM) == 0UL) {
						/* Allowed to modify bits ADC_IT_JEOC/ADC_IT_JEOS
						 * only if bit       */
						/* JADSTART==0 (no conversion on going) */
						if (LL_ADC_INJ_IsConversionOngoing(
							    hadc->Instance) == 0UL) {
							/* Disable ADC end of sequence conversion
							 * interrupt  */
							__HAL_ADC_DISABLE_IT(
								hadc, ADC_IT_JEOC | ADC_IT_JEOS);

							/* Set ADC state */
							CLEAR_BIT(hadc->State,
								  HAL_ADC_STATE_INJ_BUSY);

							if ((hadc->State &
							     HAL_ADC_STATE_REG_BUSY) == 0UL) {
								SET_BIT(hadc->State,
									HAL_ADC_STATE_READY);
							}
						} else {
							...
							/* Set ADC error code to ADC peripheral
							 * internal error */
							SET_BIT(hadc->ErrorCode,
...
HAL_StatusTypeDef HAL_ADC_ConfigChannel(ADC_HandleTypeDef *hadc, ADC_ChannelConfTypeDef *pConfig)
{
	...
	if (hadc->Init.OversamplingMode == ENABLE) {
		assert_param(IS_ADC_RANGE(ADC_GET_RESOLUTION(hadc),
					  pConfig->Offset / (hadc->Init.Oversampling.Ratio + 1U)));
	} else {
	...
	if (LL_ADC_REG_IsConversionOngoing(hadc->Instance) == 0UL) {
		...
		hadc->Instance->PCSEL |=
			(1UL << (__LL_ADC_CHANNEL_TO_DECIMAL_NB((uint32_t)pConfig->Channel) &
				 0x1FUL));
		...
		if ((tmp_adc_is_conversion_on_going_regular == 0UL) &&
			...
			if (pConfig->OffsetNumber != ADC_OFFSET_NONE) {
				...
				LL_ADC_SetOffsetSignedSaturation(
					hadc->Instance, pConfig->OffsetNumber,
				...
				LL_ADC_SetDataRightShift(hadc->Instance, pConfig->OffsetNumber,
							 (pConfig->OffsetRightShift == ENABLE)
								 ? LL_ADC_OFFSET_RSHIFT_ENABLE
								 : LL_ADC_OFFSET_RSHIFT_DISABLE);

			} else {
				/* Scan OFR1, OFR2, OFR3, OFR4 to check if the selected channel is
				  enabled. If this is the case, offset OFRx is disabled since
				  pConfig->OffsetNumber = ADC_OFFSET_NONE. */
				if (((hadc->Instance->OFR1) & ADC_OFR1_OFFSET1_CH) ==
				    ADC_OFR_CHANNEL(pConfig->Channel)) {
		...
		if (LL_ADC_IsEnabled(hadc->Instance) == 0UL) {
			...
			if (__LL_ADC_IS_CHANNEL_INTERNAL(pConfig->Channel)) {
				...
				/* Software is allowed to change common parameters only when all
				 * ADCs   */
				/* of the common group are disabled. */
				if (__LL_ADC_IS_ENABLED_ALL_COMMON_INSTANCE(
					    __LL_ADC_COMMON_INSTANCE(hadc->Instance)) == 0UL) {
					/* If the requested internal measurement path has already
					 * been enabled, */
					...
					if ((pConfig->Channel == ADC_CHANNEL_TEMPSENSOR) &&
						...
						if (ADC_TEMPERATURE_SENSOR_INSTANCE(hadc)) {
							 ...
							 * partially              */
							/*       CPU processing cycles, scaling in
							 * us split to not          */
							/*       exceed 32 bits register capacity
							 * and handle low frequency. */
							wait_loop_index =
						...
						if (ADC_VREFINT_INSTANCE(hadc)) {
						   ...
						   ((tmp_config_internal_channel &
						     LL_ADC_PATH_INTERNAL_VDDCORE) == 0UL)) {
					  ...
					  ADC_AnalogWDGConfTypeDef *pAnalogWDGConfig)
{
	...
	if ((tmp_adc_is_conversion_on_going_regular == 0UL) &&
		...
		if (pAnalogWDGConfig->WatchdogNumber == ADC_ANALOGWATCHDOG_1) {
			...
			MODIFY_REG(hadc->Instance->LTR1, ADC_LTR1_LT1,
				   tmp_awd_low_threshold_shifted);
			MODIFY_REG(hadc->Instance->HTR1, ADC_HTR1_HT1,
		...
		/* Case of ADC_ANALOGWATCHDOG_2 or ADC_ANALOGWATCHDOG_3 */
			...
			if (pAnalogWDGConfig->WatchdogNumber == ADC_ANALOGWATCHDOG_2) {
				/* Set ADC analog watchdog thresholds value of both thresholds high
				 * and low */
				MODIFY_REG(hadc->Instance->LTR2, ADC_LTR2_LT2,
					   tmp_awd_low_threshold_shifted);
				...
				/* Set ADC analog watchdog thresholds value of both thresholds high
				 * and low */
				MODIFY_REG(hadc->Instance->LTR3, ADC_LTR3_LT3,
					   tmp_awd_low_threshold_shifted);
				MODIFY_REG(hadc->Instance->HTR3, ADC_HTR3_HT3,
					   tmp_awd_high_threshold_shifted);
			}
	...
	if (LL_ADC_IsEnabled(hadc->Instance) == 0UL) {
		...
		if ((__LL_ADC_MULTI_INSTANCE_MASTER(hadc->Instance) == hadc->Instance) ||
			...
			while (__HAL_ADC_GET_FLAG(hadc, ADC_FLAG_RDY) == 0UL) {
				/*  If ADEN bit is set less than 4 ADC clock cycles after the ADCAL
				   bit has been cleared (after a calibration), ADEN bit is reset by
				   the calibration logic. The workaround is to continue setting ADEN
				   until ADRDY is becomes 1. Additionally, ADC_ENABLE_TIMEOUT is
...
HAL_StatusTypeDef ADC_Disable(ADC_HandleTypeDef *hadc)
{
	...
	if ((LL_ADC_IsEnabled(hadc->Instance) != 0UL) && (tmp_adc_is_disable_on_going == 0UL)) {
		...
		if ((hadc->Instance->CR & (ADC_CR_JADSTART | ADC_CR_ADSTART | ADC_CR_ADEN)) ==
		    ADC_CR_ADEN) {
			/* Disable the ADC peripheral */
