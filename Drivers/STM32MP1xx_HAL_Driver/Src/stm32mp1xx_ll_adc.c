		...
		#define IS_LL_ADC_INJ_SEQ_SCAN_DISCONT_MODE(__INJ_SEQ_DISCONT_MODE__)              \
			...
			#define IS_LL_ADC_MULTI_TWOSMP_DELAY(__MULTI_TWOSMP_DELAY__)               \
				(((__MULTI_TWOSMP_DELAY__) ==                                      \
				 ...
				 ((__MULTI_TWOSMP_DELAY__) ==                                      \
				  LL_ADC_MULTI_TWOSMP_DELAY_7CYCLES_5_14_BITS) ||                  \
				 ((__MULTI_TWOSMP_DELAY__) ==                                      \
				  LL_ADC_MULTI_TWOSMP_DELAY_8CYCLES_5))
			      ...
			      LL_ADC_CommonInitTypeDef *pADC_CommonInitStruct)
{
	...
	if (pADC_CommonInitStruct->Multimode != LL_ADC_MULTI_INDEPENDENT) {
		...
		assert_param(
			IS_LL_ADC_MULTI_TWOSMP_DELAY(pADC_CommonInitStruct->MultiTwoSamplingDelay));
	}
		#endif /* ADC_MULTIMODE_SUPPORT */
	...
	if (__LL_ADC_IS_ENABLED_ALL_COMMON_INSTANCE(ADCxy_COMMON) == 0UL) {
		...
		if (pADC_CommonInitStruct->Multimode != LL_ADC_MULTI_INDEPENDENT) {
			MODIFY_REG(ADCxy_COMMON->CCR,
				   ADC_CCR_CKMODE | ADC_CCR_PRESC | ADC_CCR_DUAL | ADC_CCR_DAMDF |
					   ADC_CCR_DELAY,
...
void LL_ADC_CommonStructInit(LL_ADC_CommonInitTypeDef *pADC_CommonInitStruct)
{
	...
	pADC_CommonInitStruct->MultiTwoSamplingDelay = LL_ADC_MULTI_TWOSMP_DELAY_1CYCLE_5;
		#endif /* ADC_MULTIMODE_SUPPORT */
}
...
ErrorStatus LL_ADC_DeInit(ADC_TypeDef *ADCx)
{
	...
	if (READ_BIT(ADCx->CR, (ADC_CR_JADSTP | ADC_CR_ADSTP | ADC_CR_JADSTART | ADC_CR_ADSTART |
		...
		CLEAR_BIT(ADCx->CFGR2, (ADC_CFGR2_LSHIFT | ADC_CFGR2_OVSR | ADC_CFGR2_RSHIFT1 |
		...
		CLEAR_BIT(ADCx->SMPR2,
			  (ADC_SMPR2_SMP19 | ADC_SMPR2_SMP18 | ADC_SMPR2_SMP17 | ADC_SMPR2_SMP16 |
			   ADC_SMPR2_SMP15 | ADC_SMPR2_SMP14 | ADC_SMPR2_SMP13 | ADC_SMPR2_SMP12 |
		...
		/* Reset register SQR2 */
		CLEAR_BIT(ADCx->SQR2, (ADC_SQR2_SQ9 | ADC_SQR2_SQ8 | ADC_SQR2_SQ7 | ADC_SQR2_SQ6 |
				       ADC_SQR2_SQ5));
		...
		/* Reset register JSQR */
		CLEAR_BIT(ADCx->JSQR,
			  (ADC_JSQR_JL | ADC_JSQR_JEXTSEL | ADC_JSQR_JEXTEN | ADC_JSQR_JSQ4 |
...
ErrorStatus LL_ADC_Init(ADC_TypeDef *ADCx, LL_ADC_InitTypeDef *pADC_InitStruct)
{
	...
	if (LL_ADC_IsEnabled(ADCx) == 0UL) {
		...
		MODIFY_REG(ADCx->CFGR, ADC_CFGR_RES | ADC_CFGR_AUTDLY,
...
void LL_ADC_StructInit(LL_ADC_InitTypeDef *pADC_InitStruct)
{
	...
	pADC_InitStruct->Resolution = LL_ADC_RESOLUTION_16B;
...
ErrorStatus LL_ADC_REG_Init(ADC_TypeDef *ADCx, LL_ADC_REG_InitTypeDef *pADC_REG_InitStruct)
{
	...
	if (pADC_REG_InitStruct->SequencerLength != LL_ADC_REG_SEQ_SCAN_DISABLE) {
		assert_param(
			IS_LL_ADC_REG_SEQ_SCAN_DISCONT_MODE(pADC_REG_InitStruct->SequencerDiscont));
		...
		if (pADC_REG_InitStruct->SequencerLength != LL_ADC_REG_SEQ_SCAN_DISABLE) {
			MODIFY_REG(ADCx->CFGR,
				   ADC_CFGR_EXTSEL | ADC_CFGR_EXTEN | ADC_CFGR_DISCEN |
					   ADC_CFGR_DISCNUM | ADC_CFGR_CONT | ADC_CFGR_DMNGT |
					   ADC_CFGR_OVRMOD,
...
ErrorStatus LL_ADC_INJ_Init(ADC_TypeDef *ADCx, LL_ADC_INJ_InitTypeDef *pADC_INJ_InitStruct)
{
	...
	if (pADC_INJ_InitStruct->SequencerLength != LL_ADC_INJ_SEQ_SCAN_DISABLE) {
		assert_param(
			IS_LL_ADC_INJ_SEQ_SCAN_DISCONT_MODE(pADC_INJ_InitStruct->SequencerDiscont));
	}
	...
	if (LL_ADC_IsEnabled(ADCx) == 0UL) {
		...
		if (pADC_INJ_InitStruct->SequencerLength != LL_ADC_REG_SEQ_SCAN_DISABLE) {
			...
			MODIFY_REG(ADCx->CFGR, ADC_CFGR_JDISCEN | ADC_CFGR_JAUTO,
				   LL_ADC_REG_SEQ_DISCONT_DISABLE | pADC_INJ_InitStruct->TrigAuto);
		}
