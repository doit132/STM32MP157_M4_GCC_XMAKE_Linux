...
HAL_StatusTypeDef HAL_LPTIM_Init(LPTIM_HandleTypeDef *hlptim)
{
	...
	if ((hlptim->Instance == LPTIM1) || (hlptim->Instance == LPTIM2)) {
		...
		if (hlptim->Instance == LPTIM3) {
			...
			assert_param(IS_LPTIM_INPUT1_SOURCE(hlptim->Instance,
							    hlptim->Init.Input1Source));
...
HAL_StatusTypeDef HAL_LPTIM_Counter_Start(LPTIM_HandleTypeDef *hlptim, uint32_t Period)
{
	...
	/* If clock source is not ULPTIM clock and counter source is external, then it must not be
	 * prescaled */
	if ((hlptim->Init.Clock.Source != LPTIM_CLOCKSOURCE_ULPTIM) &&
	    (hlptim->Init.CounterSource == LPTIM_COUNTERSOURCE_EXTERNAL)) {
 ...
 *          @arg @ref HAL_LPTIM_AUTORELOAD_WRITE_CB_ID Auto-reload register write complete Callback
 * ID
...
void LPTIM_Disable(LPTIM_HandleTypeDef *hlptim)
{
	...
	case LPTIM1_BASE:
		...
		break;
	case LPTIM2_BASE:
		tmpclksource = __HAL_RCC_GET_LPTIM23_SOURCE();
		break;
	case LPTIM3_BASE:
		tmpclksource = __HAL_RCC_GET_LPTIM23_SOURCE();
		break;
	case LPTIM4_BASE:
		tmpclksource = __HAL_RCC_GET_LPTIM45_SOURCE();
		break;
	case LPTIM5_BASE:
		tmpclksource = __HAL_RCC_GET_LPTIM45_SOURCE();
		break;
	...
	if ((tmpCMP != 0UL) || (tmpARR != 0UL)) {
		...
		case LPTIM1_BASE:
			...
			break;
		case LPTIM2_BASE:
			__HAL_RCC_LPTIM23_CONFIG(RCC_LPTIM23CLKSOURCE_PCLK3);
			break;
		case LPTIM3_BASE:
			__HAL_RCC_LPTIM23_CONFIG(RCC_LPTIM23CLKSOURCE_PCLK3);
			break;
		case LPTIM4_BASE:
			__HAL_RCC_LPTIM45_CONFIG(RCC_LPTIM45CLKSOURCE_PCLK3);
			break;
		case LPTIM5_BASE:
			__HAL_RCC_LPTIM45_CONFIG(RCC_LPTIM45CLKSOURCE_PCLK3);
			break;
		...
		case LPTIM1_BASE:
			...
			break;
		case LPTIM2_BASE:
			__HAL_RCC_LPTIM23_CONFIG(tmpclksource);
			break;
		case LPTIM3_BASE:
			__HAL_RCC_LPTIM23_CONFIG(tmpclksource);
			break;
		case LPTIM4_BASE:
			__HAL_RCC_LPTIM45_CONFIG(tmpclksource);
		case LPTIM5_BASE:
			__HAL_RCC_LPTIM45_CONFIG(tmpclksource);
			break;
		default:
