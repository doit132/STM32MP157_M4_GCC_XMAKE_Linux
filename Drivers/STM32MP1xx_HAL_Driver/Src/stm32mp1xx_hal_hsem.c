...
HAL_StatusTypeDef HAL_HSEM_Take(uint32_t SemID, uint32_t ProcessID)
{
	...
	HSEM->R[SemID] =
		((ProcessID & HSEM_R_PROCID) |
		 ((HAL_GetCurrentCPUID() << POSITION_VAL(HSEM_R_MASTERID)) & HSEM_R_MASTERID) |
		 HSEM_R_LOCK);

	/* second step : read the R register . Take achieved if MasterID and processID match and
	 * take bit set to 1 */
	if (HSEM->R[SemID] ==
	    ((ProcessID & HSEM_R_PROCID) |
	     ((HAL_GetCurrentCPUID() << POSITION_VAL(HSEM_R_MASTERID)) & HSEM_R_MASTERID) |
...
HAL_StatusTypeDef HAL_HSEM_FastTake(uint32_t SemID)
{
	...
	if (HSEM->RLR[SemID] ==
	    (((HAL_GetCurrentCPUID() << POSITION_VAL(HSEM_R_MASTERID)) & HSEM_RLR_MASTERID) |
	     HSEM_RLR_LOCK)) {
...
void HAL_HSEM_Release(uint32_t SemID, uint32_t ProcessID)
{
	...
	/* Clear the semaphore by writing to the R register : the MasterID , the processID and take
	 * bit = 0  */
