  ...
  ==============================================================================
    ...
    [..] The SDMMC features include the following:
	 ...
	 (+) Full compliance with SDIO card specification version 4.0. Card support
	     ...
	     (SDR104 SDMMC_CK speed limited to maximum allowed IO speed, SPI mode and
	      UHS-II mode not supported).
	 (+) Data transfer up to 208 Mbyte/s for the 8 bit mode. (depending maximum allowed IO
  speed).
...
HAL_StatusTypeDef SDMMC_ConfigData(SDMMC_TypeDef *SDMMCx, SDMMC_DataInitTypeDef *Data)
{
	...
	tmpreg |= (uint32_t)(Data->DataBlockSize | Data->TransferDir | Data->TransferMode |
			     Data->DPSM);
...
static uint32_t SDMMC_GetCmdResp1(SDMMC_TypeDef *SDMMCx, uint8_t SD_CMD, uint32_t Timeout)
{
	...
	} while (!__SDMMC_GET_FLAG(SDMMCx, SDMMC_FLAG_CCRCFAIL | SDMMC_FLAG_CMDREND |
