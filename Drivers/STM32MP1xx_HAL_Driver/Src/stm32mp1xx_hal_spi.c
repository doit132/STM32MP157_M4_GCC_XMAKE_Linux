...
static HAL_StatusTypeDef SPI_WaitOnFlagUntilTimeout(SPI_HandleTypeDef *hspi, uint32_t Flag,
						    FlagStatus FlagStatus, uint32_t Timeout,
						    uint32_t Tickstart);
...
HAL_StatusTypeDef HAL_SPI_Init(SPI_HandleTypeDef *hspi)
{
	...
	if (hspi->Init.CRCCalculation == SPI_CRCCALCULATION_ENABLE) {
		...
		assert_param(
			IS_SPI_CRC_INITIALIZATION_PATTERN(hspi->Init.TxCRCInitializationPattern));
		assert_param(
			IS_SPI_CRC_INITIALIZATION_PATTERN(hspi->Init.RxCRCInitializationPattern));
	}
	#else
	...
	if (hspi->Init.CRCCalculation == SPI_CRCCALCULATION_ENABLE) {
		...
		if (hspi->Init.CRCLength == SPI_CRC_LENGTH_DATASIZE) {
			crc_length = (hspi->Init.DataSize >> SPI_CFG1_DSIZE_Pos)
		...
		if ((hspi->Init.DataSize >> SPI_CFG1_DSIZE_Pos) >
		    (crc_length >> SPI_CFG1_CRCSIZE_Pos)) {
			return HAL_ERROR;
		}
	} else {
	...
	WRITE_REG(hspi->Instance->CFG1,
		  (hspi->Init.BaudRatePrescaler | hspi->Init.CRCCalculation | crc_length |
	...
	if (hspi->Init.CRCCalculation == SPI_CRCCALCULATION_ENABLE) {
		...
		if (hspi->Init.TxCRCInitializationPattern ==
		...
		if (hspi->Init.RxCRCInitializationPattern ==
		    SPI_CRC_INITIALIZATION_ALL_ONE_PATTERN) {
		...
		if (((!IS_SPI_HIGHEND_INSTANCE(hspi->Instance)) &&
		    ...
		    ((IS_SPI_HIGHEND_INSTANCE(hspi->Instance)) &&
		     (crc_length == SPI_CRC_LENGTH_32BIT))) {
					     ...
					     HAL_SPI_CallbackIDTypeDef CallbackID)
{
	...
	if (HAL_SPI_STATE_READY == hspi->State) {
		...
		case HAL_SPI_SUSPEND_CB_ID:
			hspi->SuspendCallback =
				HAL_SPI_SuspendCallback; /* Legacy weak SuspendCallback      */
			break;
				   ...
				   uint32_t Timeout)
{
	...
	#else
					*((__IO uint16_t *)&hspi->Instance->TXDR) =
						*((const uint16_t *)hspi->pTxBuffPtr);
	#endif /* __GNUC__ */
				  ...
				  uint32_t Timeout)
{
		...
		while (hspi->RxXferCount > 0UL) {
			...
			if (__HAL_SPI_GET_FLAG(hspi, SPI_FLAG_RXP)) {
				...
				*((uint16_t *)hspi->pRxBuffPtr) =
					*((__IO uint16_t *)&hspi->Instance->RXDR);
	#endif /* __GNUC__ */
	...
	if (hspi->Init.CRCCalculation == SPI_CRCCALCULATION_ENABLE) {
		...
		if (SPI_WaitOnFlagUntilTimeout(hspi, SPI_FLAG_EOT, RESET, tickstart, Timeout) !=
		    HAL_OK) {
			SET_BIT(hspi->ErrorCode, HAL_SPI_ERROR_FLAG);
		}
	}
					     ...
					     uint8_t *pRxData, uint16_t Size)
{
	...
	__HAL_SPI_ENABLE_IT(hspi, (SPI_IT_EOT | SPI_IT_DXP | SPI_IT_UDR | SPI_IT_OVR | SPI_IT_FRE |
				   SPI_IT_MODF | SPI_IT_TSERF));
					     ...
					     uint16_t Size)
{
	...
	if (hspi->State == HAL_SPI_STATE_BUSY_TX) {
		...
		if (((hspi->Instance->CR2 & SPI_CR2_TSER) != 0UL) &&
		    (tmp_state == HAL_SPI_STATE_READY)) {
						    ...
						    uint8_t *pRxData, uint16_t Size)
{
	...
	if (hspi->State == HAL_SPI_STATE_BUSY_TX_RX) {
		...
		/* Insert the new number of data that will be sent and received just after the
		 * current one */
		MODIFY_REG(hspi->Instance->CR2, SPI_CR2_TSER, (Size & 0xFFFFFFFFUL) << 16UL);
...
HAL_StatusTypeDef HAL_SPI_Receive_DMA(SPI_HandleTypeDef *hspi, uint8_t *pData, uint16_t Size)
{
	...
	if (HAL_OK != HAL_DMA_Start_IT(hspi->hdmarx, (uint32_t)&hspi->Instance->RXDR,
...
HAL_StatusTypeDef HAL_SPI_Abort(SPI_HandleTypeDef *hspi)
{
	...
	/* If master communication on going, make sure current frame is done before closing the
	 * connection */
...
HAL_StatusTypeDef HAL_SPI_Abort_IT(SPI_HandleTypeDef *hspi)
{
	...
	/* If DMA Tx and/or DMA Rx Handles are associated to SPI Handle, DMA Abort complete
	   callbacks should be initialized before any call to DMA Abort functions */
	...
	if ((trigger & (SPI_FLAG_MODF | SPI_FLAG_OVR | SPI_FLAG_FRE | SPI_FLAG_UDR)) != 0UL) {
		...
		if (hspi->ErrorCode != HAL_SPI_ERROR_NONE) {
			...
			__HAL_SPI_DISABLE_IT(hspi,
					     (SPI_IT_EOT | SPI_IT_RXP | SPI_IT_TXP | SPI_IT_MODF |
			...
			if (HAL_IS_BIT_SET(cfg1, SPI_CFG1_TXDMAEN | SPI_CFG1_RXDMAEN)) {
				...
				if (hspi->hdmarx != NULL) {
					...
					will lead to call HAL_SPI_ErrorCallback() at end of DMA
					abort procedure */
...
static void SPI_AbortTransfer(SPI_HandleTypeDef *hspi)
{
	...
	/* Disable ITs */
	__HAL_SPI_DISABLE_IT(hspi, (SPI_IT_EOT | SPI_IT_TXP | SPI_IT_RXP | SPI_IT_DXP | SPI_IT_UDR |
