/**
  ...
  ******************************************************************************
  ...
  (@) Sections "ADC peripheral features" and "How to use this driver" are
      available in file of generic functions "stm32mp1xx_hal_adc.c".
  [..]
  @endverbatim
  ******************************************************************************
  */
 ...
 */
	...
	#define ADC_JSQR_FIELDS                                                                    \
		((ADC_JSQR_JL | ADC_JSQR_JEXTSEL | ADC_JSQR_JEXTEN | ADC_JSQR_JSQ1 |               \
		  ...
		  ADC_JSQR_JSQ4)) /*!< ADC_JSQR fields of parameters that can be updated anytime   \
				     once the ADC is enabled */

	/* Fixed timeout value for ADC calibration.                                   */
	/* Values defined to be higher than worst cases: maximum ratio between ADC    */
	/* and CPU clock frequencies.                                                 */
	/* Example of profile low frequency : ADC frequency at 15.625kHz (ADC clock   */
	/* source PLL4_Q 4MHz, ADC clock prescaler 256), CPU frequency 800MHz.        */
	/* Calibration time max = 16384 / fADC (refer to datasheet)                   */
	/*           16384 / 15625 = 1.048s                                           */
	/* At maximum CPU speed (800 MHz), this means                                 */
	/*           = 839000000 CPU cycles                                           */
	#define ADC_CALIBRATION_TIMEOUT (839000000UL) /*!< ADC calibration time-out value */
						       ...
						       uint32_t *LinearCalib_Buffer)
{
	...
	for (cnt = 0UL; cnt < ADC_LINEAR_CALIB_REG_COUNT; cnt++) {
		LL_ADC_SetCalibrationLinearFactor(hadc->Instance, ADC_CR_LINCALRDYW6 >> cnt,
						  LinearCalib_Buffer[cnt]);
	}
	return HAL_OK;
}
...
HAL_StatusTypeDef HAL_ADCEx_MultiModeStop_DMA(ADC_HandleTypeDef *hadc)
{
	...
	if (tmp_hal_status == HAL_OK) {
		...
		if (tmp_hal_status == HAL_OK) {
			...
			/* In case of error, attempt to disable ADC master and slave without status
			 * assert */
...
/**
 * @brief  Return the last ADC Master and Slave regular conversions results when in multimode
 * configuration.
 * @param hadc ADC handle of ADC Master (handle of ADC Slave must not be used)
...
__weak void HAL_ADCEx_InjectedQueueOverflowCallback(ADC_HandleTypeDef *hadc)
{
	...
	/* NOTE : This function should not be modified. When the callback is needed,
		  function HAL_ADCEx_InjectedQueueOverflowCallback must be implemented in the user
	   file.
	*/
}
...
HAL_StatusTypeDef HAL_ADCEx_RegularStop_DMA(ADC_HandleTypeDef *hadc)
{
	...
	#if defined(ADC_MULTIMODE_SUPPORT)
/**
 * @brief  Stop DMA-based multimode ADC conversion, disable ADC DMA transfer, disable ADC peripheral
 * if no injected conversion is on-going.
 * @note   Multimode is kept enabled after this function. Multimode DMA bits
 *         (MDMA and DMACFG bits of common CCR register) are maintained. To disable
 *         multimode (set with HAL_ADCEx_MultiModeConfigChannel()), ADC must be
 *         reinitialized using HAL_ADC_Init() or HAL_ADC_DeInit(), or the user can
...
HAL_StatusTypeDef HAL_ADCEx_RegularMultiModeStop_DMA(ADC_HandleTypeDef *hadc)
{
	...
	if (tmp_hal_status == HAL_OK) {
		...
		if (tmp_hal_status == HAL_OK) {
			...
			if (tmp_hal_status == HAL_OK) {
				/* Both Master and Slave ADC's could be disabled. Update Master
				 * State */
						  ...
						  ADC_InjectionConfTypeDef *pConfigInjected)
{
	...
	if (hadc->Init.OversamplingMode == ENABLE) {
		assert_param(IS_ADC_RANGE(ADC_GET_RESOLUTION(hadc),
					  pConfigInjected->InjectedOffset /
						  (hadc->Init.Oversampling.Ratio + 1U)));
	} else {
		assert_param(
	...
	if ((hadc->Init.ScanConvMode == ADC_SCAN_DISABLE) ||
		...
		tmp_JSQR_ContextQueueBeingBuilt |= ADC_JSQR_RK(pConfigInjected->InjectedChannel,
		...
		/* 3. tmp_JSQR_ContextQueueBeingBuilt is fully built for this
		   HAL_ADCEx_InjectedConfigChannel() call, aggregate the setting to those already
		   built during the previous HAL_ADCEx_InjectedConfigChannel() calls (for the same
		   context of course)  */
		hadc->InjectionConfig.ContextQueue |= tmp_JSQR_ContextQueueBeingBuilt;
	...
	if (LL_ADC_INJ_IsConversionOngoing(hadc->Instance) == 0UL) {
		...
		hadc->Instance->PCSEL |=
			(1UL << (__LL_ADC_CHANNEL_TO_DECIMAL_NB(pConfigInjected->InjectedChannel) &
				 0x1FUL));
	...
	if ((tmp_adc_is_conversion_on_going_regular == 0UL) &&
		...
		if (pConfigInjected->InjecOversamplingMode == ENABLE) {
			assert_param(IS_ADC_OVERSAMPLING_RATIO(
			...
			assert_param(!(READ_BIT(hadc->Instance->CFGR2,
			...
			MODIFY_REG(hadc->Instance->CFGR2,
				   ...
				   ADC_CFGR2_JOVSE |
					   ((pConfigInjected->InjecOversampling.Ratio - 1UL)
					    << ADC_CFGR2_OSR_Pos) |
					   pConfigInjected->InjecOversampling.RightBitShift);
		} else {
		...
		if (pConfigInjected->InjectedOffsetNumber != ADC_OFFSET_NONE) {
			...
			LL_ADC_SetOffset(hadc->Instance, pConfigInjected->InjectedOffsetNumber,
					 pConfigInjected->InjectedChannel, tmpOffsetShifted);
			...
			LL_ADC_SetDataRightShift(
				hadc->Instance, pConfigInjected->InjectedOffsetNumber,
				(pConfigInjected->InjectedOffsetRightShift == (uint32_t)ENABLE)
	...
	if (LL_ADC_IsEnabled(hadc->Instance) == 0UL) {
		...
		LL_ADC_SetChannelSingleDiff(hadc->Instance, pConfigInjected->InjectedChannel,
			...
			if (__LL_ADC_IS_ENABLED_ALL_COMMON_INSTANCE(
				...
				if ((pConfigInjected->InjectedChannel == ADC_CHANNEL_TEMPSENSOR) &&
					...
					if (ADC_TEMPERATURE_SENSOR_INSTANCE(hadc)) {
						 ...
						 * partially              */
						/*       CPU processing cycles, scaling in us split
						 * to not          */
						/*       exceed 32 bits register capacity and handle
						 * low frequency. */
						wait_loop_index =
				...
				} else if ((pConfigInjected->InjectedChannel ==
					    ADC_CHANNEL_VREFINT) &&
					   ((tmp_config_internal_channel &
