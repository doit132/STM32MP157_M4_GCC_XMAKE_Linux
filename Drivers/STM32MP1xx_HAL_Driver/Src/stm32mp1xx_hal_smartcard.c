/**
  ...
  ******************************************************************************
  ...
  [..]
    ...
    (#) Initialize the SMARTCARD low level resources by implementing the HAL_SMARTCARD_MspInit()
  API:
  ...
  function Input).
    ...
    (#) Program the Baud Rate, Parity, Mode(Receiver/Transmitter), clock enabling/disabling and
  accordingly, the clock parameters (parity, phase, last bit), prescaler value, guard time and NACK
  on transmission error enabling or disabling in the hsmartcard handle Init structure.

    (#) If required, program SMARTCARD advanced features (TX/RX pins swap, TimeOut, auto-retry
  counter,...) in the hsmartcard handle AdvancedInit structure.
	...
	(@) The specific SMARTCARD interrupts (Transmission complete interrupt,
	     ...
	     __HAL_SMARTCARD_ENABLE_IT() and __HAL_SMARTCARD_DISABLE_IT() inside the transmit and
  receive process.
     ...
     *** Interrupt mode IO operation ***
     ...
     [..]
       ...
       (+) In case of transfer Error, HAL_SMARTCARD_ErrorCallback() function is executed and user
  can add his own code by customization of function pointer HAL_SMARTCARD_ErrorCallback()
 ...
 */
	 ...
	 * @{
	 */
	#define SMARTCARD_TEACK_REACK_TIMEOUT                                                      \
		1000U /*!< SMARTCARD TX or RX enable acknowledge time-out value  */
...
HAL_StatusTypeDef HAL_SMARTCARD_Init(SMARTCARD_HandleTypeDef *hsmartcard)
{
	...
	/* TEACK and/or REACK to check before moving hsmartcard->gState and hsmartcard->RxState to
	 * Ready */
 ...
 *           @arg @ref HAL_SMARTCARD_ABORT_TRANSMIT_COMPLETE_CB_ID Abort Transmit Complete Callback
 * ID
	...
	if (HAL_SMARTCARD_STATE_READY == hsmartcard->gState) {
		...
		case HAL_SMARTCARD_TX_COMPLETE_CB_ID:
			hsmartcard->TxCpltCallback =
				HAL_SMARTCARD_TxCpltCallback; /* Legacy weak TxCpltCallback */
			break;

		case HAL_SMARTCARD_RX_COMPLETE_CB_ID:
			hsmartcard->RxCpltCallback =
				HAL_SMARTCARD_RxCpltCallback; /* Legacy weak RxCpltCallback */
			break;

		case HAL_SMARTCARD_ERROR_CB_ID:
			hsmartcard->ErrorCallback =
				HAL_SMARTCARD_ErrorCallback; /* Legacy weak ErrorCallback  */
			break;

		case HAL_SMARTCARD_ABORT_COMPLETE_CB_ID:
			hsmartcard->AbortCpltCallback =
				HAL_SMARTCARD_AbortCpltCallback; /* Legacy weak AbortCpltCallback */
			break;

		case HAL_SMARTCARD_ABORT_TRANSMIT_COMPLETE_CB_ID:
			hsmartcard->AbortTransmitCpltCallback =
				HAL_SMARTCARD_AbortTransmitCpltCallback; /* Legacy weak
									    AbortTransmitCpltCallback*/
			break;

		case HAL_SMARTCARD_ABORT_RECEIVE_COMPLETE_CB_ID:
			hsmartcard->AbortReceiveCpltCallback =
				HAL_SMARTCARD_AbortReceiveCpltCallback; /* Legacy weak
		...
		case HAL_SMARTCARD_MSPINIT_CB_ID:
			hsmartcard->MspInitCallback =
				HAL_SMARTCARD_MspInit; /* Legacy weak MspInitCallback  */
			break;

		case HAL_SMARTCARD_MSPDEINIT_CB_ID:
			hsmartcard->MspDeInitCallback =
				HAL_SMARTCARD_MspDeInit; /* Legacy weak MspDeInitCallback */
			break;
...
code is set to allow user to identify error type, and HAL_SMARTCARD_ErrorCallback() user callback is
executed. Transfer is kept ongoing on SMARTCARD side. If user wants to abort it, Abort services
should be called by user.
       (##) Error is considered as Blocking : Transfer could not be completed properly and is
aborted. This concerns Frame Error in Interrupt mode transmission, Overrun Error in Interrupt mode
reception and all errors in DMA mode. Error code is set to allow user to identify error type, and
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SMARTCARD_Transmit(SMARTCARD_HandleTypeDef *hsmartcard, uint8_t *pData,
					 uint16_t Size, uint32_t Timeout)
{
	...
	if (hsmartcard->gState == HAL_SMARTCARD_STATE_READY) {
		...
		while (hsmartcard->TxXferCount > 0U) {
			...
			if (SMARTCARD_WaitOnFlagUntilTimeout(hsmartcard, SMARTCARD_FLAG_TXE, RESET,
		...
		if (SMARTCARD_WaitOnFlagUntilTimeout(
			    hsmartcard, SMARTCARD_TRANSMISSION_COMPLETION_FLAG(hsmartcard), RESET,
			    tickstart, Timeout) != HAL_OK) {
			return HAL_TIMEOUT;
		}
		...
		if ((hsmartcard->Init.Mode == SMARTCARD_MODE_TX) &&
		    (hsmartcard->Init.NACKEnable == SMARTCARD_NACK_ENABLE)) {
			/* In case of TX only mode, if NACK is enabled, receiver block has been
			   enabled for Transmit phase. Disable this receiver block. */
			CLEAR_BIT(hsmartcard->Instance->CR1, USART_CR1_RE);
		}
		if ((hsmartcard->Init.Mode == SMARTCARD_MODE_TX_RX) ||
		    (hsmartcard->Init.NACKEnable == SMARTCARD_NACK_ENABLE)) {
			/* Perform a TX FIFO Flush at end of Tx phase, as all sent bytes are
			 * appearing in Rx Data register */
			__HAL_SMARTCARD_FLUSH_DRREGISTER(hsmartcard);
		}
		SET_BIT(hsmartcard->Instance->CR1, USART_CR1_UE);
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SMARTCARD_Receive(SMARTCARD_HandleTypeDef *hsmartcard, uint8_t *pData,
					uint16_t Size, uint32_t Timeout)
{
	uint32_t tickstart;
	uint8_t *ptmpdata = pData;
	...
	if (hsmartcard->RxState == HAL_SMARTCARD_STATE_READY) {
		...
		while (hsmartcard->RxXferCount > 0U) {
			...
			if (SMARTCARD_WaitOnFlagUntilTimeout(hsmartcard, SMARTCARD_FLAG_RXNE, RESET,
							     tickstart, Timeout) != HAL_OK) {
				return HAL_TIMEOUT;
			}
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SMARTCARD_Transmit_IT(SMARTCARD_HandleTypeDef *hsmartcard, uint8_t *pData,
					    uint16_t Size)
{
	/* Check that a Tx process is not already ongoing */
	if (hsmartcard->gState == HAL_SMARTCARD_STATE_READY) {
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SMARTCARD_Receive_IT(SMARTCARD_HandleTypeDef *hsmartcard, uint8_t *pData,
					   uint16_t Size)
{
	/* Check that a Rx process is not already ongoing */
	if (hsmartcard->RxState == HAL_SMARTCARD_STATE_READY) {
		...
		if ((hsmartcard->FifoMode == SMARTCARD_FIFOMODE_ENABLE) &&
		    (Size >= hsmartcard->NbRxDataToProcess)) {
			...
			/* Enable the SMARTCART Parity Error interrupt and RX FIFO Threshold
			 * interrupt */
			 ...
			 */
			SET_BIT(hsmartcard->Instance->CR1,
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SMARTCARD_Transmit_DMA(SMARTCARD_HandleTypeDef *hsmartcard, uint8_t *pData,
					     uint16_t Size)
{
	/* Check that a Tx process is not already ongoing */
	if (hsmartcard->gState == HAL_SMARTCARD_STATE_READY) {
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SMARTCARD_Receive_DMA(SMARTCARD_HandleTypeDef *hsmartcard, uint8_t *pData,
					    uint16_t Size)
{
	/* Check that a Rx process is not already ongoing */
	if (hsmartcard->RxState == HAL_SMARTCARD_STATE_READY) {
		...
		if (HAL_DMA_Start_IT(hsmartcard->hdmarx, (uint32_t)&hsmartcard->Instance->RDR,
			...
			/* Enable the SMARTCARD Error Interrupt: (Frame error, noise error, overrun
			 * error) */
			SET_BIT(hsmartcard->Instance->CR3, USART_CR3_EIE);
...
HAL_StatusTypeDef HAL_SMARTCARD_Abort(SMARTCARD_HandleTypeDef *hsmartcard)
{
	...
	__HAL_SMARTCARD_CLEAR_FLAG(hsmartcard, SMARTCARD_CLEAR_OREF | SMARTCARD_CLEAR_NEF |
						       SMARTCARD_CLEAR_PEF | SMARTCARD_CLEAR_FEF |
...
{
	/* Disable RTOIE, EOBIE, RXNE, PE, RXFT, TXFT and  ERR (Frame error, noise error, overrun
	 * error) interrupts */
	CLEAR_BIT(hsmartcard->Instance->CR1,
		  (USART_CR1_RXNEIE_RXFNEIE | USART_CR1_PEIE | USART_CR1_RTOIE | USART_CR1_EOBIE));
...
HAL_StatusTypeDef HAL_SMARTCARD_Abort_IT(SMARTCARD_HandleTypeDef *hsmartcard)
{
	...
	if (HAL_IS_BIT_SET(hsmartcard->Instance->CR3, USART_CR3_DMAT)) {
		...
		if (hsmartcard->hdmatx != NULL) {
			/* SMARTCARD Tx DMA Abort callback has already been initialised :
			   will lead to call HAL_SMARTCARD_AbortCpltCallback() at end of DMA abort
			   procedure */
	...
	if (abortcplt == 1U) {
		...
		__HAL_SMARTCARD_CLEAR_FLAG(hsmartcard,
					   SMARTCARD_CLEAR_OREF | SMARTCARD_CLEAR_NEF |
						   SMARTCARD_CLEAR_PEF | SMARTCARD_CLEAR_FEF |
						   SMARTCARD_CLEAR_RTOF | SMARTCARD_CLEAR_EOBF);
...
HAL_StatusTypeDef HAL_SMARTCARD_AbortTransmit_IT(SMARTCARD_HandleTypeDef *hsmartcard)
{
	...
	if (HAL_IS_BIT_SET(hsmartcard->Instance->CR3, USART_CR3_DMAT)) {
		...
		if (hsmartcard->hdmatx != NULL) {
			...
			/* Abort DMA TX */
			if (HAL_DMA_Abort_IT(hsmartcard->hdmatx) != HAL_OK) {
				/* Call Directly hsmartcard->hdmatx->XferAbortCallback function in
				 * case of error */
...
{
	/* Disable RTOIE, EOBIE, RXNE, PE, RXFT and  ERR (Frame error, noise error, overrun error)
	 * interrupts */
	CLEAR_BIT(hsmartcard->Instance->CR1,
		  (USART_CR1_RXNEIE_RXFNEIE | USART_CR1_PEIE | USART_CR1_RTOIE | USART_CR1_EOBIE));
	CLEAR_BIT(hsmartcard->Instance->CR3, (USART_CR3_EIE | USART_CR3_RXFTIE));
	...
	if (HAL_IS_BIT_SET(hsmartcard->Instance->CR3, USART_CR3_DMAR)) {
		...
		if (hsmartcard->hdmarx != NULL) {
			...
			/* Abort DMA RX */
			if (HAL_DMA_Abort_IT(hsmartcard->hdmarx) != HAL_OK) {
				/* Call Directly hsmartcard->hdmarx->XferAbortCallback function in
				 * case of error */
			...
			__HAL_SMARTCARD_CLEAR_FLAG(
				hsmartcard, SMARTCARD_CLEAR_OREF | SMARTCARD_CLEAR_NEF |
						    SMARTCARD_CLEAR_PEF | SMARTCARD_CLEAR_FEF |
						    SMARTCARD_CLEAR_RTOF | SMARTCARD_CLEAR_EOBF);
...
void HAL_SMARTCARD_IRQHandler(SMARTCARD_HandleTypeDef *hsmartcard)
{
	...
	if ((errorflags != 0U) &&
		...
		if (((isrflags & USART_ISR_ORE) != 0U) &&
		    (((cr1its & USART_CR1_RXNEIE_RXFNEIE) != 0U) ||
		     ((cr3its & USART_CR3_RXFTIE) != 0U) || ((cr3its & USART_CR3_EIE) != 0U))) {
		...
		if (hsmartcard->ErrorCode != HAL_SMARTCARD_ERROR_NONE) {
			...
			*/
			...
			if ((HAL_IS_BIT_SET(hsmartcard->Instance->CR3, USART_CR3_DMAR)) ||
			    ((errorcode & (HAL_SMARTCARD_ERROR_RTO | HAL_SMARTCARD_ERROR_ORE)) !=
			     0U)) {
				/* Blocking error : transfer is aborted
				   Set the SMARTCARD state ready to be able to start again the
				   process, Disable Rx Interrupts, and disable Rx DMA request, if
				   ongoing */
				...
				if (HAL_IS_BIT_SET(hsmartcard->Instance->CR3, USART_CR3_DMAR)) {
					...
					if (hsmartcard->hdmarx != NULL) {
						/* Set the SMARTCARD DMA Abort callback :
						   will lead to call HAL_SMARTCARD_ErrorCallback()
						   at end of DMA abort procedure */
						hsmartcard->hdmarx->XferAbortCallback =
							SMARTCARD_DMAAbortOnError;

						/* Abort DMA RX */
						if (HAL_DMA_Abort_IT(hsmartcard->hdmarx) !=
						    HAL_OK) {
							/* Call Directly
							 * hsmartcard->hdmarx->XferAbortCallback
			...
			*/
				...
				/* Blocking error : transfer is aborted
				   Set the SMARTCARD state ready to be able to start again the
				   process, Disable Tx Interrupts, and disable Tx DMA request, if
				   ongoing */
				...
				if (HAL_IS_BIT_SET(hsmartcard->Instance->CR3, USART_CR3_DMAT)) {
					...
					if (hsmartcard->hdmatx != NULL) {
						/* Set the SMARTCARD DMA Abort callback :
						   will lead to call HAL_SMARTCARD_ErrorCallback()
						   at end of DMA abort procedure */
						hsmartcard->hdmatx->XferAbortCallback =
							SMARTCARD_DMAAbortOnError;

						/* Abort DMA TX */
						if (HAL_DMA_Abort_IT(hsmartcard->hdmatx) !=
						    HAL_OK) {
							/* Call Directly
							 * hsmartcard->hdmatx->XferAbortCallback
							 * function in case of error */
	...
	#endif /* USE_HAL_SMARTCARD_REGISTER_CALLBACK */
		/* Clear EOBF interrupt after HAL_SMARTCARD_RxCpltCallback() call for the End of
		   Block information to be available during HAL_SMARTCARD_RxCpltCallback()
		   processing */
	...
	if (__HAL_SMARTCARD_GET_IT(hsmartcard, hsmartcard->AdvancedInit.TxCompletionIndication) !=
	    RESET) {
		if (__HAL_SMARTCARD_GET_IT_SOURCE(
			    hsmartcard, hsmartcard->AdvancedInit.TxCompletionIndication) != RESET) {
...
static HAL_StatusTypeDef SMARTCARD_SetConfig(SMARTCARD_HandleTypeDef *hsmartcard)
{
	...
	static const uint16_t SMARTCARDPrescTable[12] = {1U,  2U,  4U,	6U,  8U,   10U,
	...
	/* Configure
	 * - SMARTCARD Clock Prescaler: set PRESCALER according to hsmartcard->Init.ClockPrescaler
	 * value */
	MODIFY_REG(hsmartcard->Instance->PRESC, USART_PRESC_PRESCALER,
		   hsmartcard->Init.ClockPrescaler);

	/*-------------------------- USART GTPR Configuration ----------------------*/
	tmpreg = (hsmartcard->Init.Prescaler |
		  ((uint32_t)hsmartcard->Init.GuardTime << USART_GTPR_GT_Pos));
	MODIFY_REG(hsmartcard->Instance->GTPR, (uint16_t)(USART_GTPR_GT | USART_GTPR_PSC),
		   (uint16_t)tmpreg);
	...
	case SMARTCARD_CLOCKSOURCE_PLL3Q:
		...
		tmpreg = (uint16_t)(((pll3_clocks.PLL3_Q_Frequency /
	...
	case SMARTCARD_CLOCKSOURCE_PLL4Q:
		...
		tmpreg = (uint16_t)(((pll4_clocks.PLL4_Q_Frequency /
				      SMARTCARDPrescTable[hsmartcard->Init.ClockPrescaler]) +
				     (hsmartcard->Init.BaudRate / 2U)) /
				    hsmartcard->Init.BaudRate);
		break;
	case SMARTCARD_CLOCKSOURCE_HSI:
		tmpreg = (uint16_t)(((HSI_VALUE /
				      SMARTCARDPrescTable[hsmartcard->Init.ClockPrescaler]) +
				     (hsmartcard->Init.BaudRate / 2U)) /
				    hsmartcard->Init.BaudRate);
		break;
	case SMARTCARD_CLOCKSOURCE_CSI:
		tmpreg = (uint16_t)(((CSI_VALUE /
				      SMARTCARDPrescTable[hsmartcard->Init.ClockPrescaler]) +
				     (hsmartcard->Init.BaudRate / 2U)) /
				    hsmartcard->Init.BaudRate);
		break;
	case SMARTCARD_CLOCKSOURCE_LSE:
		tmpreg = (uint16_t)(((uint16_t)(LSE_VALUE /
...
static void SMARTCARD_AdvFeatureConfig(SMARTCARD_HandleTypeDef *hsmartcard)
{
	...
	if (HAL_IS_BIT_SET(hsmartcard->AdvancedInit.AdvFeatureInit,
			   SMARTCARD_ADVFEATURE_TXINVERT_INIT)) {
		assert_param(
			IS_SMARTCARD_ADVFEATURE_TXINV(hsmartcard->AdvancedInit.TxPinLevelInvert));
		MODIFY_REG(hsmartcard->Instance->CR2, USART_CR2_TXINV,
			   hsmartcard->AdvancedInit.TxPinLevelInvert);
	}
	...
	if (HAL_IS_BIT_SET(hsmartcard->AdvancedInit.AdvFeatureInit,
		...
		assert_param(
			IS_SMARTCARD_ADVFEATURE_RXINV(hsmartcard->AdvancedInit.RxPinLevelInvert));
		MODIFY_REG(hsmartcard->Instance->CR2, USART_CR2_RXINV,
			   hsmartcard->AdvancedInit.RxPinLevelInvert);
	}
	...
	if (HAL_IS_BIT_SET(hsmartcard->AdvancedInit.AdvFeatureInit,
		...
		MODIFY_REG(hsmartcard->Instance->CR2, USART_CR2_DATAINV,
			   hsmartcard->AdvancedInit.DataInvert);
	}
	...
	if (HAL_IS_BIT_SET(hsmartcard->AdvancedInit.AdvFeatureInit,
		...
		MODIFY_REG(hsmartcard->Instance->CR2, USART_CR2_SWAP,
			   hsmartcard->AdvancedInit.Swap);
	}
	...
	if (HAL_IS_BIT_SET(hsmartcard->AdvancedInit.AdvFeatureInit,
		...
		MODIFY_REG(hsmartcard->Instance->CR3, USART_CR3_OVRDIS,
			   hsmartcard->AdvancedInit.OverrunDisable);
	}
	...
	if (HAL_IS_BIT_SET(hsmartcard->AdvancedInit.AdvFeatureInit,
		...
		assert_param(IS_SMARTCARD_ADVFEATURE_DMAONRXERROR(
			hsmartcard->AdvancedInit.DMADisableonRxError));
		MODIFY_REG(hsmartcard->Instance->CR3, USART_CR3_DDRE,
			   hsmartcard->AdvancedInit.DMADisableonRxError);
	}
	...
	if (HAL_IS_BIT_SET(hsmartcard->AdvancedInit.AdvFeatureInit,
		...
		MODIFY_REG(hsmartcard->Instance->CR2, USART_CR2_MSBFIRST,
			   hsmartcard->AdvancedInit.MSBFirst);
	}
}
...
/**
 * @brief  End ongoing Tx transfer on SMARTCARD peripheral (following error detection or Transmit
 * completion).
 * @param  hsmartcard Pointer to a SMARTCARD_HandleTypeDef structure that contains
 *                    the configuration information for the specified SMARTCARD module.
 * @retval None
...
static void SMARTCARD_TxISR(SMARTCARD_HandleTypeDef *hsmartcard)
{
	...
	if (hsmartcard->gState == HAL_SMARTCARD_STATE_BUSY_TX) {
		if (hsmartcard->TxXferCount == 0U) {
			...
			/* Enable the SMARTCARD Transmit Complete Interrupt */
			__HAL_SMARTCARD_ENABLE_IT(hsmartcard,
						  hsmartcard->AdvancedInit.TxCompletionIndication);
		} else {
	...
	if (hsmartcard->gState == HAL_SMARTCARD_STATE_BUSY_TX) {
		for (nb_tx_data = hsmartcard->NbTxDataToProcess; nb_tx_data > 0U; nb_tx_data--) {
			if (hsmartcard->TxXferCount == 0U) {
				...
				hsmartcard->Instance->TDR =
					(uint8_t)(*hsmartcard->pTxBuffPtr & 0xFFU);
				hsmartcard->pTxBuffPtr++;
				hsmartcard->TxXferCount--;
			} else {
				/* Nothing to do */
...
static void SMARTCARD_EndTransmit_IT(SMARTCARD_HandleTypeDef *hsmartcard)
{
	...
	if ((hsmartcard->Init.Mode == SMARTCARD_MODE_TX_RX) ||
	    (hsmartcard->Init.NACKEnable == SMARTCARD_NACK_ENABLE)) {
		/* Perform a TX FIFO Flush at end of Tx phase, as all sent bytes are appearing in Rx
		 * Data register */
		__HAL_SMARTCARD_FLUSH_DRREGISTER(hsmartcard);
	}
	SET_BIT(hsmartcard->Instance->CR1, USART_CR1_UE);
	...
	if (hsmartcard->RxState == HAL_SMARTCARD_STATE_BUSY_RX) {
		*hsmartcard->pRxBuffPtr = (uint8_t)(hsmartcard->Instance->RDR & (uint8_t)0xFF);
		...
		if (hsmartcard->RxXferCount == 0U) {
			...
			/* Check if a transmit process is ongoing or not. If not disable ERR IT */
			if (hsmartcard->gState == HAL_SMARTCARD_STATE_READY) {
				/* Disable the SMARTCARD Error Interrupt: (Frame error, noise error,
				 * overrun error) */
				CLEAR_BIT(hsmartcard->Instance->CR3, USART_CR3_EIE);
			}
...
static void SMARTCARD_RxISR_FIFOEN(SMARTCARD_HandleTypeDef *hsmartcard)
{
	...
	if (hsmartcard->RxState == HAL_SMARTCARD_STATE_BUSY_RX) {
		for (nb_rx_data = hsmartcard->NbRxDataToProcess; nb_rx_data > 0U; nb_rx_data--) {
			*hsmartcard->pRxBuffPtr =
				(uint8_t)(hsmartcard->Instance->RDR & (uint8_t)0xFF);
			hsmartcard->pRxBuffPtr++;
			...
			if (hsmartcard->RxXferCount == 0U) {
				...
				/* Check if a transmit process is ongoing or not. If not disable ERR
				 * IT */
				if (hsmartcard->gState == HAL_SMARTCARD_STATE_READY) {
					/* Disable the SMARTCARD Error Interrupt: (Frame error,
