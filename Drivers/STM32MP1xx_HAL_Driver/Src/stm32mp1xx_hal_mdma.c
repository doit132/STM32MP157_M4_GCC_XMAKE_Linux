/**
  ...
  ******************************************************************************
  ...
  [..]
   ...
   (#)
       For a given Channel use HAL_MDMA_Init function to program the required configuration through
  the following parameters: transfer request , channel priority, data endianness, Source increment,
  destination increment , source data size, destination data size, data alignment, source Burst,
  destination Burst , buffer Transfer Length, Transfer Trigger Mode (buffer transfer, block
  transfer, repeated block transfer or full transfer) source and destination block address offset,
  mask address and data.

       If using the MDMA in linked list mode then use function HAL_MDMA_LinkedList_CreateNode to
  fill a transfer node. Note that parameters given to the function HAL_MDMA_Init corresponds always
  to the node zero. Use function HAL_MDMA_LinkedList_AddNode to connect the created node to the
  linked list at a given position. User can make a linked list circular using function
  HAL_MDMA_LinkedList_EnableCircularMode , this function will automatically connect the last node of
  the list to the first one in order to make the list circular. In this case the linked list will
  loop on node 1 : first node connected after the initial transfer defined by the HAL_MDMA_Init

      -@-   The initial transfer itself (node 0 corresponding to the Init).
	    User can disable the circular mode using function
  HAL_MDMA_LinkedList_DisableCircularMode, this function will then remove the connection between
  last node and first one.

       Function HAL_MDMA_LinkedList_RemoveNode can be used to remove (disconnect) a node from the
  transfer linked list. When a linked list is circular (last node connected to first one), if
  removing node1  (node where the linked list loops), the linked list remains circular and node 2
  becomes the first one. Note that if the linked list is made circular the transfer will loop
  infinitely (or until aborted by the user).

    [..]
       (+) User can select the transfer trigger mode (parameter TransferTriggerMode) to define the
  amount of data to be transfer upon a request :
	     (++) MDMA_BUFFER_TRANSFER : each request triggers a transfer of BufferTransferLength
  data with BufferTransferLength defined within the HAL_MDMA_Init.
	     ...
	     (++) MDMA_REPEAT_BLOCK_TRANSFER : each request triggers a transfer of a number of
  blocks with block size and number of blocks defined within the function
  HAL_MDMA_Start/HAL_MDMA_Start_IT or within the current linked list node parameters.
	     (++) MDMA_FULL_TRANSFER : each request triggers a full transfer
	      all blocks and all nodes(if a linked list has been created using
  HAL_MDMA_LinkedList_CreateNode \ HAL_MDMA_LinkedList_AddNode).
    ...
    [..]
	  ...
	  (+) Use HAL_MDMA_PollForTransfer() to poll for the end of current transfer or a transfer
  level In this case a fixed Timeout can be configured by User depending from his application.
	  (+) Use HAL_MDMA_Abort() function to abort the current transfer : blocking method this API
  returns when the abort ends or timeout (should not be called from an interrupt service routine).
  ...
  this case the MDMA interrupt is configured.
	  ...
	  (+) Use HAL_MDMA_Abort_IT() function to abort the current transfer : non-blocking method.
  This API will finish the execution immediately then the callback XferAbortCallback (if specified
  by the user) is asserted once the MDMA channel has effectively aborted. (could be called from an
  interrupt service routine).

	  (+) Use functions HAL_MDMA_RegisterCallback and HAL_MDMA_UnRegisterCallback respectevely
  to register unregister user callbacks from the following list :
    ...
    [..]
	 (+)  If the transfer Request corresponds to SW request (MDMA_REQUEST_SW) User can use
  function HAL_MDMA_GenerateSWRequest to trigger requests manually. Function
  HAL_MDMA_GenerateSWRequest must be used with the following precautions:
	      (++) This function returns an error if used while the Transfer has ended or not
  started.
	      (++) If used while the current request has not been served yet (current request
  transfer on going) this function returns an error and the new request is ignored.

	      Generally this function should be used in conjunctions with the MDMA callbacks:
	      (++) example 1:
		 (+++) Configure a transfer with request set to MDMA_REQUEST_SW and trigger mode set
  to MDMA_BUFFER_TRANSFER
		 (+++) Register a callback for buffer transfer complete (using callback ID set to
  HAL_MDMA_XFER_BUFFERCPLT_CB_ID)
		 (+++) After calling HAL_MDMA_Start_IT the MDMA will issue the transfer of a first
  BufferTransferLength data.
		 (+++) When the buffer transfer complete callback is asserted first buffer has been
  transferred and user can ask for a new buffer transfer request using HAL_MDMA_GenerateSWRequest.

	      (++) example 2:
		 (+++) Configure a transfer with request set to MDMA_REQUEST_SW and trigger mode set
  to MDMA_BLOCK_TRANSFER
		 (+++) Register a callback for block transfer complete (using callback ID
  HAL_MDMA_XFER_BLOCKCPLT_CB_ID)
		 (+++) After calling HAL_MDMA_Start_IT the MDMA will issue the transfer of a first
  block of data.
		 (+++) When the block transfer complete callback is asserted the first block has
  been transferred and user can ask for a new block transfer request using
  HAL_MDMA_GenerateSWRequest.

    [..]  Use HAL_MDMA_GetState() function to return the MDMA state and HAL_MDMA_GetError() in case
  of error detection.
     ...
     *** MDMA HAL driver macros list ***
     ...
     [..]
      ...
      (+) __HAL_MDMA_GET_IT_SOURCE: Check whether the specified MDMA Channel interrupt has occurred
  or not.
 ...
 * @{
 */
static void MDMA_SetConfig(MDMA_HandleTypeDef *hmdma, uint32_t SrcAddress, uint32_t DstAddress,
			   uint32_t BlockDataLength, uint32_t BlockCount);
...
HAL_StatusTypeDef HAL_MDMA_DeInit(MDMA_HandleTypeDef *hmdma)
{
	...
	/* Clear all flags */
	__HAL_MDMA_CLEAR_FLAG(hmdma, (MDMA_FLAG_TE | MDMA_FLAG_CTC | MDMA_FLAG_BRT | MDMA_FLAG_BT |
				      MDMA_FLAG_BFTC));
 ...
 * @param  hmdma      : pointer to a MDMA_HandleTypeDef structure that contains
 *                               the configuration information for the specified MDMA Channel.
 * @param  MaskAddress: specifies the address to be updated (written) with MaskData after a request
 * is served.
 * @param  MaskData:    specifies the value to be written to MaskAddress after a request is served.
 *                      MaskAddress and MaskData could be used to automatically clear a peripheral
 * flag when the request is served.
 * @retval HAL status
 */
						 ...
						 MDMA_LinkNodeConfTypeDef *pNodeConfig)
{
	...
	*/
	...
	/* if block source address offset is negative set the Block Repeat Source address Update
	 * Mode to decrement */
	if (pNodeConfig->Init.SourceBlockAddressOffset < 0) {
		...
		/*write new CBRUR Register value : source repeat block offset */
		pNode->CBRUR =
			(((uint32_t)pNodeConfig->Init.SourceBlockAddressOffset) & 0x0000FFFFU);
	}

	/* if block destination address offset is negative set the Block Repeat destination address
	 * Update Mode to decrement */
	if (pNodeConfig->Init.DestBlockAddressOffset < 0) {
		...
		/*write new CBRUR Register value : destination repeat block offset */
		pNode->CBRUR |=
			((((uint32_t)pNodeConfig->Init.DestBlockAddressOffset) & 0x0000FFFFU)
			 << MDMA_CBRUR_DUV_Pos);
	}
 ...
 * @param pPrevNode : Pointer to the new node position in the linked list or zero to insert the new
		...
		if ((uint32_t)hmdma->FirstLinkedListNodeAddress == 0U) {
			...
			while ((counter < hmdma->LinkedListNodeCounter) && (hal_status == HAL_OK)) {
				if (pNode->CLAR == (uint32_t)pNewNode) {
					hal_status = HAL_ERROR; /* error this node already exist in
								   the linked list and it is not
								   first node */
				}
			...
			if (hal_status == HAL_OK) {
				/* Check if the previous node is the last one in the current list or
				 * zero */
	...
	if (HAL_MDMA_STATE_READY == hmdma->State) {
		...
		} else if (hmdma->FirstLinkedListNodeAddress == pNode) /* Deleting first node */
		{
			...
			if (hmdma->LastLinkedListNodeAddress == pNode) {
				/*if the last node is at the same time the first one (1 single node
				after the init node 0) then update the last node too */
				...
				if ((uint32_t)hmdma->FirstLinkedListNodeAddress ==
					...
					/* if last node is looping to first (circular list) one
					 * update the last node connection */
		...
		} else /* Deleting any other node */
		{
			...
			while ((counter < hmdma->LinkedListNodeCounter) && (nodeDeleted == 0U)) {
				...
				if (ptmpNode->CLAR == ((uint32_t)pNode)) {
					...
					if (pNode == hmdma->LastLinkedListNodeAddress) {
						/*Update the linked list last node address in the
						 * handle*/
						hmdma->LastLinkedListNodeAddress = ptmpNode;
					}
					/* update the next node link after deleting
					 * pMDMA_LinkedListNode */
			...
			if (nodeDeleted == 0U) {
				/* last node reashed without finding the node to delete : return
				 * error */
				hal_status = HAL_ERROR;
			}
		}
				    ...
				    uint32_t BlockCount)
{
	...
	if (HAL_MDMA_STATE_READY == hmdma->State) {
		...
		if (hmdma->XferRepeatBlockCpltCallback != NULL) {
			/* if Repeated Block transfer complete Callback is set enable the
			 * corresponding IT*/
...
HAL_StatusTypeDef HAL_MDMA_Abort(MDMA_HandleTypeDef *hmdma)
{
	...
	if (HAL_MDMA_STATE_BUSY != hmdma->State) {
		...
		__HAL_MDMA_DISABLE_IT(hmdma, (MDMA_IT_TE | MDMA_IT_CTC | MDMA_IT_BT | MDMA_IT_BRT |
					      MDMA_IT_BFTC));
		...
		__HAL_MDMA_CLEAR_FLAG(hmdma, (MDMA_FLAG_TE | MDMA_FLAG_CTC | MDMA_FLAG_BT |
					      MDMA_FLAG_BRT | MDMA_FLAG_BFTC));
					   ...
					   uint32_t Timeout)
{
	...
	while (__HAL_MDMA_GET_FLAG(hmdma, levelFlag) == 0U) {
		...
		if (Timeout != HAL_MAX_DELAY) {
			if (((HAL_GetTick() - tickstart) > Timeout) || (Timeout == 0U)) {
				...
				(void)HAL_MDMA_Abort(
					hmdma); /* if timeout then abort the current transfer */
	...
	if (CompleteLevel == HAL_MDMA_BUFFER_TRANSFER) {
		...
		__HAL_MDMA_CLEAR_FLAG(
			hmdma, (MDMA_FLAG_BRT | MDMA_FLAG_BT | MDMA_FLAG_BFTC | MDMA_FLAG_CTC));
...
HAL_StatusTypeDef HAL_MDMA_GenerateSWRequest(MDMA_HandleTypeDef *hmdma)
{
	...
	if ((hmdma->Instance->CCR & MDMA_CCR_EN) == 0U) {
		...
		/* if an MDMA ongoing request has not yet end or if request mode is not SW request
		 * retrun error */
...
void HAL_MDMA_IRQHandler(MDMA_HandleTypeDef *hmdma)
{
	...
	generalIntFlag = 1UL << ((((uint32_t)hmdma->Instance - (uint32_t)(MDMA_Channel0)) /
	...
	if ((__HAL_MDMA_GET_FLAG(hmdma, MDMA_FLAG_CTC) != 0U)) {
		if (__HAL_MDMA_GET_IT_SOURCE(hmdma, MDMA_IT_CTC) != 0U) {
			/* Disable all the transfer interrupts */
			__HAL_MDMA_DISABLE_IT(hmdma, (MDMA_IT_TE | MDMA_IT_CTC | MDMA_IT_BT |
						      MDMA_IT_BRT | MDMA_IT_BFTC));
			   ...
			   uint32_t BlockDataLength, uint32_t BlockCount)
{
	...
	/* Clear all interrupt flags */
	__HAL_MDMA_CLEAR_FLAG(hmdma, MDMA_FLAG_TE | MDMA_FLAG_CTC | MDMA_CISR_BRTIF |
					     MDMA_CISR_BTIF | MDMA_CISR_TCIF);
...
static void MDMA_Init(MDMA_HandleTypeDef *hmdma)
{
	...
	hmdma->Instance->CCR =
		hmdma->Init.Priority | hmdma->Init.SecureMode | hmdma->Init.Endianness;
