...
HAL_StatusTypeDef HAL_CEC_Init(CEC_HandleTypeDef *hcec)
{
	...
	assert_param(
		IS_CEC_BROADCASTERROR_NO_ERRORBIT_GENERATION(hcec->Init.BroadcastMsgNoErrorBitGen));
	...
	hcec->Instance->CFGR = hcec->Init.SignalFreeTime | hcec->Init.Tolerance |
			       hcec->Init.BRERxStop | hcec->Init.BREErrorBitGen |
			       hcec->Init.LBPEErrorBitGen | hcec->Init.BroadcastMsgNoErrorBitGen |
			       hcec->Init.SignalFreeTimeOption |
			       ((uint32_t)(hcec->Init.OwnAddress) << 16U) | hcec->Init.ListenMode;
	 ...
	 * Tx arbitration lost   */
	__HAL_CEC_ENABLE_IT(hcec, CEC_IT_RXBR | CEC_IT_RXEND | CEC_IER_RX_ALL_ERR | CEC_IT_TXBR |
					  CEC_IT_TXEND | CEC_IER_TX_ALL_ERR);
...
HAL_StatusTypeDef HAL_CEC_DeInit(CEC_HandleTypeDef *hcec)
{
	 ...
	 * Tx-Buffer Underrun IT
	 * Tx arbitration lost   */
	__HAL_CEC_DISABLE_IT(hcec, CEC_IT_RXBR | CEC_IT_RXEND | CEC_IER_RX_ALL_ERR | CEC_IT_TXBR |
					   CEC_IT_TXEND | CEC_IER_TX_ALL_ERR);
					     ...
					     HAL_CEC_CallbackIDTypeDef CallbackID)
{
	...
	if (hcec->gState == HAL_CEC_STATE_READY) {
		...
		case HAL_CEC_TX_CPLT_CB_ID:
			hcec->TxCpltCallback =
				HAL_CEC_TxCpltCallback; /* Legacy weak  TxCpltCallback */
			break;
 ...
 *         CEC RX complete Callback is redirected to the weak HAL_CEC_RxCpltCallback() predefined
 * callback
 * @param  hcec CEC handle
 * @retval HAL status
 */
				      ...
				      uint8_t DestinationAddress, uint8_t *pData, uint32_t Size)
{
	...
	if (hcec->gState == HAL_CEC_STATE_READY) {
		...
		/* send header block */
		hcec->Instance->TXDR =
			(uint32_t)(((uint32_t)InitiatorAddress << CEC_INITIATOR_LSB_POS) |
				   DestinationAddress);
...
void HAL_CEC_IRQHandler(CEC_HandleTypeDef *hcec)
{
	...
	if ((reg & (CEC_ISR_RXOVR | CEC_ISR_BRE | CEC_ISR_SBPE | CEC_ISR_LBPE | CEC_ISR_RXACKE |
		    CEC_ISR_TXUDR | CEC_ISR_TXERR | CEC_ISR_TXACKE)) != 0U) {
		hcec->ErrorCode = reg;
		__HAL_CEC_CLEAR_FLAG(hcec, HAL_CEC_ERROR_RXOVR | HAL_CEC_ERROR_BRE | CEC_FLAG_LBPE |
						   CEC_FLAG_SBPE | HAL_CEC_ERROR_RXACKE |
						   HAL_CEC_ERROR_TXUDR | HAL_CEC_ERROR_TXERR |
						   HAL_CEC_ERROR_TXACKE);

		if ((reg & (CEC_ISR_RXOVR | CEC_ISR_BRE | CEC_ISR_SBPE | CEC_ISR_LBPE |
			    CEC_ISR_RXACKE)) != 0U) {
			hcec->Init.RxBuffer -= hcec->RxXferSize;
			hcec->RxXferSize = 0U;
			hcec->RxState = HAL_CEC_STATE_READY;
