  ...
  HAL_SMBUS_IsDeviceReady()
    ...
    *** Interrupt mode IO operation ***
    ...
    [..]
      ...
      (++) At transmission end of transfer HAL_SMBUS_MasterTxCpltCallback() is executed and users
  can add their own code by customization of function pointer HAL_SMBUS_MasterTxCpltCallback()
...
HAL_StatusTypeDef HAL_SMBUS_Init(SMBUS_HandleTypeDef *hsmbus)
{
	...
	if (hsmbus->State == HAL_SMBUS_STATE_RESET) {
		...
		hsmbus->MasterTxCpltCallback =
			HAL_SMBUS_MasterTxCpltCallback; /* Legacy weak MasterTxCpltCallback */
		hsmbus->MasterRxCpltCallback =
			HAL_SMBUS_MasterRxCpltCallback; /* Legacy weak MasterRxCpltCallback */
		hsmbus->SlaveTxCpltCallback =
			HAL_SMBUS_SlaveTxCpltCallback; /* Legacy weak SlaveTxCpltCallback  */
		hsmbus->SlaveRxCpltCallback =
			HAL_SMBUS_SlaveRxCpltCallback; /* Legacy weak SlaveRxCpltCallback  */
		hsmbus->ListenCpltCallback =
			HAL_SMBUS_ListenCpltCallback; /* Legacy weak ListenCpltCallback   */
	...
	if (hsmbus->Init.OwnAddress1 != 0UL) {
		...
		{
			hsmbus->Instance->OAR1 =
				(I2C_OAR1_OA1EN | I2C_OAR1_OA1MODE | hsmbus->Init.OwnAddress1);
		}
	}
	...
	/* Enable the AUTOEND by default, and enable NACK (should be disable only during Slave
	 * process) */
	...
	if (HAL_SMBUS_STATE_READY == hsmbus->State) {
		...
		case HAL_SMBUS_ERROR_CB_ID:
			hsmbus->ErrorCallback =
				HAL_SMBUS_ErrorCallback; /* Legacy weak ErrorCallback        */
			break;
 ...
 *         Info Ready SMBUS Callback is redirected to the weak HAL_SMBUS_AddrCallback() predefined
 * callback
 * @param  hsmbus Pointer to a SMBUS_HandleTypeDef structure that contains
 *                the configuration information for the specified SMBUS.
 * @retval HAL status
					       ...
					       uint8_t *pData, uint16_t Size, uint32_t XferOptions)
{
	...
	if (hsmbus->State == HAL_SMBUS_STATE_READY) {
		...
		if ((hsmbus->XferSize < hsmbus->XferCount) &&
		    (hsmbus->XferSize == MAX_NBYTE_SIZE)) {
			SMBUS_TransferConfig(hsmbus, DevAddress, (uint8_t)hsmbus->XferSize,
					     SMBUS_RELOAD_MODE |
						     (hsmbus->XferOptions & SMBUS_SENDPEC_MODE),
			...
			if ((hsmbus->PreviousState == HAL_SMBUS_STATE_MASTER_BUSY_TX) &&
				...
				SMBUS_TransferConfig(hsmbus, DevAddress, (uint8_t)hsmbus->XferSize,
						     hsmbus->XferOptions, SMBUS_NO_STARTSTOP);
			}
			/* Else transfer direction change, so generate Restart with new transfer
			   direction */
			else {
			...
			/* If PEC mode is enable, size to transmit manage by SW part should be
			 * Size-1 byte, corresponding to PEC byte */
			/* PEC byte is automatically sent by HW block, no need to manage it in
			 * Transmit process */
...
HAL_StatusTypeDef HAL_SMBUS_Master_Abort_IT(SMBUS_HandleTypeDef *hsmbus, uint16_t DevAddress)
{
	if (hsmbus->State == HAL_SMBUS_STATE_READY) {
		...
		/* Set AUTOEND mode, this will generate a NACK then STOP condition to abort the
		 * current transfer */
...
/**
 * @brief  Transmit in slave/device SMBUS mode an amount of data in non-blocking mode with
 * Interrupt.
 * @param  hsmbus Pointer to a SMBUS_HandleTypeDef structure that contains
 *                the configuration information for the specified SMBUS.
 * @param  pData Pointer to data buffer
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SMBUS_Slave_Transmit_IT(SMBUS_HandleTypeDef *hsmbus, uint8_t *pData,
					      uint16_t Size, uint32_t XferOptions)
{
	...
	if ((hsmbus->State & HAL_SMBUS_STATE_LISTEN) == HAL_SMBUS_STATE_LISTEN) {
		...
		if ((hsmbus->XferSize < hsmbus->XferCount) &&
			...
			/* If PEC mode is enable, size to transmit should be Size-1 byte,
			 * corresponding to PEC byte */
			/* PEC byte is automatically sent by HW block, no need to manage it in
			 * Transmit process */
			if (SMBUS_GET_PEC_MODE(hsmbus) != 0UL) {
				hsmbus->XferSize--;
					     ...
					     uint16_t Size, uint32_t XferOptions)
{
	...
	if ((hsmbus->State & HAL_SMBUS_STATE_LISTEN) == HAL_SMBUS_STATE_LISTEN) {
		...
		/* If XferSize equal "1", or XferSize equal "2" with PEC requested (mean 1 data byte
		 * + 1 PEC byte */
		/* no need to set RELOAD bit mode, a ACK will be automatically generated in that
		 * case */
		/* else need to set RELOAD bit mode to generate an automatic ACK at each byte
		 * Received */
		/* This RELOAD bit will be reset for last BYTE to be receive in SMBUS_Slave_ISR */
		if (((SMBUS_GET_PEC_MODE(hsmbus) != 0UL) && (hsmbus->XferSize == 2U)) ||
		    (hsmbus->XferSize == 1U)) {
			SMBUS_TransferConfig(hsmbus, 0, (uint8_t)hsmbus->XferSize,
					     hsmbus->XferOptions, SMBUS_NO_STARTSTOP);
		} else {
			SMBUS_TransferConfig(hsmbus, 0, 1, hsmbus->XferOptions | SMBUS_RELOAD_MODE,
					     SMBUS_NO_STARTSTOP);
		}
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SMBUS_IsDeviceReady(SMBUS_HandleTypeDef *hsmbus, uint16_t DevAddress,
					  uint32_t Trials, uint32_t Timeout)
{
	...
	if (hsmbus->State == HAL_SMBUS_STATE_READY) {
		...
		do {
			/* Generate Start */
			hsmbus->Instance->CR2 =
				SMBUS_GENERATE_START(hsmbus->Init.AddressingMode, DevAddress);
			...
			if (__HAL_SMBUS_GET_FLAG(hsmbus, SMBUS_FLAG_AF) == RESET) {
				...
				if (SMBUS_WaitOnFlagUntilTimeout(hsmbus, SMBUS_FLAG_STOPF, RESET,
								 Timeout) != HAL_OK) {
					return HAL_ERROR;
				}
...
void HAL_SMBUS_EV_IRQHandler(SMBUS_HandleTypeDef *hsmbus)
{
	...
	if ((SMBUS_CHECK_IT_SOURCE(tmpcr1value, (SMBUS_IT_TCI | SMBUS_IT_STOPI | SMBUS_IT_NACKI |
						 SMBUS_IT_TXI)) != RESET) &&
		...
		/* Slave mode selected */
		if ((hsmbus->State & HAL_SMBUS_STATE_SLAVE_BUSY_TX) ==
		    HAL_SMBUS_STATE_SLAVE_BUSY_TX) {
			(void)SMBUS_Slave_ISR(hsmbus, tmpisrvalue);
		}
		/* Master mode selected */
		else if ((hsmbus->State & HAL_SMBUS_STATE_MASTER_BUSY_TX) ==
	...
	if ((SMBUS_CHECK_IT_SOURCE(tmpcr1value, (SMBUS_IT_TCI | SMBUS_IT_STOPI | SMBUS_IT_NACKI |
	    ...
	    ((SMBUS_CHECK_FLAG(tmpisrvalue, SMBUS_FLAG_RXNE) != RESET) ||
	     ...
	     (SMBUS_CHECK_FLAG(tmpisrvalue, SMBUS_FLAG_AF) != RESET))) {
		/* Slave mode selected */
		if ((hsmbus->State & HAL_SMBUS_STATE_SLAVE_BUSY_RX) ==
...
static HAL_StatusTypeDef SMBUS_Master_ISR(SMBUS_HandleTypeDef *hsmbus, uint32_t StatusFlags)
{
	...
	if (SMBUS_CHECK_FLAG(StatusFlags, SMBUS_FLAG_AF) != RESET) {
		...
		if (hsmbus->State == HAL_SMBUS_STATE_MASTER_BUSY_TX) {
			...
			/* Flush remaining data in Fifo register in case of error occurs before
			 * TXEmpty */
			/* Disable the selected SMBUS peripheral */
			__HAL_SMBUS_DISABLE(hsmbus);
		...
		*hsmbus->pBuffPtr = (uint8_t)(hsmbus->Instance->RXDR);
		...
		if ((hsmbus->XferCount != 0U) && (hsmbus->XferSize == 0U)) {
			...
			if (hsmbus->XferCount > MAX_NBYTE_SIZE) {
				...
				/* If PEC mode is enable, size to transmit should be Size-1 byte,
				 * corresponding to PEC byte */
				/* PEC byte is automatically sent by HW block, no need to manage it
				 * in Transmit process */
				if (SMBUS_GET_PEC_MODE(hsmbus) != 0UL) {
					hsmbus->XferSize--;
					hsmbus->XferCount--;
		...
		if (hsmbus->XferCount == 0U) {
			...
			/* Call TxCpltCallback() if no stop mode is set */
				...
				/* The stop will be done at the end of transfer, when
				 * SMBUS_AUTOEND_MODE enable */
...
static HAL_StatusTypeDef SMBUS_Slave_ISR(SMBUS_HandleTypeDef *hsmbus, uint32_t StatusFlags)
{
	...
	if (SMBUS_CHECK_FLAG(StatusFlags, SMBUS_FLAG_AF) != RESET) {
		...
		if (hsmbus->XferCount == 0U) {
			...
			/* if no, error usecase, a Non-Acknowledge of last Data is generated by the
			 * HOST*/
			/* Clear NACK Flag */
			__HAL_SMBUS_CLEAR_FLAG(hsmbus, SMBUS_FLAG_AF);
		...
		if ((hsmbus->State & HAL_SMBUS_STATE_SLAVE_BUSY_RX) ==
			...
			*hsmbus->pBuffPtr = (uint8_t)(hsmbus->Instance->RXDR);
			...
			if (hsmbus->XferCount == 1U) {
				...
				SMBUS_TransferConfig(hsmbus, 0, 1, hsmbus->XferOptions,
						     SMBUS_NO_STARTSTOP);
			} else if (hsmbus->XferCount == 0U) {
				/* Last Byte is received, disable Interrupt */
				SMBUS_Disable_IRQ(hsmbus, SMBUS_IT_RX);

				/* Remove HAL_SMBUS_STATE_SLAVE_BUSY_RX, keep only
				...
				SMBUS_TransferConfig(hsmbus, 0, 1,
						     SMBUS_RELOAD_MODE | (hsmbus->XferOptions &
									  SMBUS_SENDPEC_MODE),
						     SMBUS_NO_STARTSTOP);
		...
		} else if ((hsmbus->State & HAL_SMBUS_STATE_SLAVE_BUSY_TX) ==
			   HAL_SMBUS_STATE_SLAVE_BUSY_TX) {
			if ((hsmbus->XferCount != 0U) && (hsmbus->XferSize == 0U)) {
				if (hsmbus->XferCount > MAX_NBYTE_SIZE) {
					SMBUS_TransferConfig(
						hsmbus, 0, MAX_NBYTE_SIZE,
						(SMBUS_RELOAD_MODE |
						 (hsmbus->XferOptions & SMBUS_SENDPEC_MODE)),
						SMBUS_NO_STARTSTOP);
					hsmbus->XferSize = MAX_NBYTE_SIZE;
					...
					SMBUS_TransferConfig(hsmbus, 0, (uint8_t)hsmbus->XferSize,
							     hsmbus->XferOptions,
							     SMBUS_NO_STARTSTOP);
					/* If PEC mode is enable, size to transmit should be Size-1
					 * byte, corresponding to PEC byte */
					/* PEC byte is automatically sent by HW block, no need to
					 * manage it in Transmit process */
					if (SMBUS_GET_PEC_MODE(hsmbus) != 0UL) {
						hsmbus->XferSize--;
						hsmbus->XferCount--;
					}
		...
		/* If it is the case, this last write in TXDR is not sent, correspond to a dummy
		 * TXIS event */
		if (hsmbus->XferCount > 0U) {
			/* Write data to TXDR */
			hsmbus->Instance->TXDR = *hsmbus->pBuffPtr;
...
static void SMBUS_Enable_IRQ(SMBUS_HandleTypeDef *hsmbus, uint32_t InterruptRequest)
{
	...
	if ((InterruptRequest & SMBUS_IT_TX) == SMBUS_IT_TX) {
		...
		tmpisr |= SMBUS_IT_ERRI | SMBUS_IT_TCI | SMBUS_IT_STOPI | SMBUS_IT_NACKI |
			  SMBUS_IT_TXI;
	}
...
static void SMBUS_Disable_IRQ(SMBUS_HandleTypeDef *hsmbus, uint32_t InterruptRequest)
{
	...
	if ((tmpstate == HAL_SMBUS_STATE_READY) &&
	    ((InterruptRequest & SMBUS_IT_ALERT) == SMBUS_IT_ALERT)) {
...
static void SMBUS_ITErrorHandler(SMBUS_HandleTypeDef *hsmbus)
{
	...
	if ((tmperror != HAL_SMBUS_ERROR_NONE) && (tmperror != HAL_SMBUS_ERROR_ACKF)) {
		...
		if ((tmperror & HAL_SMBUS_ERROR_ALERT) != HAL_SMBUS_ERROR_ALERT) {
			...
			if (((tmpstate & HAL_SMBUS_STATE_SLAVE_BUSY_TX) ==
			     HAL_SMBUS_STATE_SLAVE_BUSY_TX) ||
			    ((tmpstate & HAL_SMBUS_STATE_SLAVE_BUSY_RX) ==
			     HAL_SMBUS_STATE_SLAVE_BUSY_RX)) {
...
/**
 * @brief  Handle SMBUSx communication when starting transfer or during transfer (TC or TCR flag are
 * set).
 * @param  hsmbus SMBUS handle.
