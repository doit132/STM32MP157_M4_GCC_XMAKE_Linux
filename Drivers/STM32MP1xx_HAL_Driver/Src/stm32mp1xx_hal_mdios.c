/**
 ...
 ===============================================================================
	...
	(@) A callback is executed for each genereted interrupt, so the driver provide the following
	    HAL_MDIOS_WriteCpltCallback(), HAL_MDIOS_ReadCpltCallback() and
 HAL_MDIOS_ErrorCallback()
	(@) HAL_MDIOS_IRQHandler() must be called from the MDIOS IRQ Handler, to handle the
 interrupt and execute the previous callbacks
 ...
 */
		...
		#define MDIOS_ALL_ERRORS_FLAG                                                      \
			((uint32_t)(MDIOS_SR_PERF | MDIOS_SR_SERF | MDIOS_SR_TERF))
...
HAL_StatusTypeDef HAL_MDIOS_Init(MDIOS_HandleTypeDef *hmdios)
{
	...
	tmpcr |= (uint32_t)(((hmdios->Init.PortAddress) << MDIOS_PORT_ADDRESS_SHIFT) |
			    (hmdios->Init.PreambleCheck) | (MDIOS_CR_EN));
