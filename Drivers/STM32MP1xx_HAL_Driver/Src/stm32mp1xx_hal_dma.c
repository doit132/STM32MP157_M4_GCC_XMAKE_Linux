...
HAL_StatusTypeDef HAL_DMA_Init(DMA_HandleTypeDef *hdma)
{
	...
	if (IS_DMA_INSTANCE(hdma) != 0U) /* DMA1 or DMA2 instance */
	{
		...
		/* Clear CHSEL, MBURST, PBURST, PL, MSIZE, PSIZE, MINC, PINC, CIRC, DIR, CT and DBM
		 * bits */
		registerValue &= ((uint32_t) ~(DMA_SxCR_MBURST | DMA_SxCR_PBURST | DMA_SxCR_PL |
					       DMA_SxCR_MSIZE | DMA_SxCR_PSIZE | DMA_SxCR_MINC |
					       DMA_SxCR_PINC | DMA_SxCR_CIRC | DMA_SxCR_DIR |
					       DMA_SxCR_CT | DMA_SxCR_DBM));

		/* Prepare the DMA Stream configuration */
		registerValue |= hdma->Init.Direction | hdma->Init.PeriphInc | hdma->Init.MemInc |
		...
		/* Initialize StreamBaseAddress and StreamIndex parameters to be used to calculate
		   DMA steam Base Address needed by HAL_DMA_IRQHandler() and
					  ...
					  uint32_t Timeout)
{
	...
	if (Timeout != HAL_MAX_DELAY) {
		if (((HAL_GetTick() - tickstart) > Timeout) || (Timeout == 0U)) {
			...
			/* No need to check return value: as in this case we will return HAL_ERROR
			 * with HAL_DMA_ERROR_TIMEOUT error code  */
		...
		if ((tmpisr_dma & (DMA_FLAG_HTIF0_4 << (hdma->StreamIndex & 0x1FU))) != 0U) {
			if (__HAL_DMA_GET_IT_SOURCE(hdma, DMA_IT_HT) != 0U) {
				...
				if (((((DMA_Stream_TypeDef *)hdma->Instance)->CR) &
					...
					/* Disable the half transfer interrupt if the DMA mode is
					 * not CIRCULAR */
					if ((((DMA_Stream_TypeDef *)hdma->Instance)->CR &
					     DMA_SxCR_CIRC) == 0U) {
		...
		if ((tmpisr_dma & (DMA_FLAG_TCIF0_4 << (hdma->StreamIndex & 0x1FU))) != 0U) {
			if (__HAL_DMA_GET_IT_SOURCE(hdma, DMA_IT_TC) != 0U) {
				...
				if (HAL_DMA_STATE_ABORT == hdma->State) {
					...
					/* Clear all interrupt flags at correct offset within the
					 * register */
					regs_dma->IFCR = 0x3FUL << (hdma->StreamIndex & 0x1FU);
...
static uint32_t DMA_CalcBaseAndBitshift(DMA_HandleTypeDef *hdma)
{
	...
	if (stream_number > 3U) {
		...
		hdma->StreamBaseAddress =
			(((uint32_t)((uint32_t *)hdma->Instance) & (uint32_t)(~0x3FFU)) + 4U);
	} else {
		/* return pointer to LISR and LIFCR */
		hdma->StreamBaseAddress =
			((uint32_t)((uint32_t *)hdma->Instance) & (uint32_t)(~0x3FFU));
	}
...
/**
 * @brief  Updates the DMA handle with the DMAMUX  channel and status mask depending on stream
 * number
 * @param  hdma:       pointer to a DMA_HandleTypeDef structure that contains
 *                     the configuration information for the specified DMA Stream.
 * @retval HAL status
