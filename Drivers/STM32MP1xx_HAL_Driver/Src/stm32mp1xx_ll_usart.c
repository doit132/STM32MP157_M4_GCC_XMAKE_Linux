 ...
 */
	 ...
	 */
		...
		#define IS_LL_USART_BAUDRATE(__BAUDRATE__) ((__BAUDRATE__) <= 12500001U)

		/* __VALUE__ In case of oversampling by 16 and 8, BRR content must be greater than
		 * or equal to 16d. */
...
ErrorStatus LL_USART_DeInit(USART_TypeDef *USARTx)
{
	...
	if (USARTx == USART1) {
		...
		LL_APB5_GRP1_ForceReset(LL_APB5_GRP1_PERIPH_USART1);

		/* Release reset of USART clock */
		LL_APB5_GRP1_ReleaseReset(LL_APB5_GRP1_PERIPH_USART1);
 ...
 *         the USART is disabled (USART_CR1_UE bit =0), USART Peripheral should be in disabled state
 * prior calling this function. Otherwise, ERROR result will be returned.
 * @note   Baud rate value stored in USART_InitStruct BaudRate field, should be valid (different
 * from 0).
 * @param  USARTx USART Instance
...
ErrorStatus LL_USART_Init(USART_TypeDef *USARTx, LL_USART_InitTypeDef *USART_InitStruct)
{
	...
	if (LL_USART_IsEnabled(USARTx) == 0U) {
		 ...
		 * Configure USARTx CR1 (USART Word Length, Parity, Mode and Oversampling bits) with
		 * parameters:
		 * - DataWidth:          USART_CR1_M bits according to USART_InitStruct->DataWidth
		 * value
		 * - Parity:             USART_CR1_PCE, USART_CR1_PS bits according to
		 * USART_InitStruct->Parity value
		 ...
		 * - Stop Bits:          USART_CR2_STOP bits according to USART_InitStruct->StopBits
		 * value.
		 ...
		 */
		if (USARTx == USART1) {
			periphclk = LL_RCC_GetUARTClockFreq(LL_RCC_USART1_CLKSOURCE);
		} else if (USARTx == USART2) {
			periphclk = LL_RCC_GetUARTClockFreq(LL_RCC_UART24_CLKSOURCE);
		} else if (USARTx == USART3) {
			periphclk = LL_RCC_GetUARTClockFreq(LL_RCC_UART35_CLKSOURCE);
		} else if (USARTx == UART4) {
			periphclk = LL_RCC_GetUARTClockFreq(LL_RCC_UART24_CLKSOURCE);
		} else if (USARTx == UART5) {
			periphclk = LL_RCC_GetUARTClockFreq(LL_RCC_UART35_CLKSOURCE);
		} else if (USARTx == USART6) {
			periphclk = LL_RCC_GetUARTClockFreq(LL_RCC_USART6_CLKSOURCE);
		} else if (USARTx == UART7) {
		...
		/* Configure the USART Baud Rate :
		   ...
		   - Peripheral clock as returned by RCC service, should be valid (different from
		   0).
		*/
		if ((periphclk != LL_RCC_PERIPH_FREQUENCY_NO) &&
		    (USART_InitStruct->BaudRate != 0U)) {
			status = SUCCESS;
			LL_USART_SetBaudRate(USARTx, periphclk, USART_InitStruct->PrescalerValue,
		 ...
		 * - PrescalerValue: USART_PRESC_PRESCALER bits according to
		 * USART_InitStruct->PrescalerValue value.
		 */
			       ...
			       LL_USART_ClockInitTypeDef *USART_ClockInitStruct)
{
	...
	if (LL_USART_IsEnabled(USARTx) == 0U) {
		 ...
		 * - Clock Output:                USART_CR2_CLKEN bit according to
		 * USART_ClockInitStruct->ClockOutput value
		 * - Clock Polarity:              USART_CR2_CPOL bit according to
		 * USART_ClockInitStruct->ClockPolarity value
		 * - Clock Phase:                 USART_CR2_CPHA bit according to
		 * USART_ClockInitStruct->ClockPhase value
		 * - Last Bit Clock Pulse Output: USART_CR2_LBCL bit according to
		 * USART_ClockInitStruct->LastBitClockPulse value.
		 */
...
void LL_USART_ClockStructInit(LL_USART_ClockInitTypeDef *USART_ClockInitStruct)
{
	...
	USART_ClockInitStruct->ClockPolarity =
		LL_USART_POLARITY_LOW;				  /* Not relevant when ClockOutput =
								     LL_USART_CLOCK_DISABLE */
	USART_ClockInitStruct->ClockPhase = LL_USART_PHASE_1EDGE; /* Not relevant when ClockOutput =
								     LL_USART_CLOCK_DISABLE */
	USART_ClockInitStruct->LastBitClockPulse =
		LL_USART_LASTCLKPULSE_NO_OUTPUT; /* Not relevant when ClockOutput =
						    LL_USART_CLOCK_DISABLE */
}
