 ...
 */
	...
	#define BTR_CLEAR_MASK                                                                     \
		((uint32_t)(FMC_BTR1_ADDSET | FMC_BTR1_ADDHLD | FMC_BTR1_DATAST |                  \
	...
	/* BWTR register clear mask */
		...
		#define BWTR_CLEAR_MASK                                                            \
			((uint32_t)(FMC_BWTR1_ADDSET | FMC_BWTR1_ADDHLD | FMC_BWTR1_DATAST |       \
				    FMC_BWTR1_BUSTURN | FMC_BWTR1_ACCMOD | FMC_BWTR1_DATAHLD))
	#else
		#define BWTR_CLEAR_MASK                                                            \
			((uint32_t)(FMC_BWTR1_ADDSET | FMC_BWTR1_ADDHLD | FMC_BWTR1_DATAST |       \
...
/** @defgroup FMC_LL_NORSRAM_Exported_Functions_Group1 Initialization and de-initialization
functions
...
HAL_StatusTypeDef FMC_NORSRAM_Init(FMC_NORSRAM_TypeDef *Device, FMC_NORSRAM_InitTypeDef *Init)
{
	...
	MODIFY_REG(Device->BTCR[Init->NSBank],
		   (FMC_BCR1_MBKEN | FMC_BCR1_MUXEN | FMC_BCR1_MTYP | FMC_BCR1_MWID |
		    FMC_BCR1_FACCEN | FMC_BCR1_BURSTEN | FMC_BCR1_WAITPOL | FMC_BCR1_WAITCFG |
		    FMC_BCR1_WREN | FMC_BCR1_WAITEN | FMC_BCR1_EXTMOD | FMC_BCR1_ASYNCWAIT |
		    FMC_BCR1_CBURSTRW | FMC_BCR1_CCLKEN | FMC_BCR1_NBLSET | FMC_BCR1_CPSIZE),
		   (flashaccess | Init->DataAddressMux | Init->MemoryType | Init->MemoryDataWidth |
		    Init->BurstAccessMode | Init->WaitSignalPolarity | Init->WaitSignalActive |
		    Init->WriteOperation | Init->WaitSignal | Init->ExtendedMode |
		    Init->AsynchronousWait | Init->WriteBurst | Init->ContinuousClock |
		    Init->NBLSetupTime | Init->PageSize));
	...
	if (Init->MaxChipSelectPulse == ENABLE) {
		...
		MODIFY_REG(Device->PCSCNTR, FMC_PCSCNTR_CSCOUNT,
			   (uint32_t)(Init->MaxChipSelectPulseTime));
					  ...
					  FMC_NORSRAM_TimingTypeDef *Timing, uint32_t Bank)
{
	...
	MODIFY_REG(Device->BTCR[Bank + 1U], BTR_CLEAR_MASK,
		   (Timing->AddressSetupTime | ((Timing->AddressHoldTime) << FMC_BTR1_ADDHLD_Pos) |
		    ...
		    (((Timing->DataLatency) - 2U) << FMC_BTR1_DATLAT_Pos) | (Timing->AccessMode)));

	/* Configure Clock division value (in NORSRAM bank 1) when continuous clock is enabled */
	if (HAL_IS_BIT_SET(Device->BTCR[FMC_NORSRAM_BANK1], FMC_BCR1_CCLKEN)) {
		tmpr = (uint32_t)(Device->BTCR[FMC_NORSRAM_BANK1 + 1U] &
				  ~(((uint32_t)0x0F) << FMC_BTR1_CLKDIV_Pos));
						   ...
						   uint32_t ExtendedMode)
{
	...
	if (ExtendedMode == FMC_EXTENDED_MODE_ENABLE) {
		...
		/* Set NORSRAM device timing register for write configuration, if extended mode is
		 * used */
