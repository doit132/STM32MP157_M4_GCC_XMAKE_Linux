/**
 ...
 (HAL_SPDIFRX_ReceiveControlFlow_IT() and HAL_SPDIFRX_ReceiveDataFlow_IT() API's).
 ...
 HAL_SPDIFRX_ReceiveControlFlow_DMA() API's).
	    ...
	    (+++) Configure the declared DMA handle structure CtrlRx/DataRx with the required
 parameters.
	    (+++) Configure the DMA Channel.
	    (+++) Associate the initialized DMA handle to the SPDIFRX DMA CtrlRx/DataRx handle.
	    (+++) Configure the priority and enable the NVIC for the transfer complete interrupt on
 the DMA CtrlRx/DataRx channel.

   (#) Program the input selection, re-tries number, wait for activity, channel status selection,
 data format, stereo mode and masking of user bits using HAL_SPDIFRX_Init() function.

   -@- The specific SPDIFRX interrupts (RXNE/CSRNE and Error Interrupts) will be managed using the
 macros
   ...
   *** SPDIFRX HAL driver macros list ***
   ...
   [..]
     Below the list of most used macros in USART HAL driver.
      ...
      (+) __HAL_SPDIFRX_SYNC: Enable the synchronization state of the specified SPDIFRX peripheral
 (SYNC State)
      (+) __HAL_SPDIFRX_RCV: Enable the receive state of the specified SPDIFRX peripheral (RCV
 State)
...
HAL_StatusTypeDef HAL_SPDIFRX_Init(SPDIFRX_HandleTypeDef *hspdif)
{
	...
	tmpreg &= ~((uint16_t)SPDIFRX_CR_RXSTEO | SPDIFRX_CR_DRFMT | SPDIFRX_CR_PMSK |
		    SPDIFRX_CR_VMSK | SPDIFRX_CR_CUMSK | SPDIFRX_CR_PTMSK | SPDIFRX_CR_CHSEL |
		    SPDIFRX_CR_NBTR | SPDIFRX_CR_WFA | SPDIFRX_CR_INSEL);

	/* Sets the new configuration of the SPDIFRX peripheral */
	tmpreg |= ((uint16_t)hspdif->Init.StereoMode | hspdif->Init.InputSelection |
		   hspdif->Init.Retries | hspdif->Init.WaitForActivity |
		   hspdif->Init.ChannelSelection | hspdif->Init.DataFormat |
		   hspdif->Init.PreambleTypeMask | hspdif->Init.ChannelStatusMask |
		   hspdif->Init.ValidityBitMask | hspdif->Init.ParityErrorMask);
					    ...
					    SPDIFRX_SetDataFormatTypeDef sDataFormat)
{
	...
	/* Sets the new configuration of the SPDIFRX peripheral */
	tmpreg |= ((uint16_t)sDataFormat.StereoMode | sDataFormat.DataFormat |
		   sDataFormat.PreambleTypeMask | sDataFormat.ChannelStatusMask |
		   sDataFormat.ValidityBitMask | sDataFormat.ParityErrorMask);
...
===============================================================================
	...
	(++) HAL_SPDIFRX_ReceiveControlFlow()
		(+@) Do not use blocking mode to receive both control and data flow at the same
time.
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SPDIFRX_ReceiveDataFlow(SPDIFRX_HandleTypeDef *hspdif, uint32_t *pData,
					      uint16_t Size, uint32_t Timeout)
{
	...
	if (hspdif->State == HAL_SPDIFRX_STATE_READY) {
		...
		if (SPDIFRX_WaitOnFlagUntilTimeout(hspdif, SPDIFRX_FLAG_SYNCD, RESET, Timeout,
						   tickstart) != HAL_OK) {
			return HAL_TIMEOUT;
		}
		...
		while (Size > 0U) {
			...
			if (SPDIFRX_WaitOnFlagUntilTimeout(hspdif, SPDIFRX_FLAG_RXNE, RESET,
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SPDIFRX_ReceiveControlFlow(SPDIFRX_HandleTypeDef *hspdif, uint32_t *pData,
						 uint16_t Size, uint32_t Timeout)
{
	uint32_t tickstart = 0U;
	...
	if (hspdif->State == HAL_SPDIFRX_STATE_READY) {
		...
		while (Size > 0U) {
			...
			if (SPDIFRX_WaitOnFlagUntilTimeout(hspdif, SPDIFRX_FLAG_CSRNE, RESET,
							   Timeout, tickstart) != HAL_OK) {
				return HAL_TIMEOUT;
			}
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SPDIFRX_ReceiveDataFlow_IT(SPDIFRX_HandleTypeDef *hspdif, uint32_t *pData,
						 uint16_t Size)
{
	uint32_t tickstart = 0U;
	...
	if ((hspdif->State == HAL_SPDIFRX_STATE_READY) ||
		...
		if (((SPDIFRX->CR & SPDIFRX_CR_SPDIFEN) != SPDIFRX_STATE_SYNC) ||
		    ((SPDIFRX->CR & SPDIFRX_CR_SPDIFEN) != 0x00U)) {
			/* Start synchronization */
			__HAL_SPDIFRX_SYNC(hspdif);
			...
			/* Wait until SYNCD flag is set */
			if (SPDIFRX_WaitOnFlagUntilTimeout(hspdif, SPDIFRX_FLAG_SYNCD, RESET,
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SPDIFRX_ReceiveControlFlow_IT(SPDIFRX_HandleTypeDef *hspdif, uint32_t *pData,
						    uint16_t Size)
{
	uint32_t tickstart = 0U;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SPDIFRX_ReceiveDataFlow_DMA(SPDIFRX_HandleTypeDef *hspdif, uint32_t *pData,
						  uint16_t Size)
{
	uint32_t tickstart = 0U;
	...
	if ((hspdif->State == HAL_SPDIFRX_STATE_READY) ||
		...
		HAL_DMA_Start_IT(hspdif->hdmaDrRx, (uint32_t)&hspdif->Instance->DR,
				 (uint32_t)hspdif->pRxBuffPtr, Size);
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SPDIFRX_ReceiveControlFlow_DMA(SPDIFRX_HandleTypeDef *hspdif, uint32_t *pData,
						     uint16_t Size)
{
	uint32_t tickstart = 0U;
	...
	if ((hspdif->State == HAL_SPDIFRX_STATE_READY) ||
		...
		/* Enable the DMA request */
		HAL_DMA_Start_IT(hspdif->hdmaCsRx, (uint32_t)&hspdif->Instance->CSR,
				 (uint32_t)hspdif->pCsBuffPtr, Size);
...
void HAL_SPDIFRX_IRQHandler(SPDIFRX_HandleTypeDef *hspdif)
{
	...
	if ((__HAL_SPDIFRX_GET_FLAG(hspdif, SPDIFRX_FLAG_RXNE) != RESET) &&
	    (__HAL_SPDIFRX_GET_IT_SOURCE(hspdif, SPDIFRX_IT_RXNE) != RESET)) {
		__HAL_SPDIFRX_CLEAR_IT(hspdif, SPDIFRX_IT_RXNE);
		SPDIFRX_ReceiveDataFlow_IT(hspdif);
	}

	/* SPDIFRX Synchronization done  -------------------------------------------------------*/
	if ((__HAL_SPDIFRX_GET_FLAG(hspdif, SPDIFRX_IT_SYNCDIE) != RESET) &&
	    (__HAL_SPDIFRX_GET_IT_SOURCE(hspdif, SPDIFRX_IT_SYNCDIE) != RESET)) {
		__HAL_SPDIFRX_CLEAR_IT(hspdif, SPDIFRX_IT_SYNCDIE);
	}

	/* SPDIFRX Synchronization Block detected  ---------------------------------------------*/
	if ((__HAL_SPDIFRX_GET_FLAG(hspdif, SPDIFRX_IT_SBLKIE) != RESET) &&
	    (__HAL_SPDIFRX_GET_IT_SOURCE(hspdif, SPDIFRX_IT_SBLKIE) != RESET)) {
		__HAL_SPDIFRX_CLEAR_IT(hspdif, SPDIFRX_IT_SBLKIE);
	}

	/* SPDIFRX in mode Control Flow Reception ------------------------------------------------*/
	if ((__HAL_SPDIFRX_GET_FLAG(hspdif, SPDIFRX_FLAG_CSRNE) != RESET) &&
	    (__HAL_SPDIFRX_GET_IT_SOURCE(hspdif, SPDIFRX_IT_CSRNE) != RESET)) {
	...
	if ((__HAL_SPDIFRX_GET_FLAG(hspdif, SPDIFRX_FLAG_OVR) != RESET) &&
	    (__HAL_SPDIFRX_GET_IT_SOURCE(hspdif, SPDIFRX_IT_OVRIE) != RESET)) {
	...
	if ((__HAL_SPDIFRX_GET_FLAG(hspdif, SPDIFRX_FLAG_PERR) != RESET) &&
	    (__HAL_SPDIFRX_GET_IT_SOURCE(hspdif, SPDIFRX_IT_PERRIE) != RESET)) {
...
static void SPDIFRX_DMARxCplt(DMA_HandleTypeDef *hdma)
{
	SPDIFRX_HandleTypeDef *hspdif =
		(SPDIFRX_HandleTypeDef *)((DMA_HandleTypeDef *)hdma)->Parent;
...
static void SPDIFRX_ReceiveDataFlow_IT(SPDIFRX_HandleTypeDef *hspdif)
{
	...
	if (hspdif->RxXferCount == 0) {
		...
		__HAL_SPDIFRX_DISABLE_IT(hspdif,
					 SPDIFRX_IT_OVRIE | SPDIFRX_IT_PERRIE | SPDIFRX_IT_RXNE);
