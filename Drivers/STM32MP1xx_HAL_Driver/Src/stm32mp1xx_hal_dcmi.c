...
HAL_StatusTypeDef HAL_DCMI_Init(DCMI_HandleTypeDef *hdcmi)
{
	...
	hdcmi->Instance->CR &=
		~(DCMI_CR_PCKPOL | DCMI_CR_HSPOL | DCMI_CR_VSPOL | DCMI_CR_EDM_0 | DCMI_CR_EDM_1 |
				     ...
				     uint32_t Length)
{
	...
	} else /* DCMI_DOUBLE_BUFFER Mode */
	{
		...
		if (HAL_DMAEx_MultiBufferStart_IT(hdcmi->DMA_Handle, (uint32_t)&hdcmi->Instance->DR,
						  (uint32_t)pData, SecondMemAddress,
						  hdcmi->XferSize) != HAL_OK) {
			/* Set Error Code */
			hdcmi->ErrorCode = HAL_DCMI_ERROR_DMA;
			/* Change DCMI state */
			hdcmi->State = HAL_DCMI_STATE_READY;
...
void HAL_DCMI_IRQHandler(DCMI_HandleTypeDef *hdcmi)
{
	...
	if ((isr_value & DCMI_FLAG_FRAMERI) == DCMI_FLAG_FRAMERI) {
		...
		if ((hdcmi->Instance->CR & DCMI_CR_CM) == DCMI_MODE_SNAPSHOT) {
			...
			__HAL_DCMI_DISABLE_IT(hdcmi, DCMI_IT_LINE | DCMI_IT_VSYNC | DCMI_IT_ERR |
							     DCMI_IT_OVR);
		}
...
static void DCMI_DMAXferCplt(DMA_HandleTypeDef *hdma)
{
	...
	if (hdcmi->XferCount != 0U) {
		...
		if (((hdcmi->XferCount % 2U) == 0U) && (tmp != 0U)) {
			...
			(void)HAL_DMAEx_ChangeMemory(hdcmi->DMA_Handle,
						     (tmp + (8U * hdcmi->XferSize)), MEMORY0);
			hdcmi->XferCount--;
		}
		/* Update memory 1 address location */
		else if ((((DMA_Stream_TypeDef *)(hdcmi->DMA_Handle->Instance))->CR &
			  DMA_SxCR_CT) == 0U) {
	...
	/* Update memory 1 address location */
		...
		tmp = hdcmi->pBuffPtr;
		((DMA_Stream_TypeDef *)(hdcmi->DMA_Handle->Instance))->M1AR =
			(tmp + (4U * hdcmi->XferSize));
