				     ...
				     uint32_t NumberOfBlocks, uint32_t Timeout)
{
	...
	if (hsd->State == HAL_SD_STATE_READY) {
		...
		while (!__HAL_SD_GET_FLAG(hsd, SDMMC_FLAG_TXUNDERR | SDMMC_FLAG_DCRCFAIL |
						       SDMMC_FLAG_DTIMEOUT | SDMMC_FLAG_DATAEND)) {
					...
					uint32_t NumberOfBlocks)
{
	...
	if (hsd->State == HAL_SD_STATE_READY) {
		...
		__HAL_SD_ENABLE_IT(hsd, (SDMMC_IT_DCRCFAIL | SDMMC_IT_DTIMEOUT | SDMMC_IT_TXUNDERR |
					 SDMMC_IT_DATAEND | SDMMC_FLAG_TXFIFOHE));
...
HAL_StatusTypeDef HAL_SD_GetCardCID(SD_HandleTypeDef *hsd, HAL_SD_CardCIDTypedef *pCID)
{
	...
	pCID->ProdName1 =
		(((hsd->CID[0] & 0x000000FFU) << 24U) | ((hsd->CID[1] & 0xFFFFFF00U) >> 8U));
...
static uint32_t SD_InitCard(SD_HandleTypeDef *hsd)
{
	...
	/* Select the Card */
	errorstate = SDMMC_CmdSelDesel(hsd->Instance,
				       (uint32_t)(((uint32_t)hsd->SdCard.RelCardAdd) << 16));
	if (errorstate != HAL_SD_ERROR_NONE) {
		return errorstate;
	}
...
static uint32_t SD_WideBus_Enable(SD_HandleTypeDef *hsd)
{
	...
	if ((scr[1] & SDMMC_WIDE_BUS_SUPPORT) != SDMMC_ALLZERO) {
		...
		errorstate = SDMMC_CmdAppCommand(hsd->Instance,
						 (uint32_t)(hsd->SdCard.RelCardAdd << 16));
		if (errorstate != HAL_SD_ERROR_NONE) {
			return errorstate;
		}
...
uint32_t SD_UltraHighSpeed(SD_HandleTypeDef *hsd)
{
	...
	if (hsd->SdCard.CardSpeed == CARD_ULTRA_HIGH_SPEED) {
		...
		if ((((uint8_t *)SD_hs)[13] & 2U) != 2U) {
			 ...
			 */
			MODIFY_REG(hsd->Instance->CLKCR, SDMMC_CLKCR_SELCLKRX,
...
__weak void HAL_SDEx_Write_DMADoubleBuffer0CpltCallback(SD_HandleTypeDef *hsd)
{
	...
	/* NOTE : This function should not be modified, when the callback is needed,
		  the HAL_SDEx_Write_DMADoubleBuffer0CpltCallback can be implemented in the user
	   file
	 */
}
