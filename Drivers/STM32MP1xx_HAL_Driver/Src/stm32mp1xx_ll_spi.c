	 ...
	 */
		...
		#define IS_LL_SPI_CRC(__VALUE__)                                                   \
			(((__VALUE__) == LL_SPI_CRC_4BIT) || ((__VALUE__) == LL_SPI_CRC_5BIT) ||   \
			 ((__VALUE__) == LL_SPI_CRC_6BIT) || ((__VALUE__) == LL_SPI_CRC_7BIT) ||   \
			 ((__VALUE__) == LL_SPI_CRC_8BIT) || ((__VALUE__) == LL_SPI_CRC_9BIT) ||   \
			 ((__VALUE__) == LL_SPI_CRC_10BIT) || ((__VALUE__) == LL_SPI_CRC_11BIT) || \
			 ((__VALUE__) == LL_SPI_CRC_12BIT) || ((__VALUE__) == LL_SPI_CRC_13BIT) || \
			 ((__VALUE__) == LL_SPI_CRC_14BIT) || ((__VALUE__) == LL_SPI_CRC_15BIT) || \
			 ((__VALUE__) == LL_SPI_CRC_16BIT) || ((__VALUE__) == LL_SPI_CRC_17BIT) || \
			 ((__VALUE__) == LL_SPI_CRC_18BIT) || ((__VALUE__) == LL_SPI_CRC_19BIT) || \
			 ((__VALUE__) == LL_SPI_CRC_20BIT) || ((__VALUE__) == LL_SPI_CRC_21BIT) || \
			 ((__VALUE__) == LL_SPI_CRC_22BIT) || ((__VALUE__) == LL_SPI_CRC_23BIT) || \
			 ((__VALUE__) == LL_SPI_CRC_24BIT) || ((__VALUE__) == LL_SPI_CRC_25BIT) || \
			 ((__VALUE__) == LL_SPI_CRC_26BIT) || ((__VALUE__) == LL_SPI_CRC_27BIT) || \
			 ((__VALUE__) == LL_SPI_CRC_28BIT) || ((__VALUE__) == LL_SPI_CRC_29BIT) || \
			 ((__VALUE__) == LL_SPI_CRC_30BIT) || ((__VALUE__) == LL_SPI_CRC_31BIT) || \
...
ErrorStatus LL_SPI_Init(SPI_TypeDef *SPIx, LL_SPI_InitTypeDef *SPI_InitStruct)
{
	...
	if (LL_SPI_IsEnabled(SPIx) == 0x00000000UL) {
		...
		if ((tmp_nss == LL_SPI_NSS_SOFT) &&
		    (((tmp_nss_polarity == LL_SPI_NSS_POLARITY_LOW) &&
		...
		MODIFY_REG(SPIx->CFG2,
			   SPI_CFG2_SSM | SPI_CFG2_SSOE | SPI_CFG2_CPOL | SPI_CFG2_CPHA |
		 ...
		 */
		MODIFY_REG(SPIx->CR1, SPI_CR1_HDDIR,
			   SPI_InitStruct->TransferDirection & SPI_CR1_HDDIR);
...
void LL_SPI_StructInit(LL_SPI_InitTypeDef *SPI_InitStruct)
{
	...
	SPI_InitStruct->CRCPoly = 7UL;
		...
		#define I2S_I2SCFGR_CLEAR_MASK                                                     \
			(SPI_I2SCFGR_CHLEN | SPI_I2SCFGR_DATLEN | SPI_I2SCFGR_DATFMT |             \
			 SPI_I2SCFGR_CKPOL | SPI_I2SCFGR_I2SSTD | SPI_I2SCFGR_MCKOE |              \
	 ...
	 */
		...
		#define IS_LL_I2S_BIT_ORDER(__VALUE__)                                             \
			(((__VALUE__) == LL_I2S_LSB_FIRST) || ((__VALUE__) == LL_I2S_MSB_FIRST))
/**
 * @}
 */
...
/**
 * @brief  Initializes the SPI/I2S registers according to the specified parameters in
 * I2S_InitStruct.
...
ErrorStatus LL_I2S_Init(SPI_TypeDef *SPIx, LL_I2S_InitTypeDef *I2S_InitStruct)
{
	...
	if (LL_SPI_IsEnabled(SPIx) == 0x00000000UL) {
		 ...
		 * - DataFormat     : SPI_I2SCFGR_CHLEN, SPI_I2SCFGR_DATFMT and
		 * SPI_I2SCFGR_DATLEN[1:0] bits
		 ...
		 * (i2sodd, i2sdiv) else, default values are used:  i2sodd = 0U, i2sdiv = 0U.
		 */
		if (I2S_InitStruct->AudioFreq != LL_I2S_AUDIOFREQ_DEFAULT) {
			...
			/* Compute the Real divider depending on the MCLK output state with a fixed
			 * point */
			if (I2S_InitStruct->MCLKOutput == LL_I2S_MCLK_OUTPUT_ENABLE) {
				/* MCLK output is enabled */
				tmp = (((sourceclock / (256UL >> ispcm)) * 16UL) /
				       I2S_InitStruct->AudioFreq) +
				      8UL;
			} else {
				/* MCLK output is disabled */
				tmp = (((sourceclock / ((32UL >> ispcm) * packetlength)) * 16UL) /
				       I2S_InitStruct->AudioFreq) +
				      8UL;
			}
		...
		MODIFY_REG(SPIx->I2SCFGR, SPI_I2SCFGR_ODD | SPI_I2SCFGR_I2SDIV,
...
void LL_I2S_ConfigPrescaler(SPI_TypeDef *SPIx, uint32_t PrescalerLinear, uint32_t PrescalerParity)
{
	...
	/* Write to SPIx I2SPR */
	MODIFY_REG(SPIx->I2SCFGR, SPI_I2SCFGR_I2SDIV | SPI_I2SCFGR_ODD,
