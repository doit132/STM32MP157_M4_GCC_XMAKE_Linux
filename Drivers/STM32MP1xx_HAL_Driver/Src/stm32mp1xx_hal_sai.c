...
HAL_StatusTypeDef HAL_SAI_Init(SAI_HandleTypeDef *hsai)
{
	...
	if (hsai->Init.PdmInit.Activation == ENABLE) {
		...
		/* Check that SAI sub-block is SAI1 or SAI4 sub-block A, in master RX mode with free
		 * protocol */
	#if defined(SAI4)
	...
	if (hsai->Init.AudioFrequency != SAI_AUDIO_FREQUENCY_MCKDIV) {
		...
		if (hsai->Init.NoDivider == SAI_MASTERDIVIDER_DISABLE) {
			...
			/* NODIV = 0 */
			uint32_t tmposr;
			tmposr = (hsai->Init.MckOverSampling == SAI_MCK_OVERSAMPLING_ENABLE) ? 2U
											     : 1U;
			/* (freq x 10) to keep Significant digits */
	...
	if ((hsai->Init.AudioMode == SAI_MODEMASTER_TX) ||
		...
		/* Transmit */
		ckstr_bits = (hsai->Init.ClockStrobing == SAI_CLOCKSTROBING_RISINGEDGE)
				     ? 0U
				     : SAI_xCR1_CKSTR;
	} else {
		/* Receive */
	...
	hsai->Instance->CR1 &= ~(SAI_xCR1_MODE | SAI_xCR1_PRTCFG | SAI_xCR1_DS | SAI_xCR1_LSBFIRST |
				 SAI_xCR1_CKSTR | SAI_xCR1_SYNCEN | SAI_xCR1_MONO |
				 SAI_xCR1_OUTDRIV | SAI_xCR1_DMAEN | SAI_xCR1_NODIV |
				 SAI_xCR1_MCKDIV | SAI_xCR1_OSR | SAI_xCR1_MCKEN);

	hsai->Instance->CR1 |=
		(hsai->Init.AudioMode | hsai->Init.Protocol | hsai->Init.DataSize |
		 hsai->Init.FirstBit | ckstr_bits | syncen_bits | hsai->Init.MonoStereoMode |
		 hsai->Init.OutputDrive | hsai->Init.NoDivider | (hsai->Init.Mckdiv << 20) |
		...
		while (hsai->XferCount > 0U) {
			...
			if ((hsai->Instance->SR & SAI_xSR_FLVL) != SAI_FIFOSTATUS_FULL) {
				...
				if ((((HAL_GetTick() - tickstart) > Timeout) || (Timeout == 0U)) &&
					...
					/* No need to check return value because state update,
					 * unlock and error return will be performed later */
...
HAL_StatusTypeDef HAL_SAI_DMAStop(SAI_HandleTypeDef *hsai)
{
	...
	if ((hsai->State == HAL_SAI_STATE_BUSY_TX) && (hsai->hdmatx != NULL)) {
		if (HAL_DMA_Abort(hsai->hdmatx) != HAL_OK) {
			/* If the DMA Tx errorCode is different from DMA No Transfer then return
			 * Error */
	...
	if ((hsai->State == HAL_SAI_STATE_BUSY_RX) && (hsai->hdmarx != NULL)) {
		if (HAL_DMA_Abort(hsai->hdmarx) != HAL_OK) {
			/* If the DMA Rx errorCode is different from DMA No Transfer then return
			 * Error */
...
HAL_StatusTypeDef HAL_SAI_Abort(SAI_HandleTypeDef *hsai)
{
	...
	if ((hsai->Instance->CR1 & SAI_xCR1_DMAEN) == SAI_xCR1_DMAEN) {
		...
		if ((hsai->State == HAL_SAI_STATE_BUSY_TX) && (hsai->hdmatx != NULL)) {
			if (HAL_DMA_Abort(hsai->hdmatx) != HAL_OK) {
				/* If the DMA Tx errorCode is different from DMA No Transfer then
				 * return Error */
				if (hsai->hdmatx->ErrorCode != HAL_DMA_ERROR_NO_XFER) {
					status = HAL_ERROR;
					hsai->ErrorCode |= HAL_SAI_ERROR_DMA;
		...
		if ((hsai->State == HAL_SAI_STATE_BUSY_RX) && (hsai->hdmarx != NULL)) {
			if (HAL_DMA_Abort(hsai->hdmarx) != HAL_OK) {
				/* If the DMA Rx errorCode is different from DMA No Transfer then
				 * return Error */
				if (hsai->hdmarx->ErrorCode != HAL_DMA_ERROR_NO_XFER) {
					status = HAL_ERROR;
					hsai->ErrorCode |= HAL_SAI_ERROR_DMA;
...
HAL_StatusTypeDef HAL_SAI_Transmit_DMA(SAI_HandleTypeDef *hsai, uint8_t *pData, uint16_t Size)
{
	...
	if (hsai->State == HAL_SAI_STATE_READY) {
		...
		if (HAL_DMA_Start_IT(hsai->hdmatx, (uint32_t)hsai->pBuffPtr,
				     (uint32_t)&hsai->Instance->DR, hsai->XferSize) != HAL_OK) {
			__HAL_UNLOCK(hsai);
			return HAL_ERROR;
		}
...
void HAL_SAI_IRQHandler(SAI_HandleTypeDef *hsai)
{
	if (hsai->State != HAL_SAI_STATE_RESET) {
		...
		/* SAI Overrun error interrupt occurred ----------------------------------*/
			...
			tmperror = ((hsai->State == HAL_SAI_STATE_BUSY_RX) ? HAL_SAI_ERROR_OVR
									   : HAL_SAI_ERROR_UDR);
			/* Change the SAI error code */
			hsai->ErrorCode |= tmperror;
			/* the transfer is not stopped, we will forward the information to the user
			 * and we let the user decide what needs to be done */
	#if (USE_HAL_SAI_REGISTER_CALLBACKS == 1)
		...
		/* SAI AFSDET interrupt occurred ----------------------------------*/
			...
			if ((cr1config & SAI_xCR1_DMAEN) == SAI_xCR1_DMAEN) {
				...
				/* Abort SAI */
				/* No need to check return value because HAL_SAI_ErrorCallback will
				 * be called later */
...
static void SAI_DMAError(DMA_HandleTypeDef *hdma)
{
	...
	if (HAL_DMA_GetError(hdma) != HAL_DMA_ERROR_FE) {
		...
		/* Disable SAI peripheral */
		/* No need to check return value because state will be updated and
		 * HAL_SAI_ErrorCallback will be called later */
		(void)SAI_Disable(hsai);
