 ...
 * PWR_WAKEUP_PIN5, PWR_WAKEUP_PIN6 or one of the following value where the user can explicitly
 * states the enabled pin and the chosen polarity
...
void HAL_PWR_EnableWakeUpPin(uint32_t WakeUpPinPolarity)
{
	...
	#ifdef CORE_CA7
	CLEAR_BIT(PWR->MPUWKUPENR,
		  (WakeUpPinPolarity & PWR_WAKEUP_PIN_MASK)); /* Disable WKUP pin */
	MODIFY_REG(PWR->WKUPCR, clear_mask,
		   (WakeUpPinPolarity &
		    (PWR_WKUPCR_WKUPP |
		     PWR_WKUPCR_WKUPPUPD))); /* Modify polarity and pull configuration */
	SET_BIT(PWR->WKUPCR, (WakeUpPinPolarity & PWR_WKUPCR_WKUPC)); /* Clear wake up flag */
	SET_BIT(PWR->MPUWKUPENR,
		(WakeUpPinPolarity & PWR_WAKEUP_PIN_MASK)); /* Enable the Wake up pin for CPU1 */
	#endif
	#ifdef CORE_CM4
	CLEAR_BIT(PWR->MCUWKUPENR,
		  (WakeUpPinPolarity & PWR_WAKEUP_PIN_MASK)); /* Disable WKUP pin */
	MODIFY_REG(PWR->WKUPCR, clear_mask,
		   (WakeUpPinPolarity &
		    (PWR_WKUPCR_WKUPP |
		     PWR_WKUPCR_WKUPPUPD))); /* Modify polarity and pull configuration */
	SET_BIT(PWR->WKUPCR, (WakeUpPinPolarity & PWR_WKUPCR_WKUPC)); /* Clear wake up flag */
	SET_BIT(PWR->MCUWKUPENR,
		(WakeUpPinPolarity & PWR_WAKEUP_PIN_MASK)); /* Enable the Wake up pin for CPU2 */
	#endif
}
