/**
  ...
  ******************************************************************************
  ...
  HAL_SRAM_WriteOperation_Enable()/ HAL_SRAM_WriteOperation_Disable() to respectively enable/disable
				     ...
				     uint16_t *pSrcBuffer, uint32_t BufferSize)
{
	...
	if (hsram->State == HAL_SRAM_STATE_READY) {
		...
		if (limit != 0U) {
			*psramaddress = ((uint32_t)(*psrcbuff) & 0x0000FFFFU) |
					((*psramaddress) & 0xFFFF0000U);
		}
				    ...
				    uint32_t *pDstBuffer, uint32_t BufferSize)
{
	...
	if ((state == HAL_SRAM_STATE_READY) || (state == HAL_SRAM_STATE_PROTECTED)) {
		...
		status = HAL_DMA_Start_IT(hsram->hdma, (uint32_t)pAddress, (uint32_t)pDstBuffer,
					  (uint32_t)BufferSize);
