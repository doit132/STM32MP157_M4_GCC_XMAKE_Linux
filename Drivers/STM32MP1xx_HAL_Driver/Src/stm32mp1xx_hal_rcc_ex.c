	...
	if ((pll2->PLLState) != RCC_PLL_NONE) {
		...
		if (!__IS_PLL2_IN_USE()) /* If not used then */
		{
			if ((pll2->PLLState) == RCC_PLL_ON) {
				...
				__HAL_RCC_PLL2CLKOUT_DISABLE(RCC_PLL2_DIVP | RCC_PLL2_DIVQ |
							     RCC_PLL2_DIVR);
				...
				__HAL_RCC_PLL2CLKOUT_ENABLE(RCC_PLL2_DIVP | RCC_PLL2_DIVQ |
							    RCC_PLL2_DIVR);
			} else {
				/*Disable the post-dividers*/
				__HAL_RCC_PLL2CLKOUT_DISABLE(RCC_PLL2_DIVP | RCC_PLL2_DIVQ |
	...
	if ((pll3->PLLState) != RCC_PLL_NONE) {
		...
		if (!__IS_PLL3_IN_USE()) /* If not used then*/
		{
			if ((pll3->PLLState) == RCC_PLL_ON) {
				...
				/*Disable the post-dividers*/
				__HAL_RCC_PLL3CLKOUT_DISABLE(RCC_PLL3_DIVP | RCC_PLL3_DIVQ |
							     RCC_PLL3_DIVR);
				/* Disable the main PLL. */
				...
				/* Enable the post-dividers */
				__HAL_RCC_PLL3CLKOUT_ENABLE(RCC_PLL3_DIVP | RCC_PLL3_DIVQ |
							    RCC_PLL3_DIVR);
			} else {
				/*Disable the post-dividers*/
				__HAL_RCC_PLL3CLKOUT_DISABLE(RCC_PLL3_DIVP | RCC_PLL3_DIVQ |
...
HAL_StatusTypeDef RCCEx_PLL4_Config(RCC_PLLInitTypeDef *pll4)
{
	...
	if ((pll4->PLLState) != RCC_PLL_NONE) {
		if ((pll4->PLLState) == RCC_PLL_ON) {
			...
			-Division factors (DIVMx, DIVNx, DIVPx, DIVQx & DIVRx)
			Once the PLL is enabled, these parameters can not be changed.
			If the User wants to change the PLL parameters he must disable the concerned
			PLL (PLLxON=0) and wait for the PLLxRDY flag to be at 0. The PLL
			configuration below can be done at any time: -Enable/Disable of output clock
			dividers (DIVPxEN, DIVQxEN & DIVRxEN) -Fractional Division Enable
			(PLLxFRACNEN) -Fractional Division factor (FRACNx)*/
			...
			__HAL_RCC_PLL4_CONFIG(pll4->PLLM, pll4->PLLN, pll4->PLLP, pll4->PLLQ,
					      pll4->PLLR);

			/* Configure the Fractional Divider */
			__HAL_RCC_PLL4FRACV_DISABLE(); // Set FRACLE to ‘0’
			/* In integer or clock spreading mode the application shall ensure that a 0
...
HAL_StatusTypeDef HAL_RCCEx_PeriphCLKConfig(RCC_PeriphCLKInitTypeDef *PeriphClkInit)
{
	...
	/*------------------------------ I2C12 Configuration -----------------------*/
	if (((PeriphClkInit->PeriphClockSelection) & RCC_PERIPHCLK_I2C12) == RCC_PERIPHCLK_I2C12) {
		/* Check the parameters */
	...
	/*------------------------------ I2C35 Configuration -----------------------*/
	if (((PeriphClkInit->PeriphClockSelection) & RCC_PERIPHCLK_I2C35) == RCC_PERIPHCLK_I2C35) {
		/* Check the parameters */
	...
	/*------------------------------ I2C46 Configuration -----------------------*/
	if (((PeriphClkInit->PeriphClockSelection) & RCC_PERIPHCLK_I2C46) == RCC_PERIPHCLK_I2C46) {
		/* Check the parameters */
	...
	/*---------------------------- SAI3 configuration --------------------------*/
	if (((PeriphClkInit->PeriphClockSelection) & RCC_PERIPHCLK_SAI3) == RCC_PERIPHCLK_SAI3) {
		/* Check the parameters */
	...
	/*---------------------------- SAI4 configuration --------------------------*/
	if (((PeriphClkInit->PeriphClockSelection) & RCC_PERIPHCLK_SAI4) == RCC_PERIPHCLK_SAI4) {
		/* Check the parameters */
	...
	/*---------------------------- ETH configuration ---------------------------*/
	if (((PeriphClkInit->PeriphClockSelection) & RCC_PERIPHCLK_ETH) == RCC_PERIPHCLK_ETH) {
		/* Check the parameters */
	...
	/*---------------------------- USBO configuration --------------------------*/
	if (((PeriphClkInit->PeriphClockSelection) & RCC_PERIPHCLK_USBO) == RCC_PERIPHCLK_USBO) {
		/* Check the parameters */
	...
	/*---------------------------- RNG1 configuration --------------------------*/
	if (((PeriphClkInit->PeriphClockSelection) & RCC_PERIPHCLK_RNG1) == RCC_PERIPHCLK_RNG1) {
		/* Check the parameters */
	...
	/*---------------------------- RNG2 configuration --------------------------*/
	if (((PeriphClkInit->PeriphClockSelection) & RCC_PERIPHCLK_RNG2) == RCC_PERIPHCLK_RNG2) {
		/* Check the parameters */
	...
	/*---------------------------- STGEN configuration -------------------------*/
	if (((PeriphClkInit->PeriphClockSelection) & RCC_PERIPHCLK_STGEN) == RCC_PERIPHCLK_STGEN) {
		/* Check the parameters */
	...
	if (((PeriphClkInit->PeriphClockSelection) & RCC_PERIPHCLK_RTC) == RCC_PERIPHCLK_RTC) {
		...
		if (ret == HAL_OK) {
			...
			if ((RCC->BDCR & RCC_BDCR_RTCSRC) !=
				 ...
				 * All already filled up or what shouldn't be modified must be put
				 * on the mask */
				RESERVED_BDCR_MASK =
					~(RCC_BDCR_VSWRST | RCC_BDCR_RTCCKEN | RCC_BDCR_RTCSRC |
					  RCC_BDCR_LSECSSD | RCC_BDCR_LSEDRV | RCC_BDCR_DIGBYP |
					  RCC_BDCR_LSERDY | RCC_BDCR_LSEBYP | RCC_BDCR_LSEON);

				/* Restore the BDCR context: RESERVED registers plus
				 * RCC_BDCR_LSECSSON */
				WRITE_REG(RCC->BDCR,
	...
	/*---------------------------- TIMG1 configuration -------------------------*/
	if (((PeriphClkInit->PeriphClockSelection) & RCC_PERIPHCLK_TIMG1) == RCC_PERIPHCLK_TIMG1) {
		/* Check the parameters */
	...
	/*---------------------------- TIMG2 configuration -------------------------*/
	if (((PeriphClkInit->PeriphClockSelection) & RCC_PERIPHCLK_TIMG2) == RCC_PERIPHCLK_TIMG2) {
		/* Check the parameters */
...
void HAL_RCCEx_GetPeriphCLKConfig(RCC_PeriphCLKInitTypeDef *PeriphClkInit)
{
	...
	PeriphClkInit->PeriphClockSelection =
		...
		RCC_PERIPHCLK_I2C46 | RCC_PERIPHCLK_SAI1 | RCC_PERIPHCLK_SAI2 | RCC_PERIPHCLK_SAI3 |
		RCC_PERIPHCLK_SAI4 | RCC_PERIPHCLK_SPI1 | RCC_PERIPHCLK_SPI23 |
		RCC_PERIPHCLK_SPI45 | RCC_PERIPHCLK_SPI6 | RCC_PERIPHCLK_USART1 |
		RCC_PERIPHCLK_UART24 | RCC_PERIPHCLK_UART35 | RCC_PERIPHCLK_USART6 |
		RCC_PERIPHCLK_UART78 | RCC_PERIPHCLK_SDMMC12 | RCC_PERIPHCLK_SDMMC3 |
		RCC_PERIPHCLK_ETH | RCC_PERIPHCLK_QSPI | RCC_PERIPHCLK_FMC | RCC_PERIPHCLK_FDCAN |
		RCC_PERIPHCLK_SPDIFRX | RCC_PERIPHCLK_CEC | RCC_PERIPHCLK_USBPHY |
		RCC_PERIPHCLK_USBO | RCC_PERIPHCLK_RNG1 | RCC_PERIPHCLK_RNG2 | RCC_PERIPHCLK_STGEN |
		RCC_PERIPHCLK_DSI | RCC_PERIPHCLK_ADC | RCC_PERIPHCLK_RTC | RCC_PERIPHCLK_LPTIM1 |
		RCC_PERIPHCLK_LPTIM23 | RCC_PERIPHCLK_LPTIM45;
