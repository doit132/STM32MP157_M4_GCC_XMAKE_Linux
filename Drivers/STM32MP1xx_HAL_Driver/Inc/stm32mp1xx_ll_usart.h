...
#if defined(USART1) || defined(USART2) || defined(USART3) || defined(USART6) || defined(UART4) ||  \
	defined(UART5) || defined(UART7) || defined(UART8)
...
typedef struct {
	uint32_t
		PrescalerValue; /*!< Specifies the Prescaler to compute the communication baud rate.
...
typedef struct {
	...
	#define LL_USART_CR1_RXNEIE_RXFNEIE                                                        \
		USART_CR1_RXNEIE_RXFNEIE /*!< Read data register and RXFIFO not empty interrupt    \
					    enable */
	#define LL_USART_CR1_TCIE USART_CR1_TCIE /*!< Transmission complete interrupt enable */
	#define LL_USART_CR1_TXEIE_TXFNFIE                                                         \
		USART_CR1_TXEIE_TXFNFIE /*!< Transmit data register empty and TX FIFO not full     \
					   interrupt enable */
...
{
	MODIFY_REG(USARTx->CR3, USART_CR3_TXFTCFG | USART_CR3_RXFTCFG,
					  ...
					  uint32_t BaudRate)
{
	...
	if (PrescalerValue > LL_USART_PRESCALER_DIV256) {
		...
		usartdiv = (uint16_t)(__LL_USART_DIV_SAMPLING8(PeriphClk, (uint8_t)PrescalerValue,
							       BaudRate));
...
{
	return ((READ_BIT(USARTx->ISR, USART_ISR_RXNE_RXFNE) == (USART_ISR_RXNE_RXFNE)) ? 1UL
											: 0UL);
}
