...
#ifndef STM32MP1xx_LL_WWDG_H
#define STM32MP1xx_LL_WWDG_H
 ...
 */
	 ...
	 * @brief    IT defines which can be used with LL_WWDG_ReadReg and  LL_WWDG_WriteReg
	 * functions
	 * @{
	 */
	...
	#define LL_WWDG_PRESCALER_8                                                                \
		(WWDG_CFR_WDGTB_0 | WWDG_CFR_WDGTB_1) /*!< WWDG counter clock = (PCLK1/4096)/8 */
	#define LL_WWDG_PRESCALER_16 WWDG_CFR_WDGTB_2 /*!< WWDG counter clock = (PCLK1/4096)/16 */
	#define LL_WWDG_PRESCALER_32                                                               \
		(WWDG_CFR_WDGTB_2 | WWDG_CFR_WDGTB_0) /*!< WWDG counter clock = (PCLK1/4096)/32 */
	#define LL_WWDG_PRESCALER_64                                                               \
		(WWDG_CFR_WDGTB_2 | WWDG_CFR_WDGTB_1) /*!< WWDG counter clock = (PCLK1/4096)/64 */
 ...
 * @note   When writing to the WWDG_CR register, always write 1 in the MSB b6 to avoid generating an
 * immediate reset This counter is decremented every (4096 x 2expWDGTB) PCLK cycles A reset is
 * produced when it rolls over from 0x40 to 0x3F (bit T6 becomes cleared) Setting the counter lower
 * then 0x40 causes an immediate reset (if WWDG enabled)
