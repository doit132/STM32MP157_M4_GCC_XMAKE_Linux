...
typedef struct {
	uint32_t
		SynchroMode; /*!< Specifies the Synchronization Mode: Hardware or Embedded.
	...
	DCMI_CodesInitTypeDef SyncroCode; /*!< Specifies the code of the line/frame start delimiter
					     and the line/frame end delimiter */
	...
	uint32_t ByteSelectStart; /*!< Specifies if the data to be captured by the interface is even
				     or odd This parameter can be a value of @ref
	...
	uint32_t LineSelectStart; /*!< Specifies if the line of data to be captured by the interface
				     is even or odd This parameter can be a value of @ref
...
typedef struct __DCMI_HandleTypeDef {
	...
	void (*LineEventCallback)(
		struct __DCMI_HandleTypeDef *hdcmi); /*!< DCMI Line Event callback  */
	void (*FrameEventCallback)(
		struct __DCMI_HandleTypeDef *hdcmi); /*!< DCMI Frame Event callback */
	void (*VsyncEventCallback)(
		struct __DCMI_HandleTypeDef *hdcmi);		   /*!< DCMI Vsync Event callback */
	...
	void (*MspDeInitCallback)(
		struct __DCMI_HandleTypeDef *hdcmi); /*!< DCMI Msp DeInit callback  */
 ...
 */
typedef void (*pDCMI_CallbackTypeDef)(
	DCMI_HandleTypeDef *hdcmi); /*!< pointer to a DCMI callback function */
	...
	#if (USE_HAL_DCMI_REGISTER_CALLBACKS == 1)
		#define HAL_DCMI_ERROR_INVALID_CALLBACK                                            \
			((uint32_t)0x00000080U) /*!< Invalid Callback error  */
	#endif					/* USE_HAL_DCMI_REGISTER_CALLBACKS */
	/**
	 * @}
	 */
	...
	#define DCMI_MODE_CONTINUOUS                                                               \
		(0x00000000U) /*!< The received data are transferred continuously                  \
					    into the destination memory through the DMA */
	#define DCMI_MODE_SNAPSHOT                                                                 \
		((uint32_t)DCMI_CR_CM) /*!< Once activated, the interface waits for the start of   \
					    frame and then transfers a single frame through the    \
					  DMA */
	/**
	 * @}
	 */
	...
	#define DCMI_SYNCHRO_HARDWARE                                                              \
		(0x00000000U) /*!< Hardware synchronization data capture (frame/line start/stop)   \
					    is synchronized with the HSYNC/VSYNC signals */
	#define DCMI_SYNCHRO_EMBEDDED                                                              \
		((uint32_t)DCMI_CR_ESS) /*!< Embedded synchronization data capture is synchronized \
	...
	#define DCMI_PCKPOLARITY_RISING                                                            \
		((uint32_t)DCMI_CR_PCKPOL) /*!< Pixel clock active on Rising edge  */
	...
	#define DCMI_VSPOLARITY_HIGH                                                               \
		((uint32_t)DCMI_CR_VSPOL) /*!< Vertical synchronization active High */
	...
	#define DCMI_HSPOLARITY_HIGH                                                               \
		((uint32_t)DCMI_CR_HSPOL) /*!< Horizontal synchronization active High */
	...
	#define DCMI_CR_ALTERNATE_2_FRAME                                                          \
		((uint32_t)DCMI_CR_FCRC_0) /*!< Every alternate frame captured */
	#define DCMI_CR_ALTERNATE_4_FRAME                                                          \
		((uint32_t)DCMI_CR_FCRC_1) /*!< One frame in 4 frames captured */
	 ...
	 * @{
	 */
	#define DCMI_EXTEND_DATA_8B                                                                \
		(0x00000000U) /*!< Interface captures 8-bit data on every pixel clock  */
	#define DCMI_EXTEND_DATA_10B                                                               \
		((uint32_t)DCMI_CR_EDM_0) /*!< Interface captures 10-bit data on every pixel clock \
					   */
	#define DCMI_EXTEND_DATA_12B                                                               \
		((uint32_t)DCMI_CR_EDM_1) /*!< Interface captures 12-bit data on every pixel clock \
					   */
	#define DCMI_EXTEND_DATA_14B                                                               \
		((uint32_t)(DCMI_CR_EDM_0 | DCMI_CR_EDM_1)) /*!< Interface captures 14-bit data on \
							       every pixel clock */
	...
	#define DCMI_FLAG_HSYNC                                                                    \
		((uint32_t)DCMI_SR_INDEX | DCMI_SR_HSYNC) /*!< HSYNC pin state (active line /      \
							     synchronization between lines)   */
	#define DCMI_FLAG_VSYNC                                                                    \
		((uint32_t)DCMI_SR_INDEX | DCMI_SR_VSYNC) /*!< VSYNC pin state (active frame /     \
	...
	#define DCMI_FLAG_FRAMERI                                                                  \
		((uint32_t)DCMI_RIS_FRAME_RIS) /*!< Frame capture complete interrupt flag */
	#define DCMI_FLAG_OVRRI                                                                    \
		((uint32_t)DCMI_RIS_OVR_RIS) /*!< Overrun interrupt flag                */
	#define DCMI_FLAG_ERRRI                                                                    \
		((uint32_t)DCMI_RIS_ERR_RIS) /*!< Synchronization error interrupt flag  */
	#define DCMI_FLAG_VSYNCRI                                                                  \
		((uint32_t)DCMI_RIS_VSYNC_RIS) /*!< VSYNC interrupt flag                  */
	#define DCMI_FLAG_LINERI                                                                   \
		((uint32_t)DCMI_RIS_LINE_RIS) /*!< Line interrupt flag                   */
	/**
	 * @brief   DCMI MIS register
	 */
	#define DCMI_FLAG_FRAMEMI                                                                  \
		((uint32_t)DCMI_MIS_INDEX |                                                        \
		 DCMI_MIS_FRAME_MIS) /*!< DCMI Frame capture complete masked interrupt status */
	#define DCMI_FLAG_OVRMI                                                                    \
		((uint32_t)DCMI_MIS_INDEX |                                                        \
		 DCMI_MIS_OVR_MIS) /*!< DCMI Overrun masked interrupt status                */
	#define DCMI_FLAG_ERRMI                                                                    \
		((uint32_t)DCMI_MIS_INDEX |                                                        \
		 DCMI_MIS_ERR_MIS) /*!< DCMI Synchronization error masked interrupt status  */
	#define DCMI_FLAG_VSYNCMI                                                                  \
		((uint32_t)DCMI_MIS_INDEX |                                                        \
		 DCMI_MIS_VSYNC_MIS) /*!< DCMI VSYNC masked interrupt status                  */
	#define DCMI_FLAG_LINEMI                                                                   \
		((uint32_t)DCMI_MIS_INDEX |                                                        \
		 DCMI_MIS_LINE_MIS) /*!< DCMI Line masked interrupt status                   */
	/**
	 * @}
	 */
	...
	#define DCMI_OEBS_ODD                                                                      \
		(0x00000000U) /*!< Interface captures first data from the frame/line start, second \
				 one being dropped */
	#define DCMI_OEBS_EVEN                                                                     \
		((uint32_t)DCMI_CR_OEBS) /*!< Interface captures second data from the frame/line   \
					    start, first one being dropped */
	...
	#define DCMI_OELS_ODD                                                                      \
		(0x00000000U) /*!< Interface captures first line from the frame start, second one  \
				 being dropped */
	#define DCMI_OELS_EVEN                                                                     \
		((uint32_t)DCMI_CR_OELS) /*!< Interface captures second line from the frame start, \
	 ...
	 *            @arg DCMI_FLAG_HSYNC: HSYNC pin state (active line / synchronization between
	 * lines)
	 *            @arg DCMI_FLAG_VSYNC: VSYNC pin state (active frame / synchronization between
	 * frames)
	...
	#define __HAL_DCMI_GET_FLAG(__HANDLE__, __FLAG__)                                          \
		((((__FLAG__) & (DCMI_SR_INDEX | DCMI_MIS_INDEX)) == 0x0)                          \
			 ? ((__HANDLE__)->Instance->RIS & (__FLAG__))                              \
		 : (((__FLAG__)&DCMI_SR_INDEX) == 0x0)                                             \
			 ? ((__HANDLE__)->Instance->MIS & (__FLAG__))                              \
...
HAL_StatusTypeDef HAL_DCMI_Start_DMA(DCMI_HandleTypeDef *hdcmi, uint32_t DCMI_Mode, uint32_t pData,
				     uint32_t Length);
...
uint32_t HAL_DCMI_GetError(DCMI_HandleTypeDef *hdcmi);
	...
	#define IS_DCMI_CAPTURE_MODE(MODE)                                                         \
		(((MODE) == DCMI_MODE_CONTINUOUS) || ((MODE) == DCMI_MODE_SNAPSHOT))

	#define IS_DCMI_SYNCHRO(MODE)                                                              \
		(((MODE) == DCMI_SYNCHRO_HARDWARE) || ((MODE) == DCMI_SYNCHRO_EMBEDDED))
	...
	#define IS_DCMI_MODE_JPEG(JPEG_MODE)                                                       \
		(((JPEG_MODE) == DCMI_JPEG_DISABLE) || ((JPEG_MODE) == DCMI_JPEG_ENABLE))

	#define IS_DCMI_CAPTURE_RATE(RATE)                                                         \
		(((RATE) == DCMI_CR_ALL_FRAME) || ((RATE) == DCMI_CR_ALTERNATE_2_FRAME) ||         \
		 ((RATE) == DCMI_CR_ALTERNATE_4_FRAME))

	#define IS_DCMI_EXTENDED_DATA(DATA)                                                        \
		(((DATA) == DCMI_EXTEND_DATA_8B) || ((DATA) == DCMI_EXTEND_DATA_10B) ||            \
		 ((DATA) == DCMI_EXTEND_DATA_12B) || ((DATA) == DCMI_EXTEND_DATA_14B))
