...
typedef struct {
	uint32_t BaudRate; /*!< This member configures the UART communication baud rate.
				...
				- If oversampling is 8,
			      ...
			      ((huart->Init.BaudRate)))[15:4] Baud Rate Register[3] =  0 Baud Rate
			      Register[2:0] =  (((2 * uart_ker_ckpres) /
			      ((huart->Init.BaudRate)))[3:0]) >> 1 where uart_ker_ck_pres is the
	...
	uint32_t WordLength; /*!< Specifies the number of data bits transmitted or received in a
				frame. This parameter can be a value of @ref UARTEx_Word_Length. */
	...
	uint32_t
		HwFlowCtl; /*!< Specifies whether the hardware flow control mode is enabled
				or disabled.
				This parameter can be a value of @ref UART_Hardware_Flow_Control. */

	uint32_t OverSampling; /*!< Specifies whether the Over sampling 8 is enabled or disabled, to
				  achieve higher speed (up to f_PCLK/8). This parameter can be a
				  value of @ref UART_Over_Sampling. */

	uint32_t OneBitSampling; /*!< Specifies whether a single sample or three samples' majority
	...
	uint32_t ClockPrescaler; /*!< Specifies the prescaler value used to divide the UART clock
...
typedef struct {
	...
	uint32_t AutoBaudRateMode; /*!< If auto Baud rate detection is enabled, specifies how the
				      rate detection is carried out. This parameter can be a value
				      of @ref UART_AutoBaud_Rate_Mode. */
 ...
 * @note  HAL UART State value is a combination of 2 different substates: gState and RxState (see
 * @ref UART_State_Definition).
...
typedef struct __UART_HandleTypeDef {
	...
	void (*RxISR)(
		struct __UART_HandleTypeDef *huart); /*!< Function pointer on Rx IRQ handler */

	void (*TxISR)(
		struct __UART_HandleTypeDef *huart); /*!< Function pointer on Tx IRQ handler */
...
#define HAL_UART_ERROR_RTO  ((uint32_t)0x00000020U) /*!< Receiver Timeout error  */
	...
	#define HAL_UART_ERROR_INVALID_CALLBACK                                                    \
		((uint32_t)0x00000040U) /*!< Invalid Callback error  */
#endif					/* USE_HAL_UART_REGISTER_CALLBACKS */
/**
 * @}
 */
...
#define UART_ADVFEATURE_AUTOBAUDRATE_ON0X55FRAME                                                   \
	USART_CR2_ABRMODE /*!< Auto Baud rate detection on 0x55 frame detection */
/**
 * @}
 */
...
#define UART_WAKEUP_ON_READDATA_NONEMPTY                                                           \
	USART_CR3_WUS /*!< UART wake-up on receive data register not empty or RXFIFO is not empty  \
		       */
/**
 * @}
 */
...
#define UART_DIV_SAMPLING8(__PCLK__, __BAUD__, __CLOCKPRESCALER__)                                 \
	(((((__PCLK__) / UART_GET_DIV_FACTOR((__CLOCKPRESCALER__))) * 2U) + ((__BAUD__) / 2U)) /   \
	 (__BAUD__))
...
#define UART_DIV_SAMPLING16(__PCLK__, __BAUD__, __CLOCKPRESCALER__)                                \
	((((__PCLK__) / UART_GET_DIV_FACTOR((__CLOCKPRESCALER__))) + ((__BAUD__) / 2U)) /          \
	 (__BAUD__))
...
#define IS_UART_ADVFEATURE_INIT(__INIT__)                                                          \
	((__INIT__) <= (UART_ADVFEATURE_NO_INIT | UART_ADVFEATURE_TXINVERT_INIT |                  \
