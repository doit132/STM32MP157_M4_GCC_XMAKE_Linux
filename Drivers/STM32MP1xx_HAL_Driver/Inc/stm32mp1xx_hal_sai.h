...
typedef struct {
	...
	uint32_t
		SynchroExt; /*!< Specifies SAI external output synchronization, this setup is common
	...
	uint32_t NoDivider; /*!< Specifies whether master clock will be divided or not.
				 ...
				 @note If bit NODIV in the SAI_xCR1 register is cleared, the frame
			       ...
			       256. If bit NODIV in the SAI_xCR1 register is set, the frame length
			       can take any of the values from 8 to 256. */
	...
	uint32_t
		Mckdiv; /*!< Specifies the master clock divider.
 ...
 */
typedef struct {
	uint32_t FrameLength; /*!< Specifies the Frame length, the number of SCK clocks for each
				 audio frame. This parameter must be a number between Min_Data = 8
				 and Max_Data = 256.
				   @note If master clock MCLK_x pin is declared as an output, the
				 frame length should be aligned to a number equal to power of 2 in
				 order to keep in an audio frame, an integer number of MCLK pulses
				 by bit Clock. */
...
typedef struct __SAI_HandleTypeDef {
	...
	void (*RxCpltCallback)(
		struct __SAI_HandleTypeDef *hsai); /*!< SAI receive complete callback */
	void (*RxHalfCpltCallback)(
		struct __SAI_HandleTypeDef *hsai); /*!< SAI receive half complete callback */
	void (*TxCpltCallback)(
		struct __SAI_HandleTypeDef *hsai); /*!< SAI transmit complete callback */
	...
	void (*MspDeInitCallback)(
		struct __SAI_HandleTypeDef *hsai); /*!< SAI MSP de-init callback */
#endif
} SAI_HandleTypeDef;
/**
 * @}
