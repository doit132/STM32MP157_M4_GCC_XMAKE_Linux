...
#ifndef STM32MP1xx_HAL_SPI_H
	#define STM32MP1xx_HAL_SPI_H
...
typedef struct {
	...
	uint32_t TxCRCInitializationPattern; /*!< Specifies the transmitter CRC initialization
						Pattern used for the CRC calculation. This parameter
						can be a value of
						    @ref SPI_CRC_Calculation_Initialization_Pattern
					      */

	uint32_t RxCRCInitializationPattern; /*!< Specifies the receiver CRC initialization Pattern
						used for the CRC calculation. This parameter can be
						a value of
						    @ref SPI_CRC_Calculation_Initialization_Pattern
					      */

	uint32_t MasterSSIdleness; /*!< Specifies an extra delay, expressed in number of SPI clock
				      cycle periods, inserted additionally between active edge of SS
					  and first data transaction start in master mode.
					  This parameter can be a value of @ref
				      SPI_Master_SS_Idleness */

	uint32_t MasterInterDataIdleness; /*!< Specifies minimum time delay (expressed in SPI clock
					     cycles periods) inserted between two consecutive data
					     frames in master mode. This parameter can be a value of
						 @ref SPI_Master_InterData_Idleness */

	uint32_t MasterReceiverAutoSusp; /*!< Control continuous SPI transfer in master receiver
					    mode and automatic management in order to avoid overrun
					    condition. This parameter can be a value of @ref
					    SPI_Master_RX_AutoSuspend*/

	uint32_t MasterKeepIOState; /*!< Control of Alternate function GPIOs state
					   This parameter can be a value of @ref
				       SPI_Master_Keep_IO_State */
...
typedef enum {
	...
	HAL_SPI_STATE_BUSY_TX_RX =
		0x05UL,		      /*!< Data Transmission and Reception process is ongoing */
...
typedef void (*pSPI_CallbackTypeDef)(
	...
	#define HAL_SPI_ERROR_NOT_SUPPORTED                                                        \
		(0x00000400UL)			    /*!< Requested operation not supported      */
	#define HAL_SPI_ERROR_RELOAD (0x00000800UL) /*!< Reload error                           */
	#if (USE_HAL_SPI_REGISTER_CALLBACKS == 1UL)
		#define HAL_SPI_ERROR_INVALID_CALLBACK                                             \
			(0x00001000UL) /*!< Invalid Callback error                 */
	#endif			       /* USE_HAL_SPI_REGISTER_CALLBACKS */
	/**
	 * @}
	 */
	...
	/** @defgroup SPI_CRC_Calculation_Initialization_Pattern SPI CRC Calculation Initialization
	 * Pattern
	 * @{
	 */
	...
	#define SPI_FLAG_TSERF                                                                     \
		SPI_SR_TSERF /* SPI status flag : Additional number of data reloaded flag  */
	#define SPI_FLAG_SUSP                                                                      \
		SPI_SR_SUSP /* SPI status flag : Transfer suspend complete flag           */
	#define SPI_FLAG_TXC                                                                       \
		SPI_SR_TXC /* SPI status flag : TxFIFO transmission complete flag        */
	#define SPI_FLAG_FRLVL                                                                     \
		SPI_SR_RXPLVL /* SPI status flag : Fifo reception level flag                */
	#define SPI_FLAG_RXWNE                                                                     \
		SPI_SR_RXWNE /* SPI status flag : RxFIFO word not empty flag               */
	/**
	 * @}
	 */
	 ...
	 *         This parameter can be SPI where x: 1, 2, 3, 4, 5 or 6 to select the SPI
	 * peripheral.
	 * @retval None
	 */
	#if (USE_HAL_SPI_REGISTER_CALLBACKS == 1UL)
	...
	#define __HAL_SPI_CLEAR_CRCERRFLAG(__HANDLE__)                                             \
		SET_BIT((__HANDLE__)->Instance->IFCR, SPI_IFCR_CRCEC)
	...
	#define __HAL_SPI_CLEAR_OVRFLAG(__HANDLE__)                                                \
		SET_BIT((__HANDLE__)->Instance->IFCR, SPI_IFCR_OVRC)
	...
	#define __HAL_SPI_CLEAR_FREFLAG(__HANDLE__)                                                \
		SET_BIT((__HANDLE__)->Instance->IFCR, SPI_IFCR_TIFREC)
	...
	#define __HAL_SPI_CLEAR_UDRFLAG(__HANDLE__)                                                \
		SET_BIT((__HANDLE__)->Instance->IFCR, SPI_IFCR_UDRC)

	/** @brief  Clear the SPI EOT pending flag.
	 * @param  __HANDLE__: specifies the SPI Handle.
	 * @retval None
	 */
	#define __HAL_SPI_CLEAR_EOTFLAG(__HANDLE__)                                                \
		SET_BIT((__HANDLE__)->Instance->IFCR, SPI_IFCR_EOTC)

	/** @brief  Clear the SPI UDR pending flag.
	 * @param  __HANDLE__: specifies the SPI Handle.
	 * @retval None
	 */
	#define __HAL_SPI_CLEAR_TXTFFLAG(__HANDLE__)                                               \
		SET_BIT((__HANDLE__)->Instance->IFCR, SPI_IFCR_TXTFC)

	/** @brief  Clear the SPI SUSP pending flag.
	 * @param  __HANDLE__: specifies the SPI Handle.
	 * @retval None
	 */
	#define __HAL_SPI_CLEAR_SUSPFLAG(__HANDLE__)                                               \
		SET_BIT((__HANDLE__)->Instance->IFCR, SPI_IFCR_SUSPC)

	/** @brief  Clear the SPI TSERF pending flag.
	 * @param  __HANDLE__: specifies the SPI Handle.
	 * @retval None
	 */
	#define __HAL_SPI_CLEAR_TSERFFLAG(__HANDLE__)                                              \
		SET_BIT((__HANDLE__)->Instance->IFCR, SPI_IFCR_TSERFC)
	...
	#include "stm32mp1xx_hal_spi_ex.h"
...
HAL_StatusTypeDef HAL_SPI_TransmitReceive_DMA(SPI_HandleTypeDef *hspi, const uint8_t *pTxData,
	...
	#if defined(USE_SPI_RELOAD_TRANSFER)
HAL_StatusTypeDef HAL_SPI_Reload_Transmit_IT(SPI_HandleTypeDef *hspi, const uint8_t *pData,
					     uint16_t Size);
...
uint32_t HAL_SPI_GetError(const SPI_HandleTypeDef *hspi);
	 ...
	 * @retval None
	 */
	#define SPI_2LINES_TX(__HANDLE__)                                                          \
		MODIFY_REG((__HANDLE__)->Instance->CFG2, SPI_CFG2_COMM, SPI_CFG2_COMM_0)
	 ...
	 * @retval None
	 */
	#define SPI_2LINES_RX(__HANDLE__)                                                          \
		MODIFY_REG((__HANDLE__)->Instance->CFG2, SPI_CFG2_COMM, SPI_CFG2_COMM_1)
	 ...
	 * @retval None
	 */
	#define SPI_2LINES(__HANDLE__)                                                             \
		MODIFY_REG((__HANDLE__)->Instance->CFG2, SPI_CFG2_COMM, 0x00000000UL)
	...
	#define IS_SPI_NSS(NSS)                                                                    \
		(((NSS) == SPI_NSS_SOFT) || ((NSS) == SPI_NSS_HARD_INPUT) ||                       \
		 ((NSS) == SPI_NSS_HARD_OUTPUT))

	#define IS_SPI_NSSP(NSSP)                                                                  \
		(((NSSP) == SPI_NSS_PULSE_ENABLE) || ((NSSP) == SPI_NSS_PULSE_DISABLE))
