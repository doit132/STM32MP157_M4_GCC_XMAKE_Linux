...
typedef enum {
	...
	HAL_HASH_PHASE_HMAC_STEP_1 =
		0x03U,			    /*!< HASH peripheral is in HMAC step 1 processing phase
	...
	HAL_HASH_PHASE_HMAC_STEP_3 =
		0x05U /*!< HASH peripheral is in HMAC step 3 processing phase
	...
	#endif /* (USE_HAL_HASH_REGISTER_CALLBACKS) */
{
	...
	FlagStatus DigestCalculationDisable; /*!< Digest calculation phase skip (MDMAT bit control)
						for multi-buffers DMA-based HMAC computation */

	__IO uint32_t NbWordsAlreadyPushed; /*!< Numbers of words already pushed in FIFO before
					       inputting new block */
	...
	#if (USE_HAL_HASH_REGISTER_CALLBACKS == 1)
	void (*InCpltCallback)(
		struct __HASH_HandleTypeDef *hhash); /*!< HASH input completion callback */

	void (*DgstCpltCallback)(struct __HASH_HandleTypeDef *
					 hhash); /*!< HASH digest computation completion callback */
	...
	void (*MspDeInitCallback)(
		struct __HASH_HandleTypeDef *hhash); /*!< HASH Msp DeInit callback */
...
typedef void (*pHASH_CallbackTypeDef)(
	 ...
	 */
	#define HASH_IT_DINI                                                                       \
		HASH_IMR_DINIE /*!< A new block can be entered into the input buffer (DIN) */
	#define HASH_IT_DCI                                                                        \
	...
	#define HAL_HASHEx_IRQHandler                                                              \
		HAL_HASH_IRQHandler /*!< HAL_HASHEx_IRQHandler() is re-directed to                 \
				       HAL_HASH_IRQHandler() for compatibility with legacy code */
	/**
	 * @}
	 */
	 ...
	 *            @arg @ref HASH_FLAG_DMAS DMA interface is enabled (DMAE=1) or a transfer is
	 * ongoing.
	 *            @arg @ref HASH_FLAG_BUSY The hash core is Busy : processing a block of data.
	 *            @arg @ref HASH_FLAG_DINNE DIN not empty : the input buffer contains at least
	 * one word of data.
	...
	#define HASH_DIGEST_LENGTH()                                                               \
		((READ_BIT(HASH->CR, HASH_CR_ALGO) == HASH_ALGOSELECTION_SHA1)                     \
			 ? 20U                                                                     \
			 : ((READ_BIT(HASH->CR, HASH_CR_ALGO) == HASH_ALGOSELECTION_SHA224)        \
				    ? 28U                                                          \
				    : ((READ_BIT(HASH->CR, HASH_CR_ALGO) ==                        \
	...
	#define IS_HASH_DATATYPE(__DATATYPE__)                                                     \
		(((__DATATYPE__) == HASH_DATATYPE_32B) || ((__DATATYPE__) == HASH_DATATYPE_16B) || \
		 ((__DATATYPE__) == HASH_DATATYPE_8B) || ((__DATATYPE__) == HASH_DATATYPE_1B))
	...
	#define IS_HMAC_DMA_MULTIBUFFER_SIZE(__HANDLE__, __SIZE__)                                 \
		((((__HANDLE__)->DigestCalculationDisable) == RESET) || (((__SIZE__) % 4U) == 0U))
	/**
	 * @brief Ensure that handle phase is set to HASH processing.
	 * @param __HANDLE__: HASH handle.
	 * @retval SET (handle phase is set to HASH processing) or RESET (handle phase is not set to
	 * HASH processing)
	 */
	 ...
	 * @param __HANDLE__: HASH handle.
	 * @retval SET (handle phase is set to HMAC processing) or RESET (handle phase is not set to
	 * HMAC processing)
	 */
