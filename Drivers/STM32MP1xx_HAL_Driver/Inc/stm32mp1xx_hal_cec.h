...
typedef struct {
	...
	uint32_t Tolerance; /*!< Set RXTOL bit, specifies the tolerance accepted on the received
			       waveforms, it can be a value of @ref CEC_Tolerance : it is either
			       CEC_STANDARD_TOLERANCE or CEC_EXTENDED_TOLERANCE */

	uint32_t BRERxStop; /*!< Set BRESTP bit @ref CEC_BRERxStop : specifies whether or not a Bit
			       Rising Error stops the reception. CEC_NO_RX_STOP_ON_BRE: reception is
			       not stopped. CEC_RX_STOP_ON_BRE:    reception is stopped. */

	uint32_t BREErrorBitGen; /*!< Set BREGEN bit @ref CEC_BREErrorBitGen : specifies whether or
				    not an Error-Bit is generated on the CEC line upon Bit Rising
				    Error detection. CEC_BRE_ERRORBIT_NO_GENERATION: no error-bit
				    generation. CEC_BRE_ERRORBIT_GENERATION:    error-bit generation
				    if BRESTP is set. */

	uint32_t LBPEErrorBitGen; /*!< Set LBPEGEN bit @ref CEC_LBPEErrorBitGen : specifies whether
				     or not an Error-Bit is generated on the CEC line upon Long Bit
				     Period Error detection. CEC_LBPE_ERRORBIT_NO_GENERATION:  no
				     error-bit generation. CEC_LBPE_ERRORBIT_GENERATION: error-bit
				     generation. */

	uint32_t
		BroadcastMsgNoErrorBitGen; /*!< Set BRDNOGEN bit @ref CEC_BroadCastMsgErrorBitGen :
					      allows to avoid an Error-Bit generation on the CEC
					      line upon an error detected on a broadcast message.

						It supersedes BREGEN and LBPEGEN bits for a
					      broadcast message error handling. It can take two
					      values:

						1) CEC_BROADCASTERROR_ERRORBIT_GENERATION.
						   a) BRE detection: error-bit generation on the CEC
					      line if BRESTP=CEC_RX_STOP_ON_BRE and
					      BREGEN=CEC_BRE_ERRORBIT_NO_GENERATION. b) LBPE
					      detection: error-bit generation on the CEC line if
					      LBPGEN=CEC_LBPE_ERRORBIT_NO_GENERATION.

						2) CEC_BROADCASTERROR_NO_ERRORBIT_GENERATION.
						   no error-bit generation in case neither a) nor b)
					      are satisfied. Additionally, there is no error-bit
					      generation in case of Short Bit Period Error detection
					      in a broadcast message while LSTN bit is set. */

	uint32_t SignalFreeTimeOption; /*!< Set SFTOP bit @ref CEC_SFT_Option : specifies when SFT
					  timer starts. CEC_SFT_START_ON_TXSOM SFT:    timer starts
					  when TXSOM is set by software. CEC_SFT_START_ON_TX_RX_END:
					  SFT timer starts automatically at the end of message
					  transmission/reception. */

	uint32_t ListenMode; /*!< Set LSTN bit @ref CEC_Listening_Mode : specifies device listening
				mode. It can take two values:

				  CEC_REDUCED_LISTENING_MODE: CEC peripheral receives only message
				addressed to its own address (OAR). Messages addressed to different
				destination are ignored. Broadcast messages are always received.

				  CEC_FULL_LISTENING_MODE: CEC peripheral receives messages
				addressed to its own address (OAR) with positive acknowledge.
				Messages addressed to different destination are received, but
 ...
 * @note  HAL CEC State value is a combination of 2 different substates: gState and RxState (see
 * @ref CEC_State_Definition).
	...
	#endif /* USE_HAL_CEC_REGISTER_CALLBACKS */
{
	...
	HAL_CEC_StateTypeDef
		gState; /*!< CEC state information related to global Handle management
			     and also related to Tx operations.
			     This parameter can be a value of @ref HAL_CEC_StateTypeDef */

	HAL_CEC_StateTypeDef
		RxState; /*!< CEC state information related to Rx operations.
			      This parameter can be a value of @ref HAL_CEC_StateTypeDef */
	...
	#if (USE_HAL_CEC_REGISTER_CALLBACKS == 1)
	void (*TxCpltCallback)(
		struct __CEC_HandleTypeDef *hcec); /*!< CEC Tx Transfer completed callback */
 ...
 */
typedef void (*pCEC_CallbackTypeDef)(
	CEC_HandleTypeDef *hcec); /*!< pointer to an CEC callback function */
typedef void (*pCEC_RxCallbackTypeDef)(
	...
	#define HAL_CEC_STATE_RESET                                                                \
		((uint32_t)0x00000000) /*!< Peripheral is not yet Initialized                      \
					    Value is allowed for gState and RxState  */
	#define HAL_CEC_STATE_READY                                                                \
		((uint32_t)0x00000020) /*!< Peripheral Initialized and ready for use               \
					    Value is allowed for gState and RxState  */
	#define HAL_CEC_STATE_BUSY                                                                 \
		((uint32_t)0x00000024) /*!< an internal process is ongoing                         \
					    Value is allowed for gState only         */
	#define HAL_CEC_STATE_BUSY_RX                                                              \
		((uint32_t)0x00000022) /*!< Data Reception process is ongoing                      \
					    Value is allowed for RxState only        */
	#define HAL_CEC_STATE_BUSY_TX                                                              \
		((uint32_t)0x00000021) /*!< Data Transmission process is ongoing                   \
					    Value is allowed for gState only         */
	#define HAL_CEC_STATE_BUSY_RX_TX                                                           \
		((uint32_t)0x00000023) /*!< an internal process is ongoing                         \
					    Value is allowed for gState only         */
	#define HAL_CEC_STATE_ERROR                                                                \
		((uint32_t)0x00000050) /*!< Error Value is allowed for gState only   */
	/**
	 * @}
	 */
	...
	#if (USE_HAL_CEC_REGISTER_CALLBACKS == 1)
		#define HAL_CEC_ERROR_INVALID_CALLBACK                                             \
			((uint32_t)0x00002000U) /*!< Invalid Callback Error  */
	#endif					/* USE_HAL_CEC_REGISTER_CALLBACKS */
	/**
	 * @}
	 */
	...
	/** @defgroup CEC_LBPEErrorBitGen  CEC Error Bit Generation if Long Bit Period Error
	 * reported
	 * @{
	 */
	...
	/** @defgroup CEC_Initiator_Position   CEC Initiator logical address position in message
	 * header
	 * @{
	 */
	...
	#define CEC_ISR_ALL_ERROR                                                                  \
		((uint32_t)CEC_ISR_RXOVR | CEC_ISR_BRE | CEC_ISR_SBPE | CEC_ISR_LBPE |             \
		 CEC_ISR_RXACKE | CEC_ISR_ARBLST | CEC_ISR_TXUDR | CEC_ISR_TXERR | CEC_ISR_TXACKE)
	/**
	 * @}
	 */
	...
	#define CEC_IER_TX_ALL_ERR                                                                 \
		((uint32_t)CEC_IER_TXACKEIE | CEC_IER_TXERRIE | CEC_IER_TXUDRIE | CEC_IER_ARBLSTIE)
	/**
	 * @}
	 */
	 ...
	 *            @arg CEC_FLAG_TXEND: End of transmission (successful transmission of the last
	 * byte).
	...
	/** @brief  Set OAR register (without resetting previously set address in case of
	 * multi-address mode) To reset OAR, __HAL_CEC_CLEAR_OAR() needs to be called beforehand
	 * @param  __HANDLE__ specifies the CEC Handle.
...
uint32_t HAL_CEC_GetError(CEC_HandleTypeDef *hcec);
	...
	#define IS_CEC_TOLERANCE(__RXTOL__)                                                        \
		(((__RXTOL__) == CEC_STANDARD_TOLERANCE) || ((__RXTOL__) == CEC_EXTENDED_TOLERANCE))
