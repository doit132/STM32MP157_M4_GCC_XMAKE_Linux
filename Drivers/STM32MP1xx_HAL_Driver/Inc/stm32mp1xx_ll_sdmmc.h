...
typedef struct {
	...
	uint32_t HardwareFlowControl; /*!< Specifies whether the SDMMC hardware flow control is
					 enabled or disabled. This parameter can be a value of @ref
					 SDMMC_LL_Hardware_Flow_Control      */

	uint32_t ClockDiv; /*!< Specifies the clock frequency of the SDMMC controller.
				This parameter can be a value between Min_Data = 0 and Max_Data =
			      1023   */
...
#define SDMMC_ERROR_CC_ERR                                                                         \
	((uint32_t)0x00008000U) /*!< Internal card controller error */
...
#define SDMMC_CMD_SEND_OP_COND                                                                     \
	((uint8_t)1U) /*!< Sends host capacity support information and activates the card's        \
			 initialization process. */
#define SDMMC_CMD_ALL_SEND_CID                                                                     \
	((uint8_t)2U) /*!< Asks any card connected to the host to send the CID numbers on the CMD  \
			 line.             */
...
#define SDMMC_CMD_SDMMC_SEN_OP_COND                                                                \
	((uint8_t)5U) /*!< Sends host capacity support information (HCS) and asks the accessed     \
...
#define SDMMC_CMD_SEL_DESEL_CARD                                                                   \
	((uint8_t)7U) /*!< Selects the card by its own relative address and gets deselected by any \
			 other address    */
#define SDMMC_CMD_HS_SEND_EXT_CSD                                                                  \
	((uint8_t)8U) /*!< Sends SD Memory Card interface condition, which includes host supply    \
...
#define SDMMC_CMD_VOLTAGE_SWITCH                                                                   \
	((uint8_t)11U) /*!< SD card Voltage switch to 1.8V mode. */
...
#define SDMMC_CMD_SET_BLOCKLEN                                                                     \
	((uint8_t)16U) /*!< Sets the block length (in bytes for SDSC) for all following block      \
			commands (read, write, lock). Default block length is fixed to 512 Bytes.                                  \
			Not effective for SDHS and SDXC. */
#define SDMMC_CMD_READ_SINGLE_BLOCK                                                                \
	((uint8_t)17U) /*!< Reads single block of size selected by SET_BLOCKLEN in case of SDSC,   \
			and a block of fixed 512 bytes in case of SDHC and SDXC. */
#define SDMMC_CMD_READ_MULT_BLOCK                                                                  \
	((uint8_t)18U) /*!< Continuously transfers data blocks from card to host until interrupted \
			by STOP_TRANSMISSION command. */
...
#define SDMMC_CMD_WRITE_SINGLE_BLOCK                                                               \
	((uint8_t)24U) /*!< Writes single block of size selected by SET_BLOCKLEN in case of SDSC,  \
			and a block of fixed 512 bytes in case of SDHC and SDXC. */
...
#define SDMMC_CMD_SD_ERASE_GRP_START                                                               \
	((uint8_t)32U) /*!< Sets the address of the first write block to be erased. (For SD card   \
			  only).              */
#define SDMMC_CMD_SD_ERASE_GRP_END                                                                 \
	((uint8_t)33U) /*!< Sets the address of the last write block of the continuous range to be \
			  erased.           */
#define SDMMC_CMD_ERASE_GRP_START                                                                  \
	((uint8_t)35U) /*!< Sets the address of the first write block to be erased. Reserved for   \
			each command system set by switch function command (CMD6). */
#define SDMMC_CMD_ERASE_GRP_END                                                                    \
	((uint8_t)36U) /*!< Sets the address of the last write block of the continuous range to be \
			erased. Reserved for each command system set by switch function command                                   \
			(CMD6).                      */
...
#define SDMMC_CMD_LOCK_UNLOCK                                                                      \
	((uint8_t)42U) /*!< Sets/resets the password or lock/unlock the card. The size of the data \
			block is set by the SET_BLOCK_LEN command. */
#define SDMMC_CMD_APP_CMD                                                                          \
	((uint8_t)55U) /*!< Indicates to the card that the next command is an application specific \
			command rather than a standard command. */
#define SDMMC_CMD_GEN_CMD                                                                          \
	((uint8_t)56U) /*!< Used either to transfer a data block to the card or to get a data      \
			block from the card for general purpose/application specific commands. */
...
#define SDMMC_CMD_APP_SD_SET_BUSWIDTH                                                              \
	((uint8_t)6U) /*!< (ACMD6) Defines the data bus width to be used for data transfer. The    \
...
#define SDMMC_CMD_SD_APP_SEND_NUM_WRITE_BLOCKS                                                     \
	((uint8_t)22U) /*!< (ACMD22) Sends the number of the written (without errors) write        \
			blocks. Responds with 32bit+CRC data block. */
#define SDMMC_CMD_SD_APP_OP_COND                                                                   \
	((uint8_t)41U) /*!< (ACMD41) Sends host capacity support information (HCS) and asks the    \
			accessed card to send its operating condition register (OCR) content in                                            \
			the response on the CMD line.       */
#define SDMMC_CMD_SD_APP_SET_CLR_CARD_DETECT                                                       \
	((uint8_t)42U) /*!< (ACMD42) Connect/Disconnect the 50 KOhm pull-up resistor on CD/DAT3    \
			  (pin 1) of the card  */
...
#define IS_SDMMC_CLOCK_EDGE(EDGE)                                                                  \
	(((EDGE) == SDMMC_CLOCK_EDGE_RISING) || ((EDGE) == SDMMC_CLOCK_EDGE_FALLING))
/**
 * @}
 */
...
#define IS_SDMMC_CLOCK_POWER_SAVE(SAVE)                                                            \
	(((SAVE) == SDMMC_CLOCK_POWER_SAVE_DISABLE) || ((SAVE) == SDMMC_CLOCK_POWER_SAVE_ENABLE))
/**
 * @}
 */
...
#define IS_SDMMC_BUS_WIDE(WIDE)                                                                    \
	(((WIDE) == SDMMC_BUS_WIDE_1B) || ((WIDE) == SDMMC_BUS_WIDE_4B) ||                         \
...
#define IS_SDMMC_RESPONSE(RESPONSE)                                                                \
	(((RESPONSE) == SDMMC_RESPONSE_NO) || ((RESPONSE) == SDMMC_RESPONSE_SHORT) ||              \
...
#define IS_SDMMC_WAIT(WAIT)                                                                        \
	(((WAIT) == SDMMC_WAIT_NO) || ((WAIT) == SDMMC_WAIT_IT) || ((WAIT) == SDMMC_WAIT_PEND))
/**
 * @}
 */
...
#define IS_SDMMC_RESP(RESP)                                                                        \
	(((RESP) == SDMMC_RESP1) || ((RESP) == SDMMC_RESP2) || ((RESP) == SDMMC_RESP3) ||          \
	 ((RESP) == SDMMC_RESP4))
...
#define SDMMC_ENABLE_IDMA_DOUBLE_BUFF1                                                             \
	(SDMMC_IDMA_IDMAEN | SDMMC_IDMA_IDMABMODE | SDMMC_IDMA_IDMABACT)
...
#define SDMMC_DATABLOCK_SIZE_2048B                                                                 \
	(SDMMC_DCTRL_DBLOCKSIZE_0 | SDMMC_DCTRL_DBLOCKSIZE_1 | SDMMC_DCTRL_DBLOCKSIZE_3)
#define SDMMC_DATABLOCK_SIZE_4096B (SDMMC_DCTRL_DBLOCKSIZE_2 | SDMMC_DCTRL_DBLOCKSIZE_3)
#define SDMMC_DATABLOCK_SIZE_8192B                                                                 \
	(SDMMC_DCTRL_DBLOCKSIZE_0 | SDMMC_DCTRL_DBLOCKSIZE_2 | SDMMC_DCTRL_DBLOCKSIZE_3)
...
#define IS_SDMMC_BLOCK_SIZE(SIZE)                                                                  \
	(((SIZE) == SDMMC_DATABLOCK_SIZE_1B) || ((SIZE) == SDMMC_DATABLOCK_SIZE_2B) ||             \
...
#define IS_SDMMC_TRANSFER_DIR(DIR)                                                                 \
	(((DIR) == SDMMC_TRANSFER_DIR_TO_CARD) || ((DIR) == SDMMC_TRANSFER_DIR_TO_SDMMC))
/**
 * @}
 */
...
#define IS_SDMMC_TRANSFER_MODE(MODE)                                                               \
	(((MODE) == SDMMC_TRANSFER_MODE_BLOCK) || ((MODE) == SDMMC_TRANSFER_MODE_STREAM))
/**
 * @}
 */
...
#define IS_SDMMC_READWAIT_MODE(MODE)                                                               \
	(((MODE) == SDMMC_READ_WAIT_MODE_CLK) || ((MODE) == SDMMC_READ_WAIT_MODE_DATA2))
/**
 * @}
 */
...
#define CLKCR_CLEAR_MASK                                                                           \
	((uint32_t)(SDMMC_CLKCR_CLKDIV | SDMMC_CLKCR_PWRSAV | SDMMC_CLKCR_WIDBUS |                 \
		    SDMMC_CLKCR_NEGEDGE | SDMMC_CLKCR_HWFC_EN))

/* --- DCTRL Register ---*/
/* SDMMC DCTRL Clear Mask */
#define DCTRL_CLEAR_MASK                                                                           \
	((uint32_t)(SDMMC_DCTRL_DTEN | SDMMC_DCTRL_DTDIR | SDMMC_DCTRL_DTMODE |                    \
		    SDMMC_DCTRL_DBLOCKSIZE))

/* --- CMD Register ---*/
/* CMD Register clear mask */
#define CMD_CLEAR_MASK                                                                             \
	((uint32_t)(SDMMC_CMD_CMDINDEX | SDMMC_CMD_WAITRESP | SDMMC_CMD_WAITINT |                  \
		    SDMMC_CMD_WAITPEND | SDMMC_CMD_CPSMEN | SDMMC_CMD_CMDSUSPEND))
