...
typedef struct {
	...
	uint32_t
		TransferTriggerMode; /*!< Specifies the Trigger Transfer mode : each request
					triggers a : a buffer transfer, a block transfer, a repeated
					block transfer or a linked list transfer This parameter can
					be a value of @ref MDMA_Transfer_TriggerMode  */
	...
	uint32_t
		DataAlignment; /*!< Specifies the source to destination Memory data packing/padding
				  mode. This parameter can be a value of @ref MDMA_data_Alignment */

	uint32_t BufferTransferLength; /*!< Specifies the buffer Transfer Length (number of bytes),
					   this is the number of bytes to be transferred in a single
					  transfer (1 byte to 128 bytes)*/

	uint32_t SourceBurst; /*!< Specifies the Burst transfer configuration for the source memory
				 transfers. It specifies the amount of data to be transferred in a
				 single non interruptable transaction. This parameter can be a value
				 of @ref MDMA_Source_burst
				  @note : the burst may be FIXED/INCR based on SourceInc value ,
				  the BURST must be programmed as to ensure that the burst size will
				 be lower than than BufferTransferLength */

	uint32_t DestBurst; /*!< Specifies the Burst transfer configuration for the destination
			       memory transfers. It specifies the amount of data to be transferred
			       in a single non interruptable transaction. This parameter can be a
			       value of @ref MDMA_Destination_burst
				 @note : the burst may be FIXED/INCR based on DestinationInc value ,
				 the BURST must be programmed as to ensure that the burst size will
			       be lower than than BufferTransferLength */

	int32_t SourceBlockAddressOffset; /*!< this field specifies the Next block source address
					     offset signed value : if > 0 then  increment the next
					     block source Address by offset from where the last
					     block ends if < 0 then  decrement the next block source
					     Address by offset from where the last block ends if ==
					     0, the next block source address starts from where the
					     last block ends
					   */

	int32_t DestBlockAddressOffset; /*!< this field specifies the Next block destination address
					   offset signed value : if > 0 then  increment the next
					   block destination Address by offset from where the last
					   block ends if < 0 then  decrement the next block
					   destination Address by offset from where the last block
					   ends if == 0, the next block destination address starts
 ...
 *         a new configuration (linked list node) is automatically loaded from the address given in
 * CLAR register.
 */
typedef struct {
	__IO uint32_t
		CTCR; /*!< New CTCR register configuration for the given MDMA linked list node   */
	__IO uint32_t CBNDTR; /*!< New CBNDTR register configuration for the given MDMA linked list
				 node */
	__IO uint32_t
		CSAR; /*!< New CSAR register configuration for the given MDMA linked list node   */
	__IO uint32_t
		CDAR; /*!< New CDAR register configuration for the given MDMA linked list node   */
	__IO uint32_t
		CBRUR; /*!< New CBRUR register configuration for the given MDMA linked list node  */
	__IO uint32_t
		CLAR; /*!< New CLAR register configuration for the given MDMA linked list node   */
	__IO uint32_t
		CTBR; /*!< New CTBR register configuration for the given MDMA linked list node   */
	__IO uint32_t Reserved; /*!< Reserved register */
	__IO uint32_t
		CMAR; /*!< New CMAR register configuration for the given MDMA linked list node   */
	__IO uint32_t
		CMDR; /*!< New CMDR register configuration for the given MDMA linked list node   */
...
typedef struct {
	...
	uint32_t PostRequestMaskAddress; /*!< specifies the address to be updated (written) with
					    PostRequestMaskData after a request is served.
					      PostRequestMaskAddress and PostRequestMaskData could
					    be used to automatically clear a peripheral flag when
					    the request is served  */

	uint32_t PostRequestMaskData; /*!< specifies the value to be written to
					 PostRequestMaskAddress after a request is served.
					   PostRequestMaskAddress and PostRequestMaskData could be
					 used to automatically clear a peripheral flag when the
					 request is served  */
...
typedef struct __MDMA_HandleTypeDef {
	...
	void (*XferCpltCallback)(
		struct __MDMA_HandleTypeDef *hmdma); /*!< MDMA transfer complete callback        */

	void (*XferBufferCpltCallback)(
		struct __MDMA_HandleTypeDef *hmdma); /*!< MDMA buffer transfer complete callback */

	void (*XferBlockCpltCallback)(
		struct __MDMA_HandleTypeDef *hmdma); /*!< MDMA block transfer complete callback  */

	void (*XferRepeatBlockCpltCallback)(
		struct __MDMA_HandleTypeDef *hmdma); /*!< MDMA block transfer repeat callback    */

	void (*XferErrorCallback)(
		struct __MDMA_HandleTypeDef *hmdma); /*!< MDMA transfer error callback           */

	void (*XferAbortCallback)(
		struct __MDMA_HandleTypeDef *hmdma); /*!< MDMA transfer Abort callback           */

	MDMA_LinkNodeTypeDef
		*FirstLinkedListNodeAddress; /*!< specifies the first node address of the transfer
						list (after the initial node defined by the Init
						struct) this parameter is used internally by the
						MDMA driver to construct the linked list node
					     */

	MDMA_LinkNodeTypeDef
		*LastLinkedListNodeAddress; /*!< specifies the last node address of the transfer
					       list this parameter is used internally by the MDMA
					       driver to construct the linked list node
					     */
...
#define HAL_MDMA_ERROR_READ_XFER                                                                   \
	((uint32_t)0x00000001U) /*!< Read Transfer error                    */
#define HAL_MDMA_ERROR_WRITE_XFER                                                                  \
	((uint32_t)0x00000002U) /*!< Write Transfer error                   */
#define HAL_MDMA_ERROR_MASK_DATA                                                                   \
	((uint32_t)0x00000004U) /*!< Error Mask Data error                  */
#define HAL_MDMA_ERROR_LINKED_LIST                                                                 \
	((uint32_t)0x00000008U) /*!< Linked list Data error                 */
#define HAL_MDMA_ERROR_ALIGNMENT                                                                   \
	((uint32_t)0x00000010U) /*!< Address/Size alignment  error          */
#define HAL_MDMA_ERROR_BLOCK_SIZE                                                                  \
	((uint32_t)0x00000020U) /*!< Block Size error                       */
#define HAL_MDMA_ERROR_TIMEOUT                                                                     \
	((uint32_t)0x00000040U) /*!< Timeout error                          */
#define HAL_MDMA_ERROR_NO_XFER                                                                     \
	((uint32_t)0x00000080U) /*!< Abort or SW trigger requested with no Xfer ongoing   */
#define HAL_MDMA_ERROR_BUSY                                                                        \
	((uint32_t)0x00000100U) /*!< DeInit or SW trigger requested with Xfer ongoing   */
...
#define MDMA_REQUEST_DMA1_Stream0_TC                                                               \
	((uint32_t)0x00000000U) /*!< MDMA HW request is DMA1 Stream 0 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA1_Stream1_TC                                                               \
	((uint32_t)0x00000001U) /*!< MDMA HW request is DMA1 Stream 1 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA1_Stream2_TC                                                               \
	((uint32_t)0x00000002U) /*!< MDMA HW request is DMA1 Stream 2 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA1_Stream3_TC                                                               \
	((uint32_t)0x00000003U) /*!< MDMA HW request is DMA1 Stream 3 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA1_Stream4_TC                                                               \
	((uint32_t)0x00000004U) /*!< MDMA HW request is DMA1 Stream 4 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA1_Stream5_TC                                                               \
	((uint32_t)0x00000005U) /*!< MDMA HW request is DMA1 Stream 5 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA1_Stream6_TC                                                               \
	((uint32_t)0x00000006U) /*!< MDMA HW request is DMA1 Stream 6 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA1_Stream7_TC                                                               \
	((uint32_t)0x00000007U) /*!< MDMA HW request is DMA1 Stream 7 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA2_Stream0_TC                                                               \
	((uint32_t)0x00000008U) /*!< MDMA HW request is DMA2 Stream 0 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA2_Stream1_TC                                                               \
	((uint32_t)0x00000009U) /*!< MDMA HW request is DMA2 Stream 1 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA2_Stream2_TC                                                               \
	((uint32_t)0x0000000AU) /*!< MDMA HW request is DMA2 Stream 2 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA2_Stream3_TC                                                               \
	((uint32_t)0x0000000BU) /*!< MDMA HW request is DMA2 Stream 3 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA2_Stream4_TC                                                               \
	((uint32_t)0x0000000CU) /*!< MDMA HW request is DMA2 Stream 4 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA2_Stream5_TC                                                               \
	((uint32_t)0x0000000DU) /*!< MDMA HW request is DMA2 Stream 5 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA2_Stream6_TC                                                               \
	((uint32_t)0x0000000EU) /*!< MDMA HW request is DMA2 Stream 6 Transfer Complete Flag   */
#define MDMA_REQUEST_DMA2_Stream7_TC                                                               \
	((uint32_t)0x0000000FU) /*!< MDMA HW request is DMA2 Stream 7 Transfer Complete Flag   */
#define MDMA_REQUEST_FMC_DATA                                                                      \
	((uint32_t)0x00000014U) /*!< MDMA HW request is NAND data transfer (Tx or Rx) channel  */
#define MDMA_REQUEST_FMC_ERROR                                                                     \
	((uint32_t)0x00000015U) /*!< MDMA HW request is NAND ECC/BCH Error channel             */
#define MDMA_REQUEST_QUADSPI_FIFO_TH                                                               \
	((uint32_t)0x00000016U) /*!< MDMA HW request is QSPI FIFO threshold Flag               */
#define MDMA_REQUEST_QUADSPI_TC                                                                    \
	((uint32_t)0x00000017U) /*!< MDMA HW request is QSPI Transfer complete Flag            */
#if defined(CRYP1)
	#define MDMA_REQUEST_CRYP1_IN                                                              \
		((uint32_t)0x0000001DU) /*!< MDMA HW request is CRYP1 4 word request from input */
	#define MDMA_REQUEST_CRYP1_OUT                                                             \
		((uint32_t)0x0000001EU) /*!< MDMA HW request is CRYP1 4 word request from output   \
					 */
#endif
#define MDMA_REQUEST_HASH1_IN                                                                      \
	((uint32_t)0x0000001FU) /*!< MDMA HW request is HASH1 16 word request from input       */
#define MDMA_REQUEST_USART1_RX                                                                     \
	((uint32_t)0x00000020U) /*!< MDMA HW request is USART1 Rx Transfer Complete Flag       */
#define MDMA_REQUEST_USART1_TX                                                                     \
	((uint32_t)0x00000021U) /*!< MDMA HW request is USART1 Tx Transfer Complete Flag       */
#define MDMA_REQUEST_SPI6_RX                                                                       \
	((uint32_t)0x00000022U) /*!< MDMA HW request is SPI6 Rx Transfer Complete Flag         */
#define MDMA_REQUEST_SPI6_TX                                                                       \
	((uint32_t)0x00000023U) /*!< MDMA HW request is SPI6 Tx Transfer Complete Flag         */
#define MDMA_REQUEST_I2C4_RX                                                                       \
	((uint32_t)0x00000024U) /*!< MDMA HW request is I2C4 Rx Transfer Complete Flag         */
#define MDMA_REQUEST_I2C4_TX                                                                       \
	((uint32_t)0x00000025U) /*!< MDMA HW request is I2C4 Tx Transfer Complete Flag         */
#define MDMA_REQUEST_I2C6_RX                                                                       \
	((uint32_t)0x00000026U) /*!< MDMA HW request is I2C6 Rx Transfer Complete Flag         */
#define MDMA_REQUEST_I2C6_TX                                                                       \
	((uint32_t)0x00000027U) /*!< MDMA HW request is I2C6 Tx Transfer Complete Flag         */
...
#define MDMA_BUFFER_TRANSFER                                                                       \
	((uint32_t)0x00000000U) /*!< Each MDMA request (SW or HW) triggers a buffer transfer */
#define MDMA_BLOCK_TRANSFER                                                                        \
	((uint32_t)                                                                                \
		 MDMA_CTCR_TRGM_0) /*!< Each MDMA request (SW or HW) triggers a block transfer */
#define MDMA_REPEAT_BLOCK_TRANSFER                                                                 \
	((uint32_t)MDMA_CTCR_TRGM_1) /*!< Each MDMA request (SW or HW) triggers a repeated block   \
					transfer                        */
#define MDMA_FULL_TRANSFER                                                                         \
	((uint32_t)MDMA_CTCR_TRGM) /*!< Each MDMA request (SW or HW) triggers a Full transfer or a \
				      linked list transfer if any */
...
#define MDMA_LITTLE_ENDIANNESS_PRESERVE                                                            \
	((uint32_t)0x00000000U) /*!< little endianness preserve */
#define MDMA_LITTLE_BYTE_ENDIANNESS_EXCHANGE                                                       \
	((uint32_t)MDMA_CCR_BEX) /*!< BYTEs endianness exchange when destination data size is >    \
				    Byte           */
#define MDMA_LITTLE_HALFWORD_ENDIANNESS_EXCHANGE                                                   \
	((uint32_t)MDMA_CCR_HEX) /*!< HALF WORDs endianness exchange when destination data size is \
				    > HALF WORD */
#define MDMA_LITTLE_WORD_ENDIANNESS_EXCHANGE                                                       \
	((uint32_t)MDMA_CCR_WEX) /*!< WORDs endianness exchange  when destination data size is >   \
				    DOUBLE WORD   */
...
#define MDMA_SRC_INC_DISABLE                                                                       \
	((uint32_t)0x00000000U) /*!< Source address pointer is fixed */
#define MDMA_SRC_INC_BYTE                                                                          \
	((uint32_t)MDMA_CTCR_SINC_1) /*!< Source address pointer is incremented by a BYTE (8 bits) \
				      */
#define MDMA_SRC_INC_HALFWORD                                                                      \
	((uint32_t)MDMA_CTCR_SINC_1 |                                                              \
	 (uint32_t)MDMA_CTCR_SINCOS_0) /*!< Source address pointer is incremented by a half Word   \
					  (16 bits)    */
#define MDMA_SRC_INC_WORD                                                                          \
	((uint32_t)MDMA_CTCR_SINC_1 |                                                              \
	 (uint32_t)MDMA_CTCR_SINCOS_1) /*!< Source address pointer is incremented by a Word (32    \
					  bits)         */
#define MDMA_SRC_INC_DOUBLEWORD                                                                    \
	((uint32_t)MDMA_CTCR_SINC_1 |                                                              \
	 (uint32_t)MDMA_CTCR_SINCOS) /*!< Source address pointer is incremented by a double Word   \
					(64 bits)) */
#define MDMA_SRC_DEC_BYTE                                                                          \
	((uint32_t)                                                                                \
		 MDMA_CTCR_SINC) /*!< Source address pointer is decremented by a BYTE (8 bits) */
#define MDMA_SRC_DEC_HALFWORD                                                                      \
	((uint32_t)MDMA_CTCR_SINC |                                                                \
	 (uint32_t)MDMA_CTCR_SINCOS_0) /*!< Source address pointer is decremented by a half Word   \
					  (16 bits)    */
#define MDMA_SRC_DEC_WORD                                                                          \
	((uint32_t)MDMA_CTCR_SINC |                                                                \
	 (uint32_t)MDMA_CTCR_SINCOS_1) /*!< Source address pointer is decremented by a Word (32    \
					  bits)         */
#define MDMA_SRC_DEC_DOUBLEWORD                                                                    \
	((uint32_t)MDMA_CTCR_SINC |                                                                \
	 (uint32_t)MDMA_CTCR_SINCOS) /*!< Source address pointer is decremented by a double Word   \
					(64 bits)) */
...
#define MDMA_DEST_INC_BYTE                                                                         \
	((uint32_t)MDMA_CTCR_DINC_1) /*!< Source address pointer is incremented by a BYTE (8 bits) \
				      */
#define MDMA_DEST_INC_HALFWORD                                                                     \
	((uint32_t)MDMA_CTCR_DINC_1 |                                                              \
	 (uint32_t)MDMA_CTCR_DINCOS_0) /*!< Source address pointer is incremented by a half Word   \
					  (16 bits)    */
#define MDMA_DEST_INC_WORD                                                                         \
	((uint32_t)MDMA_CTCR_DINC_1 |                                                              \
	 (uint32_t)MDMA_CTCR_DINCOS_1) /*!< Source address pointer is incremented by a Word (32    \
					  bits)         */
#define MDMA_DEST_INC_DOUBLEWORD                                                                   \
	((uint32_t)MDMA_CTCR_DINC_1 |                                                              \
	 (uint32_t)MDMA_CTCR_DINCOS) /*!< Source address pointer is incremented by a double Word   \
					(64 bits)) */
#define MDMA_DEST_DEC_BYTE                                                                         \
	((uint32_t)                                                                                \
		 MDMA_CTCR_DINC) /*!< Source address pointer is decremented by a BYTE (8 bits) */
#define MDMA_DEST_DEC_HALFWORD                                                                     \
	((uint32_t)MDMA_CTCR_DINC |                                                                \
	 (uint32_t)MDMA_CTCR_DINCOS_0) /*!< Source address pointer is decremented by a half Word   \
					  (16 bits)    */
#define MDMA_DEST_DEC_WORD                                                                         \
	((uint32_t)MDMA_CTCR_DINC |                                                                \
	 (uint32_t)MDMA_CTCR_DINCOS_1) /*!< Source address pointer is decremented by a Word (32    \
					  bits)         */
#define MDMA_DEST_DEC_DOUBLEWORD                                                                   \
	((uint32_t)MDMA_CTCR_DINC |                                                                \
	 (uint32_t)MDMA_CTCR_DINCOS) /*!< Source address pointer is decremented by a double Word   \
					(64 bits)) */
...
#define MDMA_DEST_DATASIZE_HALFWORD                                                                \
	((uint32_t)MDMA_CTCR_DSIZE_0) /*!< Destination data size is half word   */
#define MDMA_DEST_DATASIZE_WORD                                                                    \
	((uint32_t)MDMA_CTCR_DSIZE_1) /*!< Destination data size is word        */
#define MDMA_DEST_DATASIZE_DOUBLEWORD                                                              \
	((uint32_t)MDMA_CTCR_DSIZE) /*!< Destination data size is double word */
...
#define MDMA_DATAALIGN_PACKENABLE                                                                  \
	((uint32_t)                                                                                \
		 MDMA_CTCR_PKE) /*!< The source data is packed/un-packed into the destination data \
				   size All data are right aligned, in Little Endien mode. */
#define MDMA_DATAALIGN_RIGHT                                                                       \
	((uint32_t)0x00000000U) /*!< Right Aligned, padded w/ 0s (default) */
#define MDMA_DATAALIGN_RIGHT_SIGNED                                                                \
	((uint32_t)MDMA_CTCR_PAM_0) /*!< Right Aligned, Sign extended ,                            \
					 Note : this mode is allowed only if the Source data size  \
				       is smaller than Destination data size  */
#define MDMA_DATAALIGN_LEFT                                                                        \
	((uint32_t)MDMA_CTCR_PAM_1) /*!< Left Aligned (padded with 0s) */
...
#define MDMA_SOURCE_BURST_8BEATS                                                                   \
	((uint32_t)MDMA_CTCR_SBURST_0 | (uint32_t)MDMA_CTCR_SBURST_1) /*!< Burst 8 beats   */
#define MDMA_SOURCE_BURST_16BEATS ((uint32_t)MDMA_CTCR_SBURST_2)      /*!< Burst 16 beats  */
#define MDMA_SOURCE_BURST_32BEATS                                                                  \
	((uint32_t)MDMA_CTCR_SBURST_0 | (uint32_t)MDMA_CTCR_SBURST_2) /*!< Burst 32 beats  */
#define MDMA_SOURCE_BURST_64BEATS                                                                  \
	((uint32_t)MDMA_CTCR_SBURST_1 | (uint32_t)MDMA_CTCR_SBURST_2) /*!< Burst 64 beats  */
...
#define MDMA_DEST_BURST_8BEATS                                                                     \
	((uint32_t)MDMA_CTCR_DBURST_0 | (uint32_t)MDMA_CTCR_DBURST_1) /*!< Burst 8 beats   */
#define MDMA_DEST_BURST_16BEATS ((uint32_t)MDMA_CTCR_DBURST_2)	      /*!< Burst 16 beats  */
#define MDMA_DEST_BURST_32BEATS                                                                    \
	((uint32_t)MDMA_CTCR_DBURST_0 | (uint32_t)MDMA_CTCR_DBURST_2) /*!< Burst 32 beats  */
#define MDMA_DEST_BURST_64BEATS                                                                    \
	((uint32_t)MDMA_CTCR_DBURST_1 | (uint32_t)MDMA_CTCR_DBURST_2) /*!< Burst 64 beats  */
 ...
 * @retval None
 */
#define __HAL_MDMA_SET_COUNTER(__HANDLE__, __COUNTER__)                                            \
...
HAL_StatusTypeDef HAL_MDMA_ConfigPostRequestMask(MDMA_HandleTypeDef *hmdma, uint32_t MaskAddress,
						 uint32_t MaskData);
...
#define IS_MDMA_LEVEL_COMPLETE(__LEVEL__)                                                          \
	(((__LEVEL__) == HAL_MDMA_FULL_TRANSFER) || ((__LEVEL__) == HAL_MDMA_BUFFER_TRANSFER) ||   \
	 ((__LEVEL__) == HAL_MDMA_BLOCK_TRANSFER) ||                                               \
	 ((__LEVEL__) == HAL_MDMA_REPEAT_BLOCK_TRANSFER))

#define IS_MDMA_PRIORITY(__PRIORITY__)                                                             \
	(((__PRIORITY__) == MDMA_PRIORITY_LOW) || ((__PRIORITY__) == MDMA_PRIORITY_MEDIUM) ||      \
	 ((__PRIORITY__) == MDMA_PRIORITY_HIGH) || ((__PRIORITY__) == MDMA_PRIORITY_VERY_HIGH))

#define IS_MDMA_SECURE_MODE(__SECURE_MODE__)                                                       \
	(((__SECURE_MODE__) == MDMA_SECURE_MODE_DISABLE) ||                                        \
...
#define IS_MDMA_REQUEST(__REQUEST__)                                                               \
	(((__REQUEST__) == MDMA_REQUEST_SW) || ((__REQUEST__) <= MDMA_REQUEST_I2C6_TX))

#define IS_MDMA_SOURCE_INC(__INC__)                                                                \
	(((__INC__) == MDMA_SRC_INC_DISABLE) || ((__INC__) == MDMA_SRC_INC_BYTE) ||                \
	 ((__INC__) == MDMA_SRC_INC_HALFWORD) || ((__INC__) == MDMA_SRC_INC_WORD) ||               \
	 ((__INC__) == MDMA_SRC_INC_DOUBLEWORD) || ((__INC__) == MDMA_SRC_DEC_BYTE) ||             \
	 ((__INC__) == MDMA_SRC_DEC_HALFWORD) || ((__INC__) == MDMA_SRC_DEC_WORD) ||               \
	 ((__INC__) == MDMA_SRC_DEC_DOUBLEWORD))

#define IS_MDMA_DESTINATION_INC(__INC__)                                                           \
	(((__INC__) == MDMA_DEST_INC_DISABLE) || ((__INC__) == MDMA_DEST_INC_BYTE) ||              \
	 ((__INC__) == MDMA_DEST_INC_HALFWORD) || ((__INC__) == MDMA_DEST_INC_WORD) ||             \
	 ((__INC__) == MDMA_DEST_INC_DOUBLEWORD) || ((__INC__) == MDMA_DEST_DEC_BYTE) ||           \
	 ((__INC__) == MDMA_DEST_DEC_HALFWORD) || ((__INC__) == MDMA_DEST_DEC_WORD) ||             \
	 ((__INC__) == MDMA_DEST_DEC_DOUBLEWORD))

#define IS_MDMA_SOURCE_DATASIZE(__SIZE__)                                                          \
	(((__SIZE__) == MDMA_SRC_DATASIZE_BYTE) || ((__SIZE__) == MDMA_SRC_DATASIZE_HALFWORD) ||   \
	 ((__SIZE__) == MDMA_SRC_DATASIZE_WORD) || ((__SIZE__) == MDMA_SRC_DATASIZE_DOUBLEWORD))

#define IS_MDMA_DESTINATION_DATASIZE(__SIZE__)                                                     \
	(((__SIZE__) == MDMA_DEST_DATASIZE_BYTE) || ((__SIZE__) == MDMA_DEST_DATASIZE_HALFWORD) || \
	 ((__SIZE__) == MDMA_DEST_DATASIZE_WORD) || ((__SIZE__) == MDMA_DEST_DATASIZE_DOUBLEWORD))
...
#define IS_MDMA_SOURCE_BURST(__BURST__)                                                            \
	(((__BURST__) == MDMA_SOURCE_BURST_SINGLE) || ((__BURST__) == MDMA_SOURCE_BURST_2BEATS) || \
	 ((__BURST__) == MDMA_SOURCE_BURST_4BEATS) || ((__BURST__) == MDMA_SOURCE_BURST_8BEATS) || \
...
#define IS_MDMA_DESTINATION_BURST(__BURST__)                                                       \
	(((__BURST__) == MDMA_DEST_BURST_SINGLE) || ((__BURST__) == MDMA_DEST_BURST_2BEATS) ||     \
	 ((__BURST__) == MDMA_DEST_BURST_4BEATS) || ((__BURST__) == MDMA_DEST_BURST_8BEATS) ||     \
	 ((__BURST__) == MDMA_DEST_BURST_16BEATS) || ((__BURST__) == MDMA_DEST_BURST_32BEATS) ||   \
	 ((__BURST__) == MDMA_DEST_BURST_64BEATS) || ((__BURST__) == MDMA_DEST_BURST_128BEATS))

#define IS_MDMA_TRANSFER_TRIGGER_MODE(__MODE__)                                                    \
	(((__MODE__) == MDMA_BUFFER_TRANSFER) || ((__MODE__) == MDMA_BLOCK_TRANSFER) ||            \
	 ((__MODE__) == MDMA_REPEAT_BLOCK_TRANSFER) || ((__MODE__) == MDMA_FULL_TRANSFER))
