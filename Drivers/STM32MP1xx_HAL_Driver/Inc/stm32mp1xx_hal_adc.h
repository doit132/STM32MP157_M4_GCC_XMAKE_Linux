...
typedef struct {
	...
	uint32_t OversamplingStopReset; /*!< Selects the regular oversampling mode.
					     The oversampling is either temporary stopped or reset
					   upon an injected sequence interruption. If oversampling
					   is enabled on both regular and injected groups, this
					   parameter is discarded and forced to setting
					   "ADC_REGOVERSAMPLING_RESUMED_MODE" (the oversampling
					   buffer is zeroed during injection sequence). This
...
typedef struct {
	uint32_t
		ClockPrescaler; /*!< Select ADC clock source (synchronous clock derived from APB
				   clock or asynchronous clock derived from system clock or PLL
				   (Refer to reference manual for list of clocks available)) and
				   clock prescaler. This parameter can be a value of @ref
				   ADC_HAL_EC_COMMON_CLOCK_SOURCE. Note: The ADC clock configuration
				   is common to all ADC instances. Note: In case of usage of
				   channels on injected group, ADC frequency should be lower than
				   AHB clock frequency /4 for resolution 12 or 10 bits, AHB clock
				   frequency /3 for resolution 8 bits, AHB clock frequency /2 for
				   resolution 6 bits. Note: In case of synchronous clock mode based
				   on HCLK/1, the configuration must be enabled only if the system
				   clock has a 50% duty clock cycle (APB prescaler configured inside
				   RCC must be bypassed and PCLK clock must have 50% duty cycle).
				   Refer to reference manual for details. Note: In case of usage of
				   asynchronous clock, the selected clock must be preliminarily
				   enabled at RCC top level. Note: This parameter can be modified
	...
	uint32_t
		ScanConvMode; /*!< Configure the sequencer of ADC groups regular and injected.
				 ...
				 successive parts. If disabled: Conversion is performed in single
				 mode (one channel converted, the one defined in rank 1). Parameters
				 'NbrOfConversion' and 'InjectedNbrOfConversion' are discarded
				 (equivalent to set to 1). If enabled:  Conversions are performed in
				 sequence mode (multiple ranks defined by 'NbrOfConversion' or
				 'InjectedNbrOfConversion' and rank of each channel in sequencer).
						Scan direction is upward: from rank 1 to rank 'n'.
				   This parameter can be a value of @ref ADC_Scan_mode */

	uint32_t EOCSelection; /*!< Specify which EOC (End Of Conversion) flag is used for
				  conversion by polling and interruption: end of unitary conversion
				  or end of sequence conversions. This parameter can be a value of
				  @ref ADC_EOCSelection. */

	FunctionalState
		LowPowerAutoWait; /*!< Select the dynamic low power Auto Delay: new conversion start
				     only when the previous conversion (for ADC group regular) or
				     previous sequence (for ADC group injected) has been retrieved
				     by user software, using function HAL_ADC_GetValue() or
				     HAL_ADCEx_InjectedGetValue(). This feature automatically adapts
				     the frequency of ADC conversions triggers to the speed of the
				     system that reads the data. Moreover, this avoids risk of
				     overrun for low frequency applications. This parameter can be
				     set to ENABLE or DISABLE. Note: It is not recommended to use
				     with interruption or DMA (HAL_ADC_Start_IT(),
				     HAL_ADC_Start_DMA()) since these modes have to clear
				     immediately the EOC flag (by CPU to free the IRQ pending event
				     or by DMA). Auto wait will work but fort a very short time,
				     discarding its intended benefit (except specific case of high
				     load of CPU or DMA transfers which can justify usage of auto
				     wait). Do use with polling: 1. Start conversion with
				     HAL_ADC_Start(), 2. Later on, when ADC conversion data is
				     needed: use HAL_ADC_PollForConversion() to ensure that
				     conversion is completed and HAL_ADC_GetValue() to retrieve
				     conversion result and trig another conversion start. (in case
				     of usage of ADC group injected, use the equivalent functions
				     HAL_ADCExInjected_Start(), HAL_ADCEx_InjectedGetValue(), ...).
				   */

	FunctionalState ContinuousConvMode; /*!< Specify whether the conversion is performed in
					     single mode (one conversion) or continuous mode for ADC
					     group regular, after the first ADC conversion start
					     trigger occurred (software start or external trigger).
					     This parameter can be set to ENABLE or DISABLE. */

	uint32_t NbrOfConversion; /*!< Specify the number of ranks that will be converted within the
				     regular group sequencer. To use the regular group sequencer and
				     convert several ranks, parameter 'ScanConvMode' must be
				     enabled. This parameter must be a number between Min_Data = 1
				     and Max_Data = 16. Note: This parameter must be modified when
				     no conversion is on going on regular group (ADC disabled, or
				     ADC enabled without continuous mode or external trigger that
				     could launch a conversion). */

	FunctionalState
		DiscontinuousConvMode; /*!< Specify whether the conversions sequence of ADC group
				     regular is performed in
				     ...
				     disabled. If continuous mode is enabled, this parameter setting
	...
	uint32_t NbrOfDiscConversion; /*!< Specifies the number of discontinuous conversions in
					 which the main sequence of ADC group regular (parameter
					 NbrOfConversion) will be subdivided. If parameter
					 'DiscontinuousConvMode' is disabled, this parameter is
					 discarded. This parameter must be a number between Min_Data
					 = 1 and Max_Data = 8. */

	uint32_t ExternalTrigConv; /*!< Select the external event source used to trigger ADC group
				      regular conversion start. If set to ADC_SOFTWARE_START,
				      external triggers are disabled and software trigger is used
				      instead. This parameter can be a value of @ref
				      ADC_regular_external_trigger_source. Caution: external trigger
				      source is common to all ADC instances. */

	uint32_t ExternalTrigConvEdge; /*!< Select the external event edge used to trigger ADC group
					  regular conversion start. If trigger source is set to
					  ADC_SOFTWARE_START, this parameter is discarded. This
					  parameter can be a value of @ref
					  ADC_regular_external_trigger_edge */

	uint32_t ConversionDataManagement; /*!< Specifies whether the Data conversion data is
					     managed: using the DMA (oneshot or circular), or stored
					     in the DR register or transferred to DFSDM register.
					     ...
					     parameter can be a value of @ref
					     ADC_ConversionDataManagement. Note: This parameter must
	...
	uint32_t Overrun; /*!< Select the behavior in case of overrun: data overwritten or preserved
			     (default). This parameter applies to ADC group regular only. This
			     parameter can be a value of @ref ADC_HAL_EC_REG_OVR_DATA_BEHAVIOR.
			       Note: In case of overrun set to data preserved and usage with
			     programming model with interruption (HAL_Start_IT()): ADC IRQ handler
			     has to clear end of conversion flags, this induces the release of the
			     preserved data. If needed, this data can be saved in function
			       HAL_ADC_ConvCpltCallback(), placed in user program code (called
			     before end of conversion flags clear). Note: Error reporting with
			     ...
			     preserved. If overrun is set to data overwritten, user can willingly
			     not read all the converted data, this is not considered as an erroneous
			     case.
				     - Usage with ADC conversion by DMA: Error is reported whatever
			     overrun setting (DMA is expected to process all data from data
			     register). */
	...
	FunctionalState OversamplingMode; /*!< Specify whether the oversampling feature is enabled
					     or disabled. This parameter can be set to ENABLE or
					     DISABLE. Note: This parameter can be modified only if
					     there is no conversion is ongoing on ADC groups regular
					     and injected */
...
typedef struct {
	...
	uint32_t SamplingTime; /*!< Sampling time value to be set for the selected channel.
				  ...
				  parameter can be a value of @ref ADC_HAL_EC_CHANNEL_SAMPLINGTIME
				    Caution: This parameter applies to a channel that can be used
				  into regular and/or injected group. It overwrites the last
	...
	uint32_t SingleDiff; /*!< Select single-ended or differential input.
				...
				between the selected channel 'i' (positive input) and channel 'i+1'
				(negative input). Only channel 'i' has to be configured, channel
				'i+1' is configured automatically. This parameter must be a value of
				@ref ADC_HAL_EC_CHANNEL_SINGLE_DIFF_ENDING Caution: This parameter
				...
				mode. Note: When configuring a channel 'i' in differential mode, the
				channel 'i+1' is not usable separately. Note: This parameter must be
				modified when ADC is disabled (before ADC start conversion or after
				ADC stop conversion). If ADC is enabled, this parameter setting is
				bypassed without error reporting (as it can be the expected behavior
	...
	uint32_t Offset; /*!< Define the offset to be subtracted from the raw converted data.
			      ...
			      Depending of ADC resolution selected (16, 14, 12, 10, 8 bits), this
			    parameter must be a number between Min_Data = 0x0000 and Max_Data =
			    0xFFFF, 0x3FFF, 0xFFF, 0x3FF or 0xFF respectively. Note: This parameter
			    must be modified when no conversion is on going on both regular and
			    injected groups (ADC disabled, or ADC enabled without continuous mode or
			    external trigger that could launch a conversion). */

	FunctionalState
		OffsetRightShift; /*!< Define the Right-shift data after Offset correction.
				    This parameter is applied only for 16-bit or 8-bit resolution.
				    This parameter can be set to ENABLE or DISABLE.*/

	FunctionalState OffsetSignedSaturation; /*!< Specify whether the Signed saturation feature
						   is used or not. This parameter is applied only
						   for 16-bit or 8-bit resolution. This parameter
...
typedef struct {
	uint32_t WatchdogNumber; /*!< Select which ADC analog watchdog is monitoring the selected
				    channel. For Analog Watchdog 1: Only 1 channel can be monitored
				    (or overall group of channels by setting parameter
				    'WatchdogMode') For Analog Watchdog 2 and 3: Several channels
				    can be monitored (by successive calls of
				    'HAL_ADC_AnalogWDGConfig()' for each channel) This parameter can
				    be a value of @ref ADC_HAL_EC_AWD_NUMBER. */
	...
	uint32_t Channel; /*!< Select which ADC channel to monitor by analog watchdog.
			     ...
			     monitored). For Analog Watchdog 2 and 3: Several channels can be
			     monitored. To use this feature, call successively the function
			     HAL_ADC_AnalogWDGConfig() for each channel to be added (or removed with
			     value 'ADC_ANALOGWATCHDOG_NONE'). This parameter can be a value of @ref
			     ADC_HAL_EC_CHANNEL. */

	FunctionalState
		ITMode; /*!< Specify whether the analog watchdog is configured in interrupt or
			   polling mode. This parameter can be set to ENABLE or DISABLE */

	uint32_t HighThreshold; /*!< Configure the ADC analog watchdog High threshold value.
				     Depending of ADC resolution selected (16, 14, 12, 10, 8 bits),
				   this parameter must be a number between Min_Data = 0x000 and
				   Max_Data = 0xFFFF, 0x3FFF, 0xFFF, 0x3FF or 0xFF respectively.
				     Note: Analog watchdog 2 and 3 are limited to a resolution of 8
				   bits: if ADC resolution is 12 bits the 4 LSB are ignored, if ADC
				   resolution is 10 bits the 2 LSB are ignored. Note: If ADC
				   oversampling is enabled, ADC analog watchdog thresholds are
					   impacted: the comparison of analog watchdog thresholds is
				   done on oversampling intermediate computation (after ratio,
				   before shift application): intermediate register bitfield [32:7]
	...
	uint32_t LowThreshold; /*!< Configures the ADC analog watchdog Low threshold value.
				  ...
				  oversampling is enabled, ADC analog watchdog thresholds are
					  impacted: the comparison of analog watchdog thresholds is
				  done on oversampling intermediate computation (after ratio, before
				  shift application): intermediate register bitfield [32:7] (26 most
				  significant bits). */
...
#define HAL_ADC_STATE_INJ_BUSY                                                                     \
	(0x00001000UL) /*!< A conversion on ADC group injected is ongoing or can occur (either by  \
			  auto-injection mode, external trigger, low power auto power-on (if                                                     \
			  feature available), multimode ADC master control (if feature available)) \
			*/
...
#define HAL_ADC_STATE_MULTIMODE_SLAVE                                                              \
	(0x00100000UL) /*!< ADC in multimode slave state, controlled by another ADC master (when   \
			  feature available) */
...
#define ADC_CLOCK_SYNC_PCLK_DIV2                                                                   \
	(LL_ADC_CLOCK_SYNC_PCLK_DIV2) /*!< ADC synchronous clock derived from AHB clock with       \
					 prescaler division by 2 */
#define ADC_CLOCK_SYNC_PCLK_DIV4                                                                   \
	(LL_ADC_CLOCK_SYNC_PCLK_DIV4) /*!< ADC synchronous clock derived from AHB clock with       \
...
#define ADC_EXTERNALTRIG_T3_TRGO                                                                   \
	(LL_ADC_REG_TRIG_EXT_TIM3_TRGO) /*!< ADC group regular conversion trigger from external    \
					   peripheral: TIM3 TRGO event. Trigger edge set to rising \
					   edge (default setting). */
#define ADC_EXTERNALTRIG_T4_CC4                                                                    \
	(LL_ADC_REG_TRIG_EXT_TIM4_CH4) /*!< ADC group regular conversion trigger from external     \
...
#define ADC_EXTERNALTRIG_EXT_IT11                                                                  \
	(LL_ADC_REG_TRIG_EXT_EXTI_LINE11) /*!< ADC group regular conversion trigger from external  \
					     peripheral: external interrupt line 11 event. Trigger \
					     edge set to rising edge (default setting). */
#define ADC_EXTERNALTRIG_T8_TRGO                                                                   \
	(LL_ADC_REG_TRIG_EXT_TIM8_TRGO) /*!< ADC group regular conversion trigger from external    \
					   peripheral: TIM8 TRGO event. Trigger edge set to rising \
					   edge (default setting). */
#define ADC_EXTERNALTRIG_T8_TRGO2                                                                  \
	(LL_ADC_REG_TRIG_EXT_TIM8_TRGO2) /*!< ADC group regular conversion trigger from external   \
					    peripheral: TIM8 TRGO2 event. Trigger edge set to      \
...
#define ADC_EXTERNALTRIG_T1_TRGO                                                                   \
	(LL_ADC_REG_TRIG_EXT_TIM1_TRGO) /*!< ADC group regular conversion trigger from external    \
					   peripheral: TIM1 TRGO event. Trigger edge set to rising \
					   edge (default setting). */
#define ADC_EXTERNALTRIG_T1_TRGO2                                                                  \
	(LL_ADC_REG_TRIG_EXT_TIM1_TRGO2) /*!< ADC group regular conversion trigger from external   \
					    peripheral: TIM1 TRGO2 event. Trigger edge set to      \
					    rising edge (default setting). */
#define ADC_EXTERNALTRIG_T2_TRGO                                                                   \
	(LL_ADC_REG_TRIG_EXT_TIM2_TRGO) /*!< ADC group regular conversion trigger from external    \
					   peripheral: TIM2 TRGO event. Trigger edge set to rising \
					   edge (default setting). */
#define ADC_EXTERNALTRIG_T4_TRGO                                                                   \
	(LL_ADC_REG_TRIG_EXT_TIM4_TRGO) /*!< ADC group regular conversion trigger from external    \
					   peripheral: TIM4 TRGO event. Trigger edge set to rising \
					   edge (default setting). */
#define ADC_EXTERNALTRIG_T6_TRGO                                                                   \
	(LL_ADC_REG_TRIG_EXT_TIM6_TRGO) /*!< ADC group regular conversion trigger from external    \
					   peripheral: TIM6 TRGO event. Trigger edge set to rising \
					   edge (default setting). */
#define ADC_EXTERNALTRIG_T15_TRGO                                                                  \
	(LL_ADC_REG_TRIG_EXT_TIM15_TRGO) /*!< ADC group regular conversion trigger from external   \
					    peripheral: TIM15 TRGO event. Trigger edge set to      \
					    rising edge (default setting). */
#define ADC_EXTERNALTRIG_T3_CC4                                                                    \
	(LL_ADC_REG_TRIG_EXT_TIM3_CH4) /*!< ADC group regular conversion trigger from external     \
					  peripheral: TIM3 channel 4 event (capture compare: input \
					  capture or output capture). Trigger edge set to rising   \
					  edge (default setting). */
#define ADC_EXTERNALTRIG_LPTIM1_OUT                                                                \
	(LL_ADC_REG_TRIG_EXT_LPTIM1_OUT) /*!< ADC group regular conversion trigger from external   \
					    peripheral: LPTIM1 OUT event. Trigger edge set to      \
					    rising edge (default setting). */
#define ADC_EXTERNALTRIG_LPTIM2_OUT                                                                \
	(LL_ADC_REG_TRIG_EXT_LPTIM2_OUT) /*!< ADC group regular conversion trigger from external   \
					    peripheral: LPTIM2 OUT event. Trigger edge set to      \
					    rising edge (default setting). */
#define ADC_EXTERNALTRIG_LPTIM3_OUT                                                                \
	(LL_ADC_REG_TRIG_EXT_LPTIM3_OUT) /*!< ADC group regular conversion trigger from external   \
					    peripheral: LPTIM3 event OUT. Trigger edge set to      \
					    rising edge (default setting). */
/**
 * @}
 */
...
#define ADC_EXTERNALTRIGCONVEDGE_RISING                                                            \
	(LL_ADC_REG_TRIG_EXT_RISING) /*!< ADC group regular conversion trigger polarity set to     \
					rising edge */
...
#define ADC_EXTERNALTRIGCONVEDGE_RISINGFALLING                                                     \
	(LL_ADC_REG_TRIG_EXT_RISINGFALLING) /*!< ADC group regular conversion trigger polarity set \
...
#define ADC_SAMPLETIME_1CYCLE_5                                                                    \
	(LL_ADC_SAMPLINGTIME_1CYCLE_5) /*!< Sampling time 1.5 ADC clock cycles */
...
#define ADC_SAMPLETIME_8CYCLES_5                                                                   \
	(LL_ADC_SAMPLINGTIME_8CYCLES_5) /*!< Sampling time 8.5 ADC clock cycles */
#define ADC_SAMPLETIME_16CYCLES_5                                                                  \
	(LL_ADC_SAMPLINGTIME_16CYCLES_5) /*!< Sampling time 16.5 ADC clock cycles */
#define ADC_SAMPLETIME_32CYCLES_5                                                                  \
	(LL_ADC_SAMPLINGTIME_32CYCLES_5) /*!< Sampling time 32.5 ADC clock cycles */
#define ADC_SAMPLETIME_64CYCLES_5                                                                  \
	(LL_ADC_SAMPLINGTIME_64CYCLES_5) /*!< Sampling time 64.5 ADC clock cycles */
#define ADC_SAMPLETIME_387CYCLES_5                                                                 \
	(LL_ADC_SAMPLINGTIME_387CYCLES_5) /*!< Sampling time 387.5 ADC clock cycles */
#define ADC_SAMPLETIME_810CYCLES_5                                                                 \
	(LL_ADC_SAMPLINGTIME_810CYCLES_5) /*!< Sampling time 810.5 ADC clock cycles */
/**
 * @}
 */
...
#define ADC_CHANNEL_19                                                                             \
	(LL_ADC_CHANNEL_19) /*!< ADC external channel (channel connected to GPIO pin) ADCx_IN19 */
#define ADC_CHANNEL_VREFINT                                                                        \
	(LL_ADC_CHANNEL_VREFINT) /*!< ADC internal channel connected to VrefInt: Internal voltage  \
				    reference, channel specific to ADC2. */
#define ADC_CHANNEL_VCORE                                                                          \
	(LL_ADC_CHANNEL_VCORE) /*!< ADC internal channel connected to VddCore, channel specific to \
				  ADC2. */
...
#define ADC_CHANNEL_VBAT                                                                           \
	(LL_ADC_CHANNEL_VBAT) /*!< ADC internal channel connected to Vbat/4: Vbat voltage through  \
				 a divider ladder of factor 1/4 to have Vbat always below Vdda,    \
				 channel specific to ADC2. */
#define ADC_CHANNEL_DAC1CH1_ADC2                                                                   \
	(LL_ADC_CHANNEL_DAC1CH1_ADC2) /*!< ADC internal channel connected to DAC1 channel 1,       \
					 channel specific to ADC2 */
#define ADC_CHANNEL_DAC1CH2_ADC2                                                                   \
	(LL_ADC_CHANNEL_DAC1CH2_ADC2) /*!< ADC internal channel connected to DAC1 channel 2,       \
					 channel specific to ADC2 */
/**
 * @}
 */
...
#define ADC_CONVERSIONDATA_DR                                                                      \
	(0x00000000UL) /*!< Regular Conversion data stored in DR register only  */
...
#define ADC_CONVERSIONDATA_DMA_CIRCULAR                                                            \
	(ADC_CFGR_DMNGT_0 | ADC_CFGR_DMNGT_1) /*!< DMA circular mode selected */
/**
 * @}
 */
...
#define ADC_ANALOGWATCHDOG_ALL_REGINJEC                                                            \
	(ADC_CFGR_AWD1EN | ADC_CFGR_JAWD1EN) /*!< Analog watchdog applied to regular and injected  \
						groups all channels     */
/**
 * @}
 */
...
#define ADC_RIGHTBITSHIFT_9                                                                        \
	(LL_ADC_OVS_SHIFT_RIGHT_9) /*!< ADC oversampling shift of 9 (sum of the ADC conversions    \
				      data is divided by 512 to result as the ADC oversampling     \
				      conversion data) */
#define ADC_RIGHTBITSHIFT_10                                                                       \
	(LL_ADC_OVS_SHIFT_RIGHT_10) /*!< ADC oversampling shift of 10 (sum of the ADC conversions  \
				       data is divided by 1024 to result as the ADC oversampling   \
				       conversion data) */
#define ADC_RIGHTBITSHIFT_11                                                                       \
	(LL_ADC_OVS_SHIFT_RIGHT_11) /*!< ADC oversampling shift of 11 (sum of the ADC conversions  \
...
#define ADC_TRIGGEREDMODE_MULTI_TRIGGER                                                            \
	(LL_ADC_OVS_REG_DISCONT) /*!< ADC oversampling discontinuous mode: discontinuous mode      \
...
#define ADC_AWD1_EVENT                                                                             \
	(ADC_FLAG_AWD1) /*!< ADC Analog watchdog 1 event (main analog watchdog, present on all     \
			   STM32 series) */
#define ADC_AWD2_EVENT                                                                             \
	(ADC_FLAG_AWD2) /*!< ADC Analog watchdog 2 event (additional analog watchdog, not present  \
			   on all STM32 series) */
#define ADC_AWD3_EVENT                                                                             \
	(ADC_FLAG_AWD3) /*!< ADC Analog watchdog 3 event (additional analog watchdog, not present  \
			   on all STM32 series) */
...
#define ADC_IT_AWD                                                                                 \
	ADC_IT_AWD1 /*!< ADC Analog watchdog 1 interrupt source: naming for compatibility with     \
		       other STM32 devices having only one analog watchdog */
...
#define IS_ADC_CONVERSIONDATAMGT(DATA)                                                             \
	((((DATA) == ADC_CONVERSIONDATA_DR)) || (((DATA) == ADC_CONVERSIONDATA_DFSDM)) ||          \
...
#define IS_ADC_RESOLUTION(__RESOLUTION__)                                                          \
	(((__RESOLUTION__) == ADC_RESOLUTION_16B) || ((__RESOLUTION__) == ADC_RESOLUTION_14B) ||   \
...
#define IS_ADC_EXTTRIG(__REGTRIG__)                                                                \
	(((__REGTRIG__) == ADC_EXTERNALTRIG_T1_CC1) ||                                             \
	 ...
	 ((__REGTRIG__) == ADC_EXTERNALTRIG_LPTIM3_OUT) || ((__REGTRIG__) == ADC_SOFTWARE_START))
...
#define IS_ADC_SAMPLE_TIME(__TIME__)                                                               \
	(((__TIME__) == ADC_SAMPLETIME_1CYCLE_5) || ((__TIME__) == ADC_SAMPLETIME_2CYCLES_5) ||    \
	 ((__TIME__) == ADC_SAMPLETIME_8CYCLES_5) || ((__TIME__) == ADC_SAMPLETIME_16CYCLES_5) ||  \
	 ((__TIME__) == ADC_SAMPLETIME_32CYCLES_5) || ((__TIME__) == ADC_SAMPLETIME_64CYCLES_5) || \
	 ((__TIME__) == ADC_SAMPLETIME_387CYCLES_5) || ((__TIME__) == ADC_SAMPLETIME_810CYCLES_5))
 ...
 * value (unit: uV/DegCelsius). On STM32MP1, refer to device datasheet parameter "Avg_Slope".
 * @param  __TEMPSENSOR_TYP_CALX_V__     Device datasheet data: Temperature sensor voltage typical
 * value (at temperature and Vref+ defined in parameters below) (unit: mV). On STM32MP1, refer to
...
HAL_StatusTypeDef HAL_ADC_AnalogWDGConfig(ADC_HandleTypeDef *hadc,
					  ADC_AnalogWDGConfTypeDef *pAnalogWDGConfig);
