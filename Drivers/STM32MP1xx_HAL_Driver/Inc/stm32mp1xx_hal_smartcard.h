...
typedef struct {
	uint32_t BaudRate; /*!< Configures the SmartCard communication baud rate.
				The baud rate register is computed using the following formula:
				   Baud Rate Register = ((usart_ker_ckpres) /
			      ((hsmartcard->Init.BaudRate))) where usart_ker_ckpres is the USART
			      input clock divided by a prescaler */

	uint32_t WordLength; /*!< Specifies the number of data bits transmitted or received in a
				frame. This parameter @ref SMARTCARD_Word_Length can only be set to
				9 (8 data + 1 parity bits). */
	...
	uint16_t CLKLastBit; /*!< Specifies whether the clock pulse corresponding to the last
				transmitted data bit (MSB) has to be output on the SCLK pin in
				synchronous mode. This parameter can be a value of @ref
				SMARTCARD_Last_Bit */

	uint16_t OneBitSampling; /*!< Specifies whether a single sample or three samples' majority
				    vote is selected. Selecting the single sample method increases
				      the receiver tolerance to clock deviations. This parameter can
				    be a value of @ref SMARTCARD_OneBit_Sampling. */

	uint8_t Prescaler; /*!< Specifies the SmartCard Prescaler.
				This parameter can be any value from 0x01 to 0x1F. Prescaler value
	...
	uint32_t ClockPrescaler; /*!< Specifies the prescaler value used to divide the USART clock
 ...
 */
typedef struct {
	uint32_t AdvFeatureInit; /*!< Specifies which advanced SMARTCARD features is initialized.
				    Several advanced features may be initialized at the same time.
				    This parameter can be a value of @ref
	...
	uint32_t DMADisableonRxError; /*!< Specifies whether the DMA is disabled in case of
					 reception error. This parameter can be a value of @ref
					 SMARTCARD_DMA_Disable_on_Rx_Error */

	uint32_t MSBFirst; /*!< Specifies whether MSB is sent first on UART line.
				This parameter can be a value of @ref SMARTCARD_MSB_First */

	uint16_t TxCompletionIndication; /*!< Specifies which transmission completion indication is
					    used: before (when relevant flag is available) or once
					    guard time period has elapsed. This parameter can be a
					    value of @ref
...
typedef struct __SMARTCARD_HandleTypeDef {
	...
	SMARTCARD_AdvFeatureInitTypeDef
		AdvancedInit; /*!< SmartCard advanced features initialization parameters */
	...
	void (*RxISR)(
		struct __SMARTCARD_HandleTypeDef *huart); /*!< Function pointer on Rx IRQ handler */

	void (*TxISR)(
		struct __SMARTCARD_HandleTypeDef *huart); /*!< Function pointer on Tx IRQ handler */
	...
	__IO HAL_SMARTCARD_StateTypeDef
		gState; /*!< SmartCard state information related to global
			     Handle management and also related to Tx operations.
			     This parameter can be a value
			     of @ref HAL_SMARTCARD_StateTypeDef                    */

	__IO HAL_SMARTCARD_StateTypeDef
		RxState; /*!< SmartCard state information related to Rx operations.
			      This parameter can be a value
			      of @ref HAL_SMARTCARD_StateTypeDef                    */
	...
	void (*TxCpltCallback)(struct __SMARTCARD_HandleTypeDef
				       *hsmartcard); /*!< SMARTCARD Tx Complete Callback */

	void (*RxCpltCallback)(struct __SMARTCARD_HandleTypeDef
				       *hsmartcard); /*!< SMARTCARD Rx Complete Callback */

	void (*ErrorCallback)(
		struct __SMARTCARD_HandleTypeDef *hsmartcard); /*!< SMARTCARD Error Callback */

	void (*AbortCpltCallback)(struct __SMARTCARD_HandleTypeDef
					  *hsmartcard); /*!< SMARTCARD Abort Complete Callback */

	void (*AbortTransmitCpltCallback)(
		struct __SMARTCARD_HandleTypeDef
			*hsmartcard); /*!< SMARTCARD Abort Transmit Complete Callback */

	void (*AbortReceiveCpltCallback)(
		struct __SMARTCARD_HandleTypeDef
			*hsmartcard); /*!< SMARTCARD Abort Receive Complete Callback  */

	void (*RxFifoFullCallback)(struct __SMARTCARD_HandleTypeDef
					   *hsmartcard); /*!< SMARTCARD Rx Fifo Full Callback */

	void (*TxFifoEmptyCallback)(struct __SMARTCARD_HandleTypeDef
					    *hsmartcard); /*!< SMARTCARD Tx Fifo Empty Callback */

	void (*MspInitCallback)(
		struct __SMARTCARD_HandleTypeDef *hsmartcard); /*!< SMARTCARD Msp Init callback */

	void (*MspDeInitCallback)(
		struct __SMARTCARD_HandleTypeDef *hsmartcard); /*!< SMARTCARD Msp DeInit callback */
...
typedef enum {
	...
	HAL_SMARTCARD_ABORT_TRANSMIT_COMPLETE_CB_ID =
		0x04U, /*!< SMARTCARD Abort Transmit Complete Callback ID */
	HAL_SMARTCARD_ABORT_RECEIVE_COMPLETE_CB_ID =
		0x05U, /*!< SMARTCARD Abort Receive Complete Callback ID  */
...
#define SMARTCARD_LASTBIT_DISABLE                                                                  \
	0x00000000U /*!< SMARTCARD frame last data bit clock pulse not output to SCLK pin */
#define SMARTCARD_LASTBIT_ENABLE                                                                   \
	USART_CR2_LBCL /*!< SMARTCARD frame last data bit clock pulse output to SCLK pin     */
/**
 * @}
 */
...
#define SMARTCARD_ONE_BIT_SAMPLE_ENABLE                                                            \
	USART_CR3_ONEBIT /*!< SMARTCARD frame one-bit sample enabled  */
/**
 * @}
 */
...
#define __HAL_SMARTCARD_CLEAR_PEFLAG(__HANDLE__)                                                   \
	__HAL_SMARTCARD_CLEAR_FLAG((__HANDLE__), SMARTCARD_CLEAR_PEF)

/** @brief  Clear the SMARTCARD FE pending flag.
 * @param  __HANDLE__ specifies the SMARTCARD Handle.
 * @retval None
 */
#define __HAL_SMARTCARD_CLEAR_FEFLAG(__HANDLE__)                                                   \
	__HAL_SMARTCARD_CLEAR_FLAG((__HANDLE__), SMARTCARD_CLEAR_FEF)

/** @brief  Clear the SMARTCARD NE pending flag.
 * @param  __HANDLE__ specifies the SMARTCARD Handle.
 * @retval None
 */
#define __HAL_SMARTCARD_CLEAR_NEFLAG(__HANDLE__)                                                   \
	__HAL_SMARTCARD_CLEAR_FLAG((__HANDLE__), SMARTCARD_CLEAR_NEF)
...
#define __HAL_SMARTCARD_CLEAR_OREFLAG(__HANDLE__)                                                  \
	__HAL_SMARTCARD_CLEAR_FLAG((__HANDLE__), SMARTCARD_CLEAR_OREF)

/** @brief  Clear the SMARTCARD IDLE pending flag.
 * @param  __HANDLE__ specifies the SMARTCARD Handle.
 * @retval None
 */
#define __HAL_SMARTCARD_CLEAR_IDLEFLAG(__HANDLE__)                                                 \
	__HAL_SMARTCARD_CLEAR_FLAG((__HANDLE__), SMARTCARD_CLEAR_IDLEF)

/** @brief  Check whether the specified Smartcard flag is set or not.
 * @param  __HANDLE__ specifies the SMARTCARD Handle.
 * @param  __FLAG__ specifies the flag to check.
 *        This parameter can be one of the following values:
 *            @arg @ref SMARTCARD_FLAG_TCBGT Transmission complete before guard time flag (when flag
 * available)
...
#define __HAL_SMARTCARD_GET_IT(__HANDLE__, __INTERRUPT__)                                          \
	((((__HANDLE__)->Instance->ISR &                                                           \
	   (0x01UL << (((__INTERRUPT__)&SMARTCARD_ISR_MASK) >> SMARTCARD_ISR_POS))) != 0U)         \
		 ? SET                                                                             \
		 : RESET)
 ...
 * @retval None
 */
#define __HAL_SMARTCARD_SEND_REQ(__HANDLE__, __REQ__)                                              \
 ...
 * @retval None
 */
#define __HAL_SMARTCARD_ONE_BIT_SAMPLE_ENABLE(__HANDLE__)                                          \
...
#define IS_SMARTCARD_PHASE(__CPHA__)                                                               \
	(((__CPHA__) == SMARTCARD_PHASE_1EDGE) || ((__CPHA__) == SMARTCARD_PHASE_2EDGE))
...
#define IS_SMARTCARD_ADVFEATURE_INIT(__INIT__)                                                     \
	((__INIT__) <=                                                                             \
	 (SMARTCARD_ADVFEATURE_NO_INIT | SMARTCARD_ADVFEATURE_TXINVERT_INIT |                      \
