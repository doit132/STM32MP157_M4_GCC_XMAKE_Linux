...
typedef struct {
	...
	uint32_t *Header;    /*!< used only in AES GCM and CCM Algorithm for authentication,
			      ...
			      CCM : named B1 composed of the associated data length and Associated
			      Data. */
	uint32_t HeaderSize; /*!< The size of header buffer in word  */
	uint32_t *B0;	     /*!< B0 is first authentication block used only  in AES CCM mode */
	uint32_t DataWidthUnit;	  /*!< Data With Unit, this parameter can be value of @ref
				     CRYP_Data_Width_Unit*/
	uint32_t KeyIVConfigSkip; /*!< CRYP peripheral Key and IV configuration skip, to config Key
				     and Initialization Vector only once and to skip configuration
				     for consecutive processings. This parameter can be a value of
				     @ref CRYP_Configuration_Skip */
	...
	#endif /* (USE_HAL_CRYP_REGISTER_CALLBACKS) */
{
	...
	#if (USE_HAL_CRYP_REGISTER_CALLBACKS == 1)
	void (*InCpltCallback)(struct __CRYP_HandleTypeDef
				       *hcryp); /*!< CRYP Input FIFO transfer completed callback  */
	void (*OutCpltCallback)(struct __CRYP_HandleTypeDef *
					hcryp); /*!< CRYP Output FIFO transfer completed callback */
	...
	void (*MspDeInitCallback)(
		struct __CRYP_HandleTypeDef *hcryp); /*!< CRYP Msp DeInit callback  */
	...
	#if (USE_HAL_CRYP_REGISTER_CALLBACKS == 1)
/** @defgroup HAL_CRYP_Callback_ID_enumeration_definition HAL CRYP Callback ID enumeration
 * definition
...
typedef enum {
	HAL_CRYP_INPUT_COMPLETE_CB_ID =
		0x01U, /*!< CRYP Input FIFO transfer completed callback ID */
	HAL_CRYP_OUTPUT_COMPLETE_CB_ID =
		0x02U,		      /*!< CRYP Output FIFO transfer completed callback ID */
...
typedef void (*pCRYP_CallbackTypeDef)(
	CRYP_HandleTypeDef *hcryp); /*!< pointer to a common CRYP callback function */
	...
	#define HAL_CRYP_ERROR_AUTH_TAG_SEQUENCE                                                   \
		0x00000040U /*!< Sequence are not respected only for GCM or CCM */
	#if (USE_HAL_CRYP_REGISTER_CALLBACKS == 1)
		#define HAL_CRYP_ERROR_INVALID_CALLBACK                                            \
	...
	#define CRYP_FLAG_BUSY                                                                     \
		CRYP_SR_BUSY /*!< The CRYP core is currently processing a block of data            \
	...
	#define CRYP_KEYIVCONFIG_ALWAYS                                                            \
		0x00000000U /*!< Peripheral Key and IV configuration to do systematically */
...
uint32_t HAL_CRYP_GetError(CRYP_HandleTypeDef *hcryp);
	...
	#define IS_CRYP_ALGORITHM(ALGORITHM)                                                       \
		(((ALGORITHM) == CRYP_DES_ECB) || ((ALGORITHM) == CRYP_DES_CBC) ||                 \
		 ((ALGORITHM) == CRYP_TDES_ECB) || ((ALGORITHM) == CRYP_TDES_CBC) ||               \
		 ((ALGORITHM) == CRYP_AES_ECB) || ((ALGORITHM) == CRYP_AES_CBC) ||                 \
		 ((ALGORITHM) == CRYP_AES_CTR) || ((ALGORITHM) == CRYP_AES_GCM) ||                 \
	...
	#define IS_CRYP_INIT(CONFIG)                                                               \
		(((CONFIG) == CRYP_KEYIVCONFIG_ALWAYS) || ((CONFIG) == CRYP_KEYIVCONFIG_ONCE))
