...
typedef struct {
	...
	uint32_t ChannelSelection; /*!< Specifies whether the control flow will take the channel
				      status from channel A or B. This parameter can be a value of
				      @ref SPDIFRX_Channel_Selection */
	...
	uint32_t PreambleTypeMask; /*!< Specifies whether The preamble type bits are copied or not
				      into the received frame. This parameter can be a value of @ref
				      SPDIFRX_PT_Mask */

	uint32_t ChannelStatusMask; /*!< Specifies whether the channel status and user bits are
				       copied or not into the received frame. This parameter can be
				       a value of @ref SPDIFRX_ChannelStatus_Mask */

	uint32_t ValidityBitMask; /*!< Specifies whether the validity bit is copied or not into the
				     received frame. This parameter can be a value of @ref
				     SPDIFRX_V_Mask */

	uint32_t ParityErrorMask; /*!< Specifies whether the parity error bit is copied or not into
				     the received frame. This parameter can be a value of @ref
				     SPDIFRX_PE_Mask */
...
typedef enum {
	...
	HAL_SPDIFRX_STATE_BUSY_CX =
		0x04U, /*!< SPDIFRX internal Control Flow RX process is ongoing    */
...
typedef struct {
	...
	DMA_HandleTypeDef *hdmaCsRx; /* SPDIFRX EC60958_channel_status and user_information DMA
					handle parameters */
	...
	#define SPDIFRX_CHANNELSTATUS_OFF                                                          \
		((uint32_t)0x00000000U) /* The channel status and user bits are copied into the    \
					   SPDIF_DR */
	#define SPDIFRX_CHANNELSTATUS_ON                                                           \
		((uint32_t)SPDIFRX_CR_CUMSK) /* The channel status and user bits are not copied    \
	...
	#define __HAL_SPDIFRX_RESET_HANDLE_STATE(__HANDLE__)                                       \
		((__HANDLE__)->State = (uint16_t)SPDIFRX_CR_SPDIFEN)
...
uint32_t HAL_SPDIFRX_GetError(SPDIFRX_HandleTypeDef *hspdif);
	...
	#define IS_SPDIFRX_MAX_RETRIES(RET)                                                        \
		(((RET) == SPDIFRX_MAXRETRIES_NONE) || ((RET) == SPDIFRX_MAXRETRIES_3) ||          \
	...
	#define IS_PREAMBLE_TYPE_MASK(VAL)                                                         \
		(((VAL) == SPDIFRX_PREAMBLETYPEMASK_ON) || ((VAL) == SPDIFRX_PREAMBLETYPEMASK_OFF))
	#define IS_VALIDITY_MASK(VAL)                                                              \
		(((VAL) == SPDIFRX_VALIDITYMASK_OFF) || ((VAL) == SPDIFRX_VALIDITYMASK_ON))
	#define IS_PARITY_ERROR_MASK(VAL)                                                          \
		(((VAL) == SPDIFRX_PARITYERRORMASK_OFF) || ((VAL) == SPDIFRX_PARITYERRORMASK_ON))
	#define IS_SPDIFRX_CHANNEL(CHANNEL)                                                        \
		(((CHANNEL) == SPDIFRX_CHANNEL_A) || ((CHANNEL) == SPDIFRX_CHANNEL_B))
	#define IS_SPDIFRX_DATA_FORMAT(FORMAT)                                                     \
		(((FORMAT) == SPDIFRX_DATAFORMAT_LSB) || ((FORMAT) == SPDIFRX_DATAFORMAT_MSB) ||   \
		 ((FORMAT) == SPDIFRX_DATAFORMAT_32BITS))
	#define IS_STEREO_MODE(MODE)                                                               \
		(((MODE) == SPDIFRX_STEREOMODE_DISABLE) || ((MODE) == SPDIFRX_STEREOMODE_ENABLE))

	#define IS_CHANNEL_STATUS_MASK(VAL)                                                        \
		(((VAL) == SPDIFRX_CHANNELSTATUS_ON) || ((VAL) == SPDIFRX_CHANNELSTATUS_OFF))
	/**
	 * @}
	 */
