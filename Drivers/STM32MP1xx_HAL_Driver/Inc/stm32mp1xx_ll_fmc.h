...
#define IS_FMC_NORSRAM_BANK(__BANK__)                                                              \
	(((__BANK__) == FMC_NORSRAM_BANK1) || ((__BANK__) == FMC_NORSRAM_BANK2) ||                 \
	 ((__BANK__) == FMC_NORSRAM_BANK3) || ((__BANK__) == FMC_NORSRAM_BANK4))
#define IS_FMC_MUX(__MUX__)                                                                        \
	(((__MUX__) == FMC_DATA_ADDRESS_MUX_DISABLE) || ((__MUX__) == FMC_DATA_ADDRESS_MUX_ENABLE))
#define IS_FMC_MEMORY(__MEMORY__)                                                                  \
	(((__MEMORY__) == FMC_MEMORY_TYPE_SRAM) || ((__MEMORY__) == FMC_MEMORY_TYPE_PSRAM) ||      \
...
#define IS_FMC_PAGESIZE(__SIZE__)                                                                  \
	(((__SIZE__) == FMC_PAGE_SIZE_NONE) || ((__SIZE__) == FMC_PAGE_SIZE_128) ||                \
	 ((__SIZE__) == FMC_PAGE_SIZE_256) || ((__SIZE__) == FMC_PAGE_SIZE_512) ||                 \
	 ((__SIZE__) == FMC_PAGE_SIZE_1024))
#define IS_FMC_ACCESS_MODE(__MODE__)                                                               \
	(((__MODE__) == FMC_ACCESS_MODE_A) || ((__MODE__) == FMC_ACCESS_MODE_B) ||                 \
	 ((__MODE__) == FMC_ACCESS_MODE_C) || ((__MODE__) == FMC_ACCESS_MODE_D))
#define IS_FMC_NBL_SETUPTIME(__NBL__)                                                              \
	(((__NBL__) == FMC_NBL_SETUPTIME_0) || ((__NBL__) == FMC_NBL_SETUPTIME_1) ||               \
	 ((__NBL__) == FMC_NBL_SETUPTIME_2) || ((__NBL__) == FMC_NBL_SETUPTIME_3))
...
#define IS_FMC_WAIT_SIGNAL_ACTIVE(__ACTIVE__)                                                      \
	(((__ACTIVE__) == FMC_WAIT_TIMING_BEFORE_WS) || ((__ACTIVE__) == FMC_WAIT_TIMING_DURING_WS))
...
#define IS_FMC_WAITE_SIGNAL(__SIGNAL__)                                                            \
	(((__SIGNAL__) == FMC_WAIT_SIGNAL_DISABLE) || ((__SIGNAL__) == FMC_WAIT_SIGNAL_ENABLE))
#define IS_FMC_EXTENDED_MODE(__MODE__)                                                             \
	(((__MODE__) == FMC_EXTENDED_MODE_DISABLE) || ((__MODE__) == FMC_EXTENDED_MODE_ENABLE))
...
#define IS_FMC_WRITE_BURST(__BURST__)                                                              \
	(((__BURST__) == FMC_WRITE_BURST_DISABLE) || ((__BURST__) == FMC_WRITE_BURST_ENABLE))
...
typedef struct {
	...
	uint32_t
		BurstAccessMode; /*!< Enables or disables the burst access mode for Flash memory,
				      valid only with synchronous burst Flash memories.
				      This parameter can be a value of @ref FMC_Burst_Access_Mode */

	uint32_t WaitSignalPolarity; /*!< Specifies the wait signal polarity, valid only when
					accessing the Flash memory in burst mode. This parameter can
					be a value of @ref FMC_Wait_Signal_Polarity             */
	...
	uint32_t WriteOperation; /*!< Enables or disables the write operation in the selected device
				    by the FMC. This parameter can be a value of @ref
				    FMC_Write_Operation                  */
	...
	uint32_t AsynchronousWait; /*!< Enables or disables wait signal during asynchronous
				      transfers, valid only with asynchronous Flash memories. This
				      parameter can be a value of @ref FMC_AsynchronousWait */

	uint32_t WriteBurst; /*!< Enables or disables the write burst operation.
				  This parameter can be a value of @ref FMC_Write_Burst */

	uint32_t ContinuousClock; /*!< Enables or disables the FMC clock output to external memory
				     devices. This parameter is only enabled through the FMC_BCR1
				     register, and don't care through FMC_BCR2..4 registers. This
				     parameter can be a value of @ref FMC_Continous_Clock */
	...
	FunctionalState MaxChipSelectPulse; /*!< Enables or disables the maximum chip select pulse
					       management in this NSBank for PSRAM refresh. This
					       parameter can be set to ENABLE or DISABLE */

	uint32_t MaxChipSelectPulseTime; /*!< Specifies the maximum chip select pulse time in
					    FMC_CLK cycles for synchronous accesses and in HCLK
					    cycles for asynchronous accesses, valid only if
					    MaxChipSelectPulse is ENABLE. This parameter can be a
					    value between Min_Data = 1 and Max_Data = 65535.
...
typedef struct {
	uint32_t AddressSetupTime; /*!< Defines the number of HCLK cycles to configure
					...
					This parameter can be a value between Min_Data = 0 and
				      Max_Data = 15.
					@note This parameter is not used with synchronous NOR Flash
				      memories.      */

	uint32_t AddressHoldTime; /*!< Defines the number of HCLK cycles to configure
				       the duration of the address hold time.
				       This parameter can be a value between Min_Data = 1 and
				     Max_Data = 15.
				       @note This parameter is not used with synchronous NOR Flash
				     memories.      */
	...
	uint32_t DataSetupTime; /*!< Defines the number of HCLK cycles to configure
				     ...
				     This parameter can be a value between Min_Data = 1 and Max_Data
				   = 255.
	...
	uint32_t CLKDivision; /*!< Defines the period of CLK clock output signal, expressed in
				 number of HCLK cycles. This parameter can be a value between
				 Min_Data = 2 and Max_Data = 16.
				   @note This parameter is not used for asynchronous NOR Flash, SRAM
				 or ROM accesses. */
 ...
 *  @{
 */
/** @defgroup FMC_LL_NORSRAM_Private_Functions_Group1 NOR SRAM Initialization/de-initialization
 * functions
 *  @{
 */
