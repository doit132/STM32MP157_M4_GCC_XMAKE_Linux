 ...
 */
typedef struct {
	uint32_t ClockPrescaler; /* Specifies the prescaler factor for generating clock based on the
				    AHB clock. This parameter can be a number between 0 and 255 */
	uint32_t FifoThreshold; /* Specifies the threshold number of bytes in the FIFO (used only in
				   indirect mode) This parameter can be a value between 1 and 32 */
	uint32_t SampleShifting; /* Specifies the Sample Shift. The data is sampled 1/2 clock cycle
				    delay later to take in account external signal delays. (It
				    should be QSPI_SAMPLE_SHIFTING_NONE in DDR mode) This parameter
				    can be a value of @ref QSPI_SampleShifting */
	uint32_t FlashSize; /* Specifies the Flash Size. FlashSize+1 is effectively the number of
			       address bits required to address the flash memory. The flash capacity
			       can be up to 4GB (addressed using 32 bits) in indirect mode, but the
			       addressable space in memory-mapped mode is limited to 256MB This
			       parameter can be a number between 0 and 31 */
	uint32_t ChipSelectHighTime; /* Specifies the Chip Select High Time. ChipSelectHighTime+1
					defines the minimum number of clock cycles which the chip
					select must remain high between commands. This parameter can
					be a value of @ref QSPI_ChipSelectHighTime */
	uint32_t ClockMode; /* Specifies the Clock Mode. It indicates the level that clock takes
			       between commands. This parameter can be a value of @ref
			       QSPI_ClockMode */
...
typedef struct {
	...
	uint32_t Address;     /* Specifies the Address to be sent (Size from 1 to 4 bytes according
				 AddressSize)     This parameter can be a value (32-bits) between 0x0 and
				 0xFFFFFFFF */
	...
	uint32_t NbData;  /* Specifies the number of data to transfer. (This is the number of bytes)
			     This parameter can be any value between 0 and 0xFFFFFFFF (0 means
			     undefined length  until end of memory)*/
	uint32_t DdrMode; /* Specifies the double data rate mode for address, alternate byte and
			     data phase This parameter can be a value of @ref QSPI_DdrMode */
	uint32_t DdrHoldHalfCycle; /* Specifies if the DDR hold is enabled. When enabled it delays
				      the data output by one half of system clock in DDR mode. This
				      parameter can be a value of @ref QSPI_DdrHoldHalfCycle */
 ...
 */
typedef struct {
	uint32_t Match; /* Specifies the value to be compared with the masked status register to get
			   a match. This parameter can be any value between 0 and 0xFFFFFFFF */
	uint32_t Mask;	/* Specifies the mask to be applied to the status bytes received.
			   This parameter can be any value between 0 and 0xFFFFFFFF */
	uint32_t
		Interval; /* Specifies the number of clock cycles between two read during automatic
 ...
 */
typedef struct {
	uint32_t TimeOutPeriod;	    /* Specifies the number of clock to wait when the FIFO is full
				       before to release the chip select.     This parameter can be any
				       value between 0 and 0xFFFF */
	uint32_t TimeOutActivation; /* Specifies if the timeout counter is enabled to release the
				       chip select. This parameter can be a value of @ref
...
typedef void (*pQSPI_CallbackTypeDef)(QSPI_HandleTypeDef *hqspi);
	...
	#define QSPI_CS_HIGH_TIME_2_CYCLE                                                          \
		((uint32_t)QUADSPI_DCR_CSHT_0) /*!<nCS stay high for at least 2 clock cycles       \
						  between commands*/
	#define QSPI_CS_HIGH_TIME_3_CYCLE                                                          \
		((uint32_t)QUADSPI_DCR_CSHT_1) /*!<nCS stay high for at least 3 clock cycles       \
						  between commands*/
	#define QSPI_CS_HIGH_TIME_4_CYCLE                                                          \
		((uint32_t)QUADSPI_DCR_CSHT_0 |                                                    \
		 QUADSPI_DCR_CSHT_1) /*!<nCS stay high for at least 4 clock cycles between         \
					commands*/
	#define QSPI_CS_HIGH_TIME_5_CYCLE                                                          \
		((uint32_t)QUADSPI_DCR_CSHT_2) /*!<nCS stay high for at least 5 clock cycles       \
						  between commands*/
	#define QSPI_CS_HIGH_TIME_6_CYCLE                                                          \
		((uint32_t)QUADSPI_DCR_CSHT_2 |                                                    \
		 QUADSPI_DCR_CSHT_0) /*!<nCS stay high for at least 6 clock cycles between         \
					commands*/
	#define QSPI_CS_HIGH_TIME_7_CYCLE                                                          \
		((uint32_t)QUADSPI_DCR_CSHT_2 |                                                    \
		 QUADSPI_DCR_CSHT_1) /*!<nCS stay high for at least 7 clock cycles between         \
					commands*/
	#define QSPI_CS_HIGH_TIME_8_CYCLE                                                          \
		((uint32_t)QUADSPI_DCR_CSHT) /*!<nCS stay high for at least 8 clock cycles between \
						commands*/
	/**
	 * @}
	 */
	...
	#define QSPI_CLOCK_MODE_3                                                                  \
		((uint32_t)QUADSPI_DCR_CKMODE) /*!<Clk goes high while nCS is released*/
	/**
	 * @}
	 */
	...
	#define QSPI_ALTERNATE_BYTES_16_BITS                                                       \
		((uint32_t)QUADSPI_CCR_ABSIZE_0) /*!<16-bit alternate bytes*/
	#define QSPI_ALTERNATE_BYTES_24_BITS                                                       \
		((uint32_t)QUADSPI_CCR_ABSIZE_1) /*!<24-bit alternate bytes*/
	#define QSPI_ALTERNATE_BYTES_32_BITS                                                       \
		((uint32_t)QUADSPI_CCR_ABSIZE) /*!<32-bit alternate bytes*/
	/**
	 * @}
	 */
	...
	#define QSPI_INSTRUCTION_1_LINE                                                            \
		((uint32_t)QUADSPI_CCR_IMODE_0) /*!<Instruction on a single line*/
	#define QSPI_INSTRUCTION_2_LINES                                                           \
		((uint32_t)QUADSPI_CCR_IMODE_1) /*!<Instruction on two lines*/
	#define QSPI_INSTRUCTION_4_LINES                                                           \
		((uint32_t)QUADSPI_CCR_IMODE) /*!<Instruction on four lines*/
	/**
	 * @}
	 */
	...
	#define QSPI_ADDRESS_1_LINE	  ((uint32_t)QUADSPI_CCR_ADMODE_0) /*!<Address on a single \
									      line*/
	...
	#define QSPI_DDR_MODE_ENABLE                                                               \
		((uint32_t)QUADSPI_CCR_DDRM) /*!<Double data rate mode enabled*/
	/**
	 * @}
	 */
	...
	#define QSPI_DDR_HHC_HALF_CLK_DELAY                                                        \
		((uint32_t)QUADSPI_CCR_DHHC) /*!<Delay the data output by one half of system clock \
						in DDR mode*/
	/**
	 * @}
	 */
	...
	#define QSPI_MATCH_MODE_OR                                                                 \
		((uint32_t)QUADSPI_CR_PMM) /*!<OR match mode between unmasked bits*/
	/**
	 * @}
	 */
	...
	#define QSPI_TIMEOUT_COUNTER_DISABLE                                                       \
		0x00000000U /*!<Timeout counter disabled, nCS remains active*/
	#define QSPI_TIMEOUT_COUNTER_ENABLE                                                        \
		((uint32_t)QUADSPI_CR_TCEN) /*!<Timeout counter enabled, nCS released when timeout \
					       expires*/
	/**
	 * @}
	 */
	...
	#define QSPI_FLAG_TO   QUADSPI_SR_TOF  /*!<Timeout flag: timeout occurs in memory-mapped   \
						  mode*/
	#define QSPI_FLAG_SM                                                                       \
		QUADSPI_SR_SMF /*!<Status match flag: received data matches in autopolling mode*/
	#define QSPI_FLAG_FT                                                                       \
		QUADSPI_SR_FTF /*!<Fifo threshold flag: Fifo threshold reached or data left after  \
				  read from memory is complete*/
	#define QSPI_FLAG_TC                                                                       \
		QUADSPI_SR_TCF /*!<Transfer complete flag: programmed number of data have been     \
				  transferred or the transfer has been aborted*/
	#define QSPI_FLAG_TE                                                                       \
		QUADSPI_SR_TEF /*!<Transfer error flag: invalid address is being accessed*/
	/**
	 * @}
	 */
